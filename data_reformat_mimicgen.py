...
if __name__ == "__main__":
    ...
    for ep in demos:
        ...
        action_pcd_arrays.extend(pointcloud[:, 1024:, :3])  # gripper pcd
        anchor_pcd_arrays.extend(pointcloud[:, :1024, :3])  # whole scene pcd
        ...
        for i in range(len_episode):
            tax3d_demo = {
                'action_pc': pointcloud[i, 1024:, :3],
                'action_seg': np.ones(pointcloud[i, 1024:].shape[0]),
                'anchor_pc': pointcloud[i, :1024, :3],
                'anchor_seg': np.ones(pointcloud[i, :1024].shape[0]),
                'state': states[i],
                'gripper_flow': goal_gripper_pcd[i, :, :3] - pointcloud[i, 1024:, :3],
            }
