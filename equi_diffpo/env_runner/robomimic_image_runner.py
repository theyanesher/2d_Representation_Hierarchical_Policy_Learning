...
from equi_diffpo.common.goal_to_image_converter import Goal2ImageConverter
...
class RobomimicImageRunner(BaseImageRunner):
    ...
    def __init__(self, 
        ...
        if self.conditioning_type is not None:
            self.goal_to_image_converter = Goal2ImageConverter(conditioning_type=self.conditioning_type) #'3d_flow_world_frame')
    ...
    def run(self, high_level_policy, low_level_policy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            while not done:
                ...
                with torch.no_grad():
                    if self.high_level == 'weighted_displacement':
                        subgoal_pred, weights = ru.run_high_level_policy_inference(high_level_policy, obs_dict,
                                                                        return_weights=True)
                    elif self.high_level == 'gmm':
                        subgoal_pred, weights = ru.run_high_level_gmm_inference(high_level_policy, obs_dict,
                                                                                return_weights=True)
                subgoal_pred = subgoal_pred.repeat(1,self.n_obs_steps,1,1)
                if self.conditioning_type is not None:
                    obs_dict = self.goal_to_image_converter.generate_image_conditioning(
                        subgoal_pred, obs_dict)
                # run policy
                with torch.no_grad():
                ...
                if self.conditioning_type in ('3d_flow_world_frame', '2d_flow_world_frame', 'heatmap_goal_conditioning'):
                    output_list.append({
                        'agentview_image': obs_dict['agentview_image_84'].detach().cpu().numpy(),
                        'robot0_eye_in_hand_image': obs_dict['robot0_eye_in_hand_image_84'].detach().cpu().numpy(),
                        'agentview_cond': obs_dict['agentview_cond_84'].detach().cpu().numpy(),
                        'robot0_eye_in_hand_cond': obs_dict['robot0_eye_in_hand_cond_84'].detach().cpu().numpy()
                    })
                else: 
                    output_list.append({
                        'agentview_image': obs_dict['agentview_image_84'].detach().cpu().numpy(),
            ...
            for i, video_path in enumerate(all_video_paths[this_global_slice]):
                ...
                for j, output_dict in enumerate(output_list):
                    with open(video_dir / f'{j}.pkl', 'wb') as f:
                        save_dict = {
                            'agentview_image': output_dict['agentview_image'][i, -1].transpose(1,2,0),
                            'robot0_eye_in_hand_image': output_dict['robot0_eye_in_hand_image'][i, -1].transpose(1,2,0),
                            }
                        if self.conditioning_type in ('3d_flow_world_frame', '2d_flow_world_frame', 'heatmap_goal_conditioning'):
                            save_dict['agentview_cond'] = output_dict['agentview_cond'][i, -1].transpose(1,2,0)
                            save_dict['robot0_eye_in_hand_cond'] = output_dict['robot0_eye_in_hand_cond'][i, -1].transpose(1,2,0)
