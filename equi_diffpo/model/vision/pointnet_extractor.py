...
from equi_diffpo.model.vision.more_pointnet_extractors import PN2Act3dEncoder
from third_party.robogen.test_PointNet2.model_invariant import PointNetSetAbstractionMsg, PointNetFeaturePropagation
...
class PointNet2Encoder(nn.Module):
    ...
    PointNet++ Encoder
    """
    def __init__(self, input_channel=3, output_channels=256, keep_gripper_in_fps=False, use_group_norm=True):
        """
        use_in: whether to use instance norm of batch norm
        """
        super(PointNet2Encoder, self).__init__()
        self.sa1 = PointNetSetAbstractionMsg(npoint=1024, radius_list=[0.05, 0.1], nsample_list=[16, 32], in_channel=input_channel - 3, mlp_list=[[16, 16, 32], [32, 32, 64]])
        ...
        self.sa3 = PointNetSetAbstractionMsg(64, [0.2, 0.4], [16, 32], 128+128, [[128, 192, 256], [128, 192, 256]])
        self.final_projection = nn.Linear(512, output_channels)
        cprint("[PointNet2Encoder] use_group norm: {}".format(use_group_norm), 'cyan')
        cprint("[PointNet2Encoder] keep_gripper_in_fps: {}".format(keep_gripper_in_fps), 'cyan')

        if use_group_norm:
            for i in range(1, 4):
                sa = getattr(self, f"sa{i}")
                print(f'replacing batch norm in sa{i} with group norm')
                replace_bn_with_gn(sa)

    def forward(self, xyz):
        # [BxN, 4500, 3]
        if len(xyz.shape) == 4:
            B, H, N, _ = xyz.shape
            xyz = xyz.reshape(B*H, N, 3)
        ...
        # features = torch.max(l4_points, dim=-1)[0] # (B, 1024)
        features = torch.mean(l3_points, dim=-1) # (B, 1024)
    ...
    def __init__(self, 
        ...
        if pointnet_type == "pointnet":
            ...
            if goal_conditioning:
                ...
                self.n_output_channels = 240
        elif pointnet_type == 'pointnet2':
            self.extractor = PointNet2Encoder(input_channel=6, 
                                              output_channels=self.n_output_channels,
                                              keep_gripper_in_fps=False,
                                              use_group_norm=True)
        elif pointnet_type == 'pn2act3d':
            self.extractor = PN2Act3dEncoder()
            self.n_output_channels = 480
        else:
