...
def infer_contact_graspnet(model, pcd, topk=10, device=torch.device("cuda"), siglip_embedding=None, articubot_format=False):
    ...
    with torch.no_grad():
        ...
        pred = model(pcd, build_grasp=True, embedding=embedding, articubot_format=articubot_format) 
...
if __name__ == "__main__":
    ...
    parser.add_argument("--articubot_format", type=int, default=0)
    ...
    if args.model_type == "pointnet++":
        ...
        if m2t2_config['general'].category_embedding_type == "siglip":
            project_dir = os.environ["PROJECT_DIR"]
            siglip_text_features = torch.load(os.path.join(project_dir, "siglip_text_features.pt")).float().to("cuda")
            siglip_text_features = siglip_text_features[-1]    
    ...
    elif args.model_type == 'original_cgn':
        # TODO: load and evaluate the original contact graspnet model
        pass
    elif args.model_type == 'original_m2t2':
        # TODO: load and evaluate the original m2t2 model
        pass
    ...
    for scene_path in scene_path_list:
        ...
        if args.model_type == 'pointnet++':
            pred_grasps = infer_contact_graspnet(model, pc_in_camera, topk=10, siglip_embedding=siglip_text_features, articubot_format=args.articubot_format)
        elif args.model_type == 'm2t2':
