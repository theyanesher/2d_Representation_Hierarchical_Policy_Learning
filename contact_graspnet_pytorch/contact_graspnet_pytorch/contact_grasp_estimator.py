...
class GraspEstimator:
    ...
    def predict_grasps(self, pc, constant_offset=False, convert_cam_coords=True, forward_passes=1):
        ...
        if self._contact_grasp_cfg['TEST'].get("max_num_grasps") is None:
            ...
            print("taking the top 10 score grasp per object")
            topk_score_indices = np.argsort(pred_scores)[::-1][:self._contact_grasp_cfg['TEST']['max_num_grasps']]
            ...
            return pred_grasps_cam[topk_score_indices], pred_scores[topk_score_indices], pred_points[topk_score_indices].squeeze(), gripper_openings[topk_score_indices].squeeze()
    ...
    def predict_scene_grasps(self, pc_full, pc_segments={}, local_regions=False, filter_grasps=False, forward_passes=1, 
        ...
        if local_regions:
            ...
            if use_cam_boxes:
                ...
                pc_regions, _ = self.extract_3d_cam_boxes(pc_full, pc_segments, min_size=0.3, max_size=0.6)
            else:
                pc_regions = self.filter_pc_segments(pc_segments) 
            for k, pc_region in pc_regions.items():
                print("predicting grasps for object {}".format(k))
        ...
        if filter_grasps:
            ...
            for k in segment_keys:
                print("filter grasps for object {}".format(k))
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    ...
                    if local_regions and np.any(pred_grasps_cam[k]):
                        ...
                        print("no predicted contact is on segmented object ", len(segment_idcs))
                else:
    ...
    def select_grasps(self, contact_pts, contact_conf, max_farthest_points = 150, num_grasps = 200, first_thres = 0.25, second_thres = 0.2, with_replacement=False):
        ...
        # print("first threshold: {}, second threshold: {}, max farthest points: {}, num grasps: {}, with replacement: {}".format(first_thres, second_thres, max_farthest_points, num_grasps, with_replacement))
