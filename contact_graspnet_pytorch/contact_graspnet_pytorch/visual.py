...
from contact_graspnet_pytorch.visualization_utils_o3d import draw_grasps, plot_coordinates
import matplotlib.pyplot as plt

def visualize_grasps(full_pc, pred_grasps_cam, gt_grasps_cam, plot_opencv_cam=False, pc_colors=None, gripper_openings=None, gripper_width=0.08,
    ...
    pred_color = [(0.0, 1.0, 0.0) for i in range(len(pred_grasps_cam))]  # Green
    gt_color = [[1, 0, 0] for _ in range(len(gt_grasps_cam))]  # Red

    gripper_openings = np.ones(len(pred_grasps_cam))*gripper_width

    # import pdb; pdb.set_trace()
    draw_grasps(vis, pred_grasps_cam, np.eye(4), colors=pred_color, gripper_openings=gripper_openings)
    # draw_grasps(vis, gt_grasps_cam, np.eye(4), colors=gt_color, gripper_openings=gripper_openings)
...
with torch.no_grad():
    ...
    for val_it, data in enumerate(tqdm(test_dataloader)):
        ...
        pc_full = pc_cam.cpu().numpy()[0]
        pred_grasps_cam = pred_grasps_cam[np.arange(B)[:, None], top_k_score_idx][:, :top_k][0]
        min_dist_gt_pose = gt_pose[np.arange(B)[:, None], min_dist_idx][0]
        visualize_grasps(pc_full, pred_grasps_cam, min_dist_gt_pose)
