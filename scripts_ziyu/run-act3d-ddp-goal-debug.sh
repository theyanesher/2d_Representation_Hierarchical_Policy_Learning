...
exp_name="debug"
dataset_prefix=/scratch/ziyu/dp3_demo

torchrun --standalone --nproc_per_node=1 \
    train_ddp.py --config-name=dp3.yaml task=robogen_open_door exp_name="${exp_name}" eval_first=0  \
    task.dataset.zarr_path="[${dataset_prefix}/${save_data_name_0},${dataset_prefix}/${save_data_name_1}]" \
    task.env_runner.demo_experiment_path="[${dataset_prefix}/${save_data_name_0},${dataset_prefix}/${save_data_name_1}]" \
    task.env_runner.experiment_name="[${demo_name_0},${demo_name_1}]" \
    task.env_runner.experiment_folder="[${exp_folder_0},${exp_folder_1}]" \
    ...
    policy.act3d_encoder_cfg.pointcloud_backbone=unet \
    task.dataset.dataset_keys="['state', 'action', 'point_cloud', 'gripper_pcd', 'displacement_gripper_to_object', 'goal_gripper_pcd', 'feature_map', 'pcd_mask']" \
    policy.noise_model_type=transformer \
    policy.diffusion_attn_embed_dim=240 \
    # policy.act3d_encoder_cfg.use_attn_for_point_features=true \
