...
```
- Run - `bash update_envs.sh` (one-time, after activating conda env)
...
```sh
bash shell_scripts/train_diffPo.sh
```


- Generate data:
```
python -m sandbox.mino.dataset_modifications.add_goal_to_image --task square_d2 --src_h5_suffix pcd_abs_images.hdf5 --dst_h5_suffix pcd_abs_images_heatmapGoal.hdf5 --conditioning-type heatmap_goal_conditioning
```
