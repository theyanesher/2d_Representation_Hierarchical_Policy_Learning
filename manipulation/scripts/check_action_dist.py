...
def parallel_render(args):
    ...
    observation = simulator._get_observation(render=False)
...
def extract_demos_from_a_directory(dirtory_path, object_category, exp_name=None, in_gripper_frame=False, parallel=True, 
    ...
    for task_path in task_paths[args.task_beg_idx:args.task_end_idx]:
        ...
        all_exp_paths = [store_label_path.split("/")[-2] for store_label_path in store_label_paths]
        ...
        for traj_idx, (pc, pos_ori, stage_length, store_label_path) in tqdm.tqdm(enumerate(zip(ret_pc, ret_pos_ori, stages, store_label_paths))):
            ...
            cprint(f"Plotting {experiment_name}", "blue")
            # if traj_idx % 1 == 0:     
                # try:   
            reach_handle_idx = stage_length.get("reach_handle", 0)
            total_translation = np.sum(np.linalg.norm(np.array(traj_actions[:reach_handle_idx])[:, :3], axis=1))
            total_orientation = np.sum(quaternion_diffs[:reach_handle_idx])
            info = {
                "total_translation_for_reaching": total_translation,
                "total_orientation_for_reaching": total_orientation,
                "reach_handle_time": reach_handle_idx,
                "average_translation_for_reaching": total_translation / reach_handle_idx,
                "average_orientation_for_reaching": total_orientation / reach_handle_idx,
                "max_translation_for_reaching": np.max(np.linalg.norm(np.array(traj_actions[:reach_handle_idx])[:, :3], axis=1)),
                "max_orientation_for_reaching": np.max(quaternion_diffs[:reach_handle_idx]),
            }
            with open(os.path.join(action_dist_save_path, "{}.json".format(experiment_name)), "w") as f:
            ...
            # save_fig_path = os.path.join(action_dist_save_path, "delta_distribution_{}.png".format(experiment_name))
