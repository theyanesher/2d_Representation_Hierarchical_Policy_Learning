...
def approach_object_link_parallel(simulator, object_name, link_name, invert=False, debug=False):    
    ...
    # pcd.points = o3d.utility.Vector3dVector(link_pc)
    ...
    # print(all_handle_pos, handle_joint_id, axis_world, axis_end_world)
    ...
    # print("distance to line: ", distance_pc.shape)
    ...
    # p.addUserDebugPoints(to_try_handle_points, [[1, 0, 0] for _ in range(len(to_try_handle_points))], 10, 0, physicsClientId=simulator.id)
    # import pdb; pdb.set_trace()
    ...
    for target_pos in to_try_handle_points:
        ...
        if object_name == "laptop" or object_name == "toilet" or object_name == "foldingchair" or object_name == "bucket":
            axis_dir = axis - axis_end
            target_pos_displacement = target_pos - axis_end
            align_normal = target_pos_displacement - np.dot(target_pos_displacement, axis_dir) * axis_dir
            # if object_name == 'bucket':
            #     align_normal[2] = 0
            align_normal = align_normal / np.linalg.norm(align_normal)
            # print("align normal: ", align_normal)
        ...
        for normal in [align_normal]:
            if simulator.robot_name == 'panda':
                ...
                real_target_pos = target_pos + normal * -0.01
    ...
    # with open("data/debug/paper_plot_data_2.pkl", "wb") as f:
    #     pickle.dump([all_traj_states, all_traj_rgbs, all_stage_lengths, door_opened_ratios], f)
    # exit()
...
def close_gripper(simulator, handle_pc):
    intermediate_states = []
    rgbs = []
    # close_steps = 40 if simulator.robot_name == "panda" else 120
    close_steps = 40 
    ...
    ### TODO: implement the closing of gripper for xarm
    for t in range(close_steps):
        ...
        if left_collision and right_collision and simulator.robot_name == "panda":
            break
        
    # if simulator.robot_name == 'xarm':
    #     intermediate_states = intermediate_states[::3]
    #     rgbs = rgbs[::3]
...
def open_gripper(simulator):
    intermediate_states = []
    rgbs = []
    # open_steps = 40 if simulator.robot_name == "panda" else 120
    open_steps = 40 
    ...
    for t in range(open_steps):
        ...
        if simulator.robot_name == "xarm":
            if p.getJointState(agent.body, agent.right_gripper_indices[0], physicsClientId=simulator.id)[0] > 0.83:
                break
...
def open_door(simulator, object_name, link_name, handle_joint_id, handle_pc, invert=False):
    ...
    for t in range(1, timesteps):
        ...
        # print("joint angle: ", joint_angle, "new link pos: ", new_link_pos, "new eef pos: ", new_eef_pos, )
        ...
        if simulator.robot_name == "panda":
            ...
            for _ in range(2):
                p.setJointMotorControlArray(simulator.robot.body, jointIndices=simulator.robot.right_arm_joint_indices, 
                                            controlMode=p.POSITION_CONTROL, targetPositions=ik_joint_angles, physicsClientId=simulator.id)
                simulator.robot.set_gripper_open_position(simulator.robot.right_gripper_indices, 
                                                            [simulator.robot.finger_fully_close_joint_angle, simulator.robot.finger_fully_close_joint_angle], set_instantly=False)
                p.stepSimulation(physicsClientId=simulator.id)
        
                ### check if gripper is still in contact with the handle
                points_left_finger = p.getContactPoints(bodyA=simulator.robot.body, linkIndexA=simulator.robot.right_gripper_indices[0], bodyB=simulator.urdf_ids[object_name], physicsClientId=simulator.id)
                points_right_finger = p.getContactPoints(bodyA=simulator.robot.body, linkIndexA=simulator.robot.right_gripper_indices[1], bodyB=simulator.urdf_ids[object_name], physicsClientId=simulator.id)
            ...
            ### if the contact points are not close enough to the handle points, break too
            # dist_left_contact_to_handle = scipy.spatial.distance.cdist(np.array([points_left_finger[6]]), handle_pc).min()
            # dist_right_contact_to_handle = scipy.spatial.distance.cdist(np.array([points_right_finger[6]]), handle_pc).min()
            # if dist_left_contact_to_handle > 0.01 and dist_right_contact_to_handle > 0.01:
            #     break
...
def parallel_motion_planning(args):
    ...
    # print("motion planning result: ", res)
