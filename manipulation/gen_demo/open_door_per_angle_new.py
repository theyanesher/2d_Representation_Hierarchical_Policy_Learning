...
def recover_and_save(env, start_state, new_demo_path, step_name, recover_eef_pos, recover_eef_quat, 
    ...
    backup_direction = np.array(recover_eef_pos) - np.array(reach_contact_target_pos)
    plan_contact_target_pos = reach_contact_target_pos + 0.003 * backup_direction
    translation_distance = np.linalg.norm(np.array(plan_contact_target_pos) - np.array(current_eef_pos))
    ...
    res, path, path_length = motion_planning(
        env, plan_contact_target_pos, reach_contact_target_orient, obstacles=obstacles, allow_collision_links=allow_collision_links, 
        smooth_path=True, interpolation_num=interpolation_num, goal_allow_collide=False)
        
    if res:    
        if path_length > 3 * translation_distance:
            return 
        ...
        stage_lengths['grasp_handle'] = len(path)
        stage_lengths['stage'].append("grasp_handle")
        ...
        stage_lengths["stage"].append("close_gripper")
        ...
        stage_lengths["stage"].append("open_door")
