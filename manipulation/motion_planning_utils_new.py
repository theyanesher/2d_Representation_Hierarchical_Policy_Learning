...
def motion_planning(env, target_pos, target_orientation, planner=None, 
                ...
                try_times=6, interpolation_num=None, goal_allow_collide=False, return_IK=False):
    ...
    for try_idx in range(try_times):
        ...
        if robot_target_joint_angle is None:
            ...
            while True:
                ...
                print("joint limit: ", np.all(target_joint_angle[env.robot.right_arm_joint_indices] >= ik_lower_limits[env.robot.right_arm_joint_indices]) \
                    and np.all(target_joint_angle[env.robot.right_arm_joint_indices] <= ik_upper_limits[env.robot.right_arm_joint_indices]))
                print("collision: ", pb_ompl_interface.is_state_valid(target_joint_angle))
