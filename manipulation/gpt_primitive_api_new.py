...
def approach_object_link(simulator, object_name, link_name, dynamics=False, grasp_handle=True, 
    ...
    object_name = object_name.lower()
    # link_pc = get_groundtruth_link_pc(simulator, object_name, link_name) 
    ...
    if grasp_handle is not None:
        ...
        median_points = []
        for i in range(len(all_handle_pos)):
            median_points.append(np.median(all_handle_pos[i], axis=0))
        median_points = np.array(median_points)
        median_points = median_points.reshape(-1, 3)
        all_pc_to_handle_distances = scipy.spatial.distance.cdist(object_pc, flattened_handle_pos)
        pc_to_handle_distance = all_pc_to_handle_distances.min(axis=1)

        # deal with the handle_joint_id
        transfered_handle_joint_id = []
        for i in range(len(all_handle_pos)):
            for j in range(len(all_handle_pos[i])):
                transfered_handle_joint_id.append(handle_joint_id[i])
        pc_to_handle_distance_idx_handle_pts = np.argmin(all_pc_to_handle_distances, axis=1)
        handle_joint_id_list = [transfered_handle_joint_id[i] for i in pc_to_handle_distance_idx_handle_pts]

        threshold = 0.01
        handle_pc = object_pc[pc_to_handle_distance < threshold]
        handle_joint_id_list = np.array(handle_joint_id_list)[pc_to_handle_distance < threshold]
        pc_to_median_distance = scipy.spatial.distance.cdist(handle_pc, median_points).min(axis=1)
    ...
    for it in range(1):
        ...
        if grasp_handle is not None:
            ...
            handle_joint_id = handle_joint_id_list[sorted_idx[it]]
        ...
        for normal in [align_normal, -align_normal]:
            ...
            if grasp_handle is not None:
                ...
                target_orientation = align_gripper_z_with_normal(-normal, horizontal=horizontal_grasp, randomize=randomize).as_quat()
            else:
                target_orientation = align_gripper_z_with_normal(-normal).as_quat()
            ...
            res, path, path_length = motion_planning(
                simulator, mp_target_pos, target_orientation, obstacles=obstacles, allow_collision_links=allow_collision_links, save_path=save_path, smooth_path=True)
            ...
            if res:
                ...
                for t in range(steps):
                    ...
                    if np.abs(current_joint_angle[0] - 0.04) < 0.001:
                        break
                        
                stage_length['open_gripper'] = t + 1
                ...
                for t in range(movement_steps):
                    ik_indices = [_ for _ in range(len(simulator.robot.right_arm_joint_indices))]
                    # decompose the translation into small delta actions
                ...
                stage_length['reach_to_contact'] = t + 1
                ...
                for t in range(close_steps):
                    ...
                    if after_collision_steps >= 5:
                        break

                if not (left_collision and right_collision):
                    score = 0
                
                stage_length['close_gripper'] = t + 1
...
def approach_object_link_parallel(simulator, object_name, link_name):    
    ...
    args = [[env_kwargs, object_name, real_target_poses[it], mp_target_poses[it], target_orientations[it],\
            handle_pc, handle_joint_id, save_path, ori_simulator_state, it, link_name, 
            # target_normals[it]] for it in range(len(target_orientations))]
    ...
    if len(door_opened_scores) > 0 and np.max(door_opened_scores) > 0.1:
        ...
        if not np.sum(door_opened_scores > 0.8) > 0:
            ...
            for idx, score in enumerate(door_opened_scores):
                if score > 0.8 and all_motion_planning_path_lengths[idx] < min_length:
                    best_idx = idx
                    min_length = all_motion_planning_path_lengths[idx]
...
def open_door(simulator, object_name, link_name, handle_joint_id):
    ...
    full_move_amount = 0.6 * (joint_limit[1] - joint_limit[0])
...
def parallel_motion_planning(args):
    ...
    if res:
        ...
        stage_length['reach_to_contact'] = len(rgbs)
...
def parallel_motion_planning_new(args):
    np.random.seed(time.time_ns() % 2**32)
    
    env_kwargs, object_name, real_target_pos, mp_target_pos, target_orientation, \
        handle_pc, handle_joint_id, save_path, ori_simulator_state, \
        it, link_name, normal = args
    ...
    debug_id = p.addUserDebugLine(mp_target_pos, mp_target_pos + normal, [1, 0, 0], 5, 0, physicsClientId=simulator.id)
    
    
    # TODO: find a collision-free IK for the target position and orientation
    # and then approaching following the normal direction unitl contact
    
    # all_objects = list(simulator.urdf_ids.keys())
    # all_objects.remove("robot")
    # obstacles = [simulator.urdf_ids[x] for x in all_objects]
    # allow_collision_links = []
    # target_joint_angle = motion_planning(
    #     simulator, mp_target_pos, target_orientation, obstacles=obstacles, allow_collision_links=allow_collision_links, 
    #     return_IK=True, try_times=1, max_sampling_it=500)
    ...
    if target_joint_angle:
                                
        # reach till contact is made
        contact_eef_pos, contact_eef_orient = reach_till_contact(simulator, real_target_pos, target_orientation, return_contact_pose=True)
        dir_contact_to_mp_target = mp_target_pos - contact_eef_pos
        dir_contact_to_mp_target = dir_contact_to_mp_target / np.linalg.norm(dir_contact_to_mp_target)
        real_mp_target_pos = contact_eef_pos + dir_contact_to_mp_target * 0.005 # retract 5mm
        
        intermediate_states = []
        rgbs = []
        stage_length = {}
        res, path, path_length = motion_planning(
            simulator, real_mp_target_pos, target_orientation, obstacles=obstacles, allow_collision_links=allow_collision_links, save_path=save_path, smooth_path=True)
      
        if res:
            stage_length['grasp_handle'] = len(path)
