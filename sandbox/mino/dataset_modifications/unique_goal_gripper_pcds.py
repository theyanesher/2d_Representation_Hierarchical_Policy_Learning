    ...
    with h5py.File(src_path, 'r') as src, h5py.File(dst_path, 'w') as dst:
        ...
        for demo_key in tqdm.tqdm(src_data, desc="demos"):
            ...
            if all(k in obs_dst for k in required):
                ...
                new_pcd, switch_idxs = ru.compute_new_goal_gripper_pcd(grip_pcd, grip_qpos, actions,return_switch_idxs=True)
                unique_goal_pcds = new_pcd[np.asarray(switch_idxs) - 1]
                ...
                # create new group and add the unique dataset
                demo_group = dst_data[demo_key]
                unique_group = demo_group.create_group('unique_goal_gripper_pcds')
                copy_dataset(old_ds, unique_group, 'goal_gripper_pcd', data=unique_goal_pcds)
...
if __name__ == "__main__":
    ...
    dst_hdf5 = f"data/robomimic/datasets/{task}/{task}_pcd_abs_w_oracle.hdf5"
