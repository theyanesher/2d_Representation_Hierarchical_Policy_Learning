...
# eval_results = "/media/yufei/42b0d2d4-94e0-45f4-9930-4d8222ae63e51/yufei/projects/RoboGen-sim2real/3d_diffusion_policy/3D-Diffusion-Policy/3D-Diffusion-Policy/data/0518-vary-obj-loc-ori-segmask-correct-1/2024.05.18/01.04.26_train_dp3_robogen_open_door/videos/epoch_3000_trainset/opened_joint_angles.json"
eval_results = "/media/yufei/42b0d2d4-94e0-45f4-9930-4d8222ae63e51/yufei/projects/RoboGen-sim2real/3d_diffusion_policy/3D-Diffusion-Policy/3D-Diffusion-Policy/data/0518-vary-obj-loc-ori-segmask-correct-0.8/2024.05.18/01.05.20_train_dp3_robogen_open_door/videos/epoch_3000_valset/opened_joint_angles.json"
with open(eval_results, "r") as f:
    eval_results = json.load(f)

traj_info_data_dir = "data/dp3_demo/action-dist-0511-vary-obj-loc-ori-init-angle-robot-init-joint-near-handle-300-demo-0.4-0.15-translation-first-new/action_dist"
...
for experiment_name, res in eval_results.items():
    ...
    opened_angle = normalized_performance
    info = os.path.join(traj_info_data_dir, f"{experiment}.json")
    if not os.path.exists(info):
        cprint(f"Trajectory info {info} does not exist", "red")
        continue
    
    cprint(f"Processing {experiment_name}", "green")
...
axes[2].boxplot(good_traj_average_translations, positions=[1], widths=0.6, patch_artist=True, showfliers=False)
axes[2].boxplot(bad_traj_average_translations, positions=[2], widths=0.6, patch_artist=True, showfliers=False)

axes[3].boxplot(good_traj_average_orientations, positions=[1], widths=0.6, patch_artist=True, showfliers=False)
axes[3].boxplot(bad_traj_average_orientations, positions=[2], widths=0.6, patch_artist=True, showfliers=False)

axes[4].boxplot(good_traj_max_translations, positions=[1], widths=0.6, patch_artist=True, showfliers=False)
axes[4].boxplot(bad_traj_max_translations, positions=[2], widths=0.6, patch_artist=True, showfliers=False)

axes[5].boxplot(good_traj_max_orientations, positions=[1], widths=0.6, patch_artist=True, showfliers=False)
axes[5].boxplot(bad_traj_max_orientations, positions=[2], widths=0.6, patch_artist=True, showfliers=False)
