...
def get_range_normalizer_from_stat(stat, output_max=1, output_min=-1, range_eps=1e-7):
    ...
    # import pdb(); pdb.set_trace()
    print("inside get_range_normalizer_from_stat", scale, offset)
...
def get_image_range_normalizer():
    ...
    print("inside get_image_range_normalizer", scale, offset)
    # import pdb(); pdb.set_trace()
    return SingleFieldLinearNormalizer.create_manual(
        scale=scale,
        offset=offset,
...
def get_identity_normalizer_from_stat(stat):
    # import pdb(); pdb.set_trace()
    print("STATTTTTTTTTTTTTTT ISNIDE get_identity_normalizer_from_stat", stat)
    scale = np.ones_like(stat['min'])
    offset = np.zeros_like(stat['min'])
    # import pdb(); pdb.set_trace()
    print("inside get_identity_normalizer_from_stat", scale, offset)
    return SingleFieldLinearNormalizer.create_manual(
        scale=scale,
        offset=offset,
        input_stats_dict=stat
