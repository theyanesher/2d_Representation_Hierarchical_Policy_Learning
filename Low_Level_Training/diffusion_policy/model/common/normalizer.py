...
class LinearNormalizer(DictOfTensorMixin):
    ...
    def __setitem__(self, key: str , value): # 'SingleFieldLinearNormalizer'
        ...
        #     self.params_dict[key] = [value[0].params_dict, value[1].params_dict]
        # else:
        self.params_dict[key] = value.params_dict

    def _normalize_impl(self, x, forward=True, seperate_params_pos_ori = False):
        # import pdb; pdb.set_trace();
        if isinstance(x, dict):
            ...
            for key, value in x.items():
                # import pdb; pdb.set_trace();
                if seperate_params_pos_ori and (key == "action" or key == "agent_pos"):
                    # import pdb; pdb.set_trace();
                    pos_params = self.params_dict[key+'_pos']
                    quat_params = self.params_dict[key+'_quat']
                    only_pos = _normalize(value[:,:,:3], pos_params, forward=forward)
                    only_quat = _normalize(value[:,:,3:7], quat_params, forward=forward)
                    result[key] = torch.cat(
                        [only_pos, only_quat, value[:, :, 7:].clone()],  
                        dim=-1
                    ) # last dim is value[:,:,7:]
                else:
                    # import pdb; pdb.set_trace();
    ...
    def normalize(self, x: Union[Dict, torch.Tensor, np.ndarray], seperate_params_pos_ori = False) -> torch.Tensor:
        return self._normalize_impl(x, forward=True, seperate_params_pos_ori =seperate_params_pos_ori)

    def unnormalize(self, x: Union[Dict, torch.Tensor, np.ndarray], seperate_params_pos_ori = False) -> torch.Tensor:
        # import pdb; pdb.set_trace();
        return self._normalize_impl(x, forward=False, seperate_params_pos_ori =seperate_params_pos_ori)
