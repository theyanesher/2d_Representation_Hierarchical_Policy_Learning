...
class HighLevelHeatmapTrain(BaseImageDataset):  # torch.utils.data.Dataset
    def __init__(self, episode_path, sequence_length, pad_before, pad_after, use_wrist_camera, episode_mask = None, num_cameras = 4): # image_path, heatmap_path, low_dim_path, depth_path
        super().__init__()
    ...
    def __getitem__(self,idx):
        ...
        if self.return_single_image:
            ...
            data = {
                'obs': {
                    'image_cam1': torch.cat((gripper_image[0], gripper_heatmap[0], gripper_depth[0]), dim = 1).contiguous(), # T, 3, 96, 96
                    'agent_pos': gripper_proprioception.squeeze(1).contiguous(), # T, 2
                },
                'action': gripper_proprioception.squeeze(1).contiguous() # T, 2 goal_gripper_proprioception
            }
...
episode_path = "/scratch/pbhowal/Diffusion_Policy_Training/Zarr_Trajectories/insert_onto_square_peg/" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data"
dataset = HighLevelHeatmapTrain(episode_path=episode_path, sequence_length = 16, pad_before = 4, pad_after = 4, use_wrist_camera = False)
