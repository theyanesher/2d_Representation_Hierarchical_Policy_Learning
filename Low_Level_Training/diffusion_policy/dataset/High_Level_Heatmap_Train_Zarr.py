...
class HighLevelHeatmapTrain(BaseImageDataset):  # torch.utils.data.Dataset
    def __init__(self, episode_path, image_path, heatmap_path, low_dim_path, depth_path, sequence_length, pad_before, pad_after, use_wrist_camera, episode_mask = None, num_cameras = 4):
        super().__init__()
    ...
    def read_pickle_data(self, proprio_data, indices): #file_name
        # import pdb; pdb.set_trace();
        gripper_pose = torch.from_numpy(proprio_data["pose"][indices]) #.clone().detach().cpu() #torch.tensor(data["gripper_pose"].detach().cpu())
        gripper_open_close = torch.from_numpy(proprio_data["open_close"][indices])  #.clone().detach().cpu() #torch.tensor(data["gripper_open_close"].detach().cpu())
        proprioceprion = torch.cat((gripper_pose, gripper_open_close), dim = -1).squeeze(0)
        gripper_action = torch.from_numpy(proprio_data["action"][indices])    # .clone().detach().cpu().squeeze(0) # torch.tensor(data["gripper_action"].detach().cpu()).squeeze(0)
        return proprioceprion, gripper_open_close # proprioceprion, #step_path # gripper_action

    

    def read_images(self, episode_of_interest, indices): #step_idx image_path, heatmap_path, depth_path, episode_idx, file_name
        # import pdb; pdb.set_trace()
        ...
        for cam_num in range(1, total_cams):
            rgb = episode_of_interest["rgb"][f"camera{cam_num}"][indices]  # (H, W, 3), uint8
            ...
            rgb_step_idx.append(rgb)
        rgb_step_idx = torch.cat(rgb_step_idx, dim=0)
        heatmap_step_idx = []
        for cam_num in range(1, self.num_cameras):
            # import pdb; pdb.set_trace()
            heatmap = episode_of_interest["heatmaps"]["camera"+str(cam_num)][indices]
            heatmap = torch.from_numpy(heatmap).permute(2, 0, 1).float() / 255.0 
            heatmap = F.interpolate(
                heatmap.unsqueeze(0),          # (1, 3, H, W)
                size=(128, 128),
                mode="bilinear",
                align_corners=False
            ).squeeze(0) 
        ...
        for cam_num in range(1, self.num_cameras):
            # import pdb; pdb.set_trace()
            depth = episode_of_interest["depth"]["camera"+str(cam_num)][indices]
            depth = torch.from_numpy(depth).permute(2, 0, 1).float() / 255.0 
            depth = F.interpolate(
                depth.unsqueeze(0),          # (1, 3, H, W)
                size=(128, 128),
                mode="bilinear",
                align_corners=False
            ).squeeze(0)
    ...
    def __getitem__(self,idx):
        ...
        for indices in range(start_idx, end_idx):
            ...
            temp_pro, action_temp_pro = self.read_pickle_data(proprioception_data, indices=indices)
            goal_sample_proprioception.append(action_temp_pro)
            sample_proprioception.append(temp_pro)

            ###################### LOAD THE IMAGE AND HEATMAP DATA #####################
            # import pdb; pdb.set_trace();
            temp_image, temp_heatmap, temp_depth, _ = self.read_images(episode_of_interest, indices)
...
episode_path = "/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data"
dataset = HighLevelHeatmapTrain(episode_path=episode_path, image_path=image_path, heatmap_path = heatmap_path, depth_path=depth_path, low_dim_path = low_dim_path, sequence_length = 16, pad_before = 4, pad_after = 4, use_wrist_camera = False)
# # import pdb; pdb.set_trace()
# print(dataset[718])
