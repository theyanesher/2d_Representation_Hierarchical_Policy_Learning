...
class HighLevelHeatmapTrain(BaseImageDataset):  # torch.utils.data.Dataset
    def __init__(self, base_dir, tasks, sequence_length, pad_before, pad_after, use_wrist_camera, episode_mask = None, num_cameras = 4, return_single_image = False, early_fusion = False, debug = False, use_rvt_dist_heatmap = False): # image_path, heatmap_path, low_dim_path, depth_path
        super().__init__()
        print("Initializing HighLevelHeatmapTrain Dataset... self.use_rvt_dist_heatmap = ", use_rvt_dist_heatmap)
        self.use_rvt_dist_heatmap = use_rvt_dist_heatmap
        ...
        # import pdb; pdb.set_trace()
        if debug:
            self.all_task_dirs = [os.path.join(self.base_dir, f, "trajectories_data_FINAL") for f in tasks] 
        else:
            self.all_task_dirs = [os.path.join(self.base_dir, f) for f in tasks] # , "trajectories_data_FINAL"
        # import pdb; pdb.set_trace();
        self.episode_paths = []
        for episode_path in self.all_task_dirs:
            self.episode_path = episode_path
            self.sequence_length = sequence_length
            # import pdb; pdb.set_trace()
            each_episode_dir_paths = [os.path.join(self.episode_path, f) for f in os.listdir(self.episode_path)]
            each_episode_dir_paths = sorted(each_episode_dir_paths, key=self.extract_number)
            # import pdb; pdb.set_trace()
            self.episode_paths.extend(each_episode_dir_paths) #sorted(self.episode_paths, key=self.extract_number)
        # import pdb; pdb.set_trace()
        self.episode_lengths = []
        for ep_pa in self.episode_paths:
            ep = zarr.open(ep_pa)
            # print("I DONT KNOWWWWW WHYYYYY", ep_pa)
            # import pdb; pdb.set_trace()
            self.episode_lengths.append(min(len(ep["rgb/camera1"]),len(ep["heatmap_dist_rvt/camera1"])))
        # import pdb; pdb.set_trace()
    ...
    def read_language_instructions(self, episode_of_interest, path):
        # start_idx = 0 
        # end_idx = 8
        if "language" not in episode_of_interest or "pooled_text_features" not in episode_of_interest["language"]:
            print("PATHHHHHHHHH DOES NOT EXISTTTTTTTTT", path)
        # import pdb; pdb.set_trace();
        lang_emb = episode_of_interest["language"]["pooled_text_features"] #[start_idx:end_idx]
        return lang_emb


    def read_images(self, episode_of_interest, start_idx, end_idx, path=None): #step_idx image_path, heatmap_path, depth_path, episode_idx, file_name
        # import pdb; pdb.set_trace()
        ...
        for cam_num in range(1, self.num_cameras):
            # import pdb; pdb.set_trace()
            if self.use_rvt_dist_heatmap:
                if "heatmap_dist_rvt" not in episode_of_interest:
                    print("PATHHHHHHHHH DOES NOT EXISTTTTTTTTT", path)
                # print(episode_of_interest)
                heatmap = episode_of_interest["heatmap_dist_rvt"]["camera"+str(cam_num)][start_idx:end_idx]
            else:
                # print("USINGGGGGGGGGG FEATUREEEEEEE MAPPPPPPPPPPSSSSSSSSSSSSSSS")
        ...
        depth_step_idx = []
        for cam_num in range(1, total_cams): # self.num_cameras
            # import pdb; pdb.set_trace()
    ...
    def get_normalizer(self, **kwargs) -> LinearNormalizer:
        ...
        if self.return_single_image:
            normalizer["image"] = get_image_range_normalizer() # T, 3, 96, 96
            ...
            if self.use_wrist_camera:
                normalizer["image_cam4"] = get_image_range_normalizer()
        else:
            normalizer["image_cam1"] = get_image_range_normalizer()
            normalizer["image_cam2"] = get_image_range_normalizer()
            normalizer["image_cam3"] = get_image_range_normalizer()
            normalizer["image_cam5"] = get_image_range_normalizer()
            normalizer["image_cam6"] = get_image_range_normalizer()
            normalizer["image_cam7"] = get_image_range_normalizer()
            if self.use_wrist_camera:
                normalizer["image_cam4"] = get_image_range_normalizer()
        return normalizer
    ...
    def __getitem__(self,idx):
        ...
        # import pdb; pdb.set_trace();
        sample_image, sample_heatmap, sample_depth = self.read_images(episode_of_interest, start_idx=start_idx, end_idx = end_idx, path = episode_of_interest_path)


        ###################### LOAD THE LANGUAGE DATA ##############################
        lang_emb = self.read_language_instructions(episode_of_interest, episode_of_interest_path)
        ...
        if (sample_start_idx > 0) or (sample_end_idx < self.sequence_length):
            ...
            if sample_end_idx < self.sequence_length:
                ...
                ####### CHNAGING IT CAUSE OF CLAUDEEEEEEE ################
                gripper_image[sample_end_idx:] = sample_image[-1]
                gripper_heatmap[sample_end_idx:] = sample_heatmap[-1]
                gripper_depth[sample_end_idx:] = sample_depth[-1]
        ...
        if self.return_single_image:
            data = {
                ...
                'obs_lang_emb' : torch.from_numpy(np.array(lang_emb)).squeeze(0),
                'action': gripper_proprioception.squeeze(1).contiguous() # T, 2 goal_gripper_proprioception
            }
        elif self.early_fusion:
            # import pdb; pdb.set_trace()
            if not self.use_wrist_camera:
                data = {
                    'obs': {
                ...
                data = {
                    'obs': {
                        ...
                        'image_cam2': torch.cat((gripper_image[:,3:6,:,:], gripper_heatmap[:,1:2,:,:], gripper_depth[:,1:2,:,:]), dim = 1).contiguous(),
                        'image_cam3': torch.cat((gripper_image[:,6:9,:,:], gripper_heatmap[:,2:3,:,:], gripper_depth[:,2:3,:,:]), dim = 1).contiguous(),
                        'image_cam4': torch.cat((gripper_image[:,9:12,:,:], gripper_depth[:,3:4,:,:]), dim = 1).contiguous(),
                        'agent_pos': gripper_proprioception.squeeze(1).contiguous(), # T, 2
                    },
                    'obs_lang_emb' : torch.from_numpy(np.array(lang_emb)).squeeze(0),
                    'action': gripper_proprioception.squeeze(1).contiguous() # T, 2 goal_gripper_proprioception
            ...
            if not self.use_wrist_camera:
                data = {
                    'obs': {
                        'image_cam1': torch.cat((gripper_image[:,:3,:,:], gripper_depth[:,:1,:,:]), dim = 1).contiguous(), # T, 3, 96, 96
                        'image_cam2': torch.cat((gripper_image[:,3:6,:,:], gripper_depth[:,1:2,:,:]), dim = 1).contiguous(),
                        'image_cam3': torch.cat((gripper_image[:,6:9,:,:], gripper_depth[:,2:3,:,:]), dim = 1).contiguous(),
                        'image_cam4': gripper_heatmap[:,:1,:,:].contiguous(),
                        'image_cam5': gripper_heatmap[:,1:2,:,:].contiguous(),
                        'image_cam6': gripper_heatmap[:,2:3,:,:].contiguous(),
                    },
                    'obs_lang_emb' : torch.from_numpy(np.array(lang_emb)).squeeze(0),
                    'action': gripper_proprioception.squeeze(1).contiguous() # T, 2 goal_gripper_proprioception
                }
                ...
                data = {
                    'obs': {
                        ...
                        'image_cam3': torch.cat((gripper_image[:,6:9,:,:], gripper_depth[:,2:3,:,:]), dim = 1).contiguous(),
                        'image_cam4': torch.cat((gripper_image[:,9:12,:,:], gripper_depth[:,3:4,:,:]), dim = 1).contiguous(),
                        'image_cam5': gripper_heatmap[:,:1,:,:].contiguous(),
                        'image_cam6': gripper_heatmap[:,1:2,:,:].contiguous(),
                        'image_cam7': gripper_heatmap[:,2:3,:,:].contiguous(),
                        'agent_pos': gripper_proprioception.squeeze(1).contiguous(), # T, 2
                    },
                    'obs_lang_emb' : torch.from_numpy(np.array(lang_emb)).squeeze(0),
                    'action': gripper_proprioception.squeeze(1).contiguous() # T, 2 goal_gripper_proprioception
...
# base_dir = "/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/" #outputs_1st_insert_onto_square_peg/trajectories_data" #"/scratch/pbhowal/Diffusion_Policy_Training/Zarr_Trajectories/insert_onto_square_peg_WITH_LANGUAGE_EMBEDDINGS/" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data_FINAL_WITH_TEXT_EMBEDDING" #"/scratch/pbhowal/Diffusion_Policy_Training/Zarr_Trajectories/insert_onto_square_peg/" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data"
# tasks = ["outputs_1st_insert_onto_square_peg", "outputs_1st_close_jar", "outputs_1st_place_shape_in_shape_sorter", "outputs_1st_reach_and_drag"]
# base_dir = "/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/"
# tasks = ["outputs_1st_close_jar"]
...
# base_dir = "/scratch/pbhowal/data_Diffusion_Policy" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/" #outputs_1st_insert_onto_square_peg/trajectories_data" #"/scratch/pbhowal/Diffusion_Policy_Training/Zarr_Trajectories/insert_onto_square_peg_WITH_LANGUAGE_EMBEDDINGS/" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data_FINAL_WITH_TEXT_EMBEDDING" #"/scratch/pbhowal/Diffusion_Policy_Training/Zarr_Trajectories/insert_onto_square_peg/" #"/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/RVT2_Codebase/RVT_Backup_IDK/RVT/data_diffusion_policy/outputs_1st_insert_onto_square_peg/trajectories_data"
# tasks = ["light_bulb_in", "push_buttons", "close_jar","open_drawer","reach_and_drag","insert_onto_square_peg","place_cups","put_groceries_in_cupboard","slide_block_to_color_target","place_shape_in_shape_sorter","stack_blocks","meat_off_grill","place_wine_at_rack_location","put_money_in_safe","sweep_to_dustpan_of_size"] #  "put_item_in_drawer"
# # # # tasks = ["close_jar"]
# dataset = HighLevelHeatmapTrain(base_dir=base_dir, tasks = tasks, sequence_length = 16, pad_before = 4, pad_after = 4, use_wrist_camera = True, early_fusion = False, debug = False, use_rvt_dist_heatmap = True)
# # import pdb; pdb.set_trace();
