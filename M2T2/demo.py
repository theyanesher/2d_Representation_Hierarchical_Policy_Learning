...
def load_and_predict(data_dir, cfg):
    ...
    for _ in range(cfg.eval.num_runs):
        ...
        data_remove_rgb = copy.deepcopy(data)
        data_remove_rgb['inputs'][:, 3:] = torch.zeros_like(data['inputs'][:, 3:]).float()
        
        data_batch = collate([data])
        to_gpu(data_batch)
        data_remove_rgb_batch = collate([data_remove_rgb])
        to_gpu(data_remove_rgb_batch)

        with torch.no_grad():
            model_ouputs = model.infer(data_batch, cfg.eval)
            model_outputs_no_rgb = model.infer(data_remove_rgb_batch, cfg.eval)
            
        for key in model_ouputs:
            import pdb; pdb.set_trace()
            diff = torch.sum(torch.abs(model_ouputs[key][0] - model_outputs_no_rgb[key][0]))
...
def main(cfg):
    ...
    cam_pose_world = np.zeros((4,4))
    cam_pose_world[:3, :3] = np.eye(3)
    make_frame(vis, 'camera', T=cam_pose_world)
