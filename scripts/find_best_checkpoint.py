...
This script is meant to run on a PSC login/compute node (it needs real
Python + filesystem access to outputs/), not on the psc-data transfer node,
which only allows a small whitelist of commands (ls/rsync/scp).

Usage:
    python3 find_best_checkpoint.py 'outputs/2026.08.16/*_hammercleanup_D1_*'
    python3 find_best_checkpoint.py --metric train_action_mse_error outputs/2026.08.16/*
"""
import argparse
import glob
import json
