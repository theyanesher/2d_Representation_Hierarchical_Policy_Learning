...
for idx in range(247):
    obj =  globals()[f"save_data_name_{idx}"]
    if 'act3d' in obj:
        obj_id = obj.split("-")[3]
    else:
        obj_id = obj.split("-")[2]
...
#     else: 
#         print(f"camera_random_200_save_data_name_{idx}='{non_randomize_objs_dict[obj_id]}'")
#         idx += 1
...
#         print(f"camera_random_200_save_data_name_{idx}='{old_save_data_name}'")
#         idx += 1
#     if idx == 200:
#         break
        
# for obj_dir in randomize_obj_dirs:
#     print(f"camera_random_200_save_data_name_{idx}='{obj_dir}'")
#     idx += 1
...
#         continue # print("************************ not in: ", obj_id)
#     else: 
#         print(f"camera_random_100_save_data_name_{idx}='{non_randomize_objs_dict[obj_id]}'")
#         idx += 1
...
#     obj_id = old_save_data_name.split("-")[-1]
#     if obj_id not in all_randomize_obj_ids:
#         print(f"camera_random_100_save_data_name_{idx}='{old_save_data_name}'")
#         idx += 1
#     if idx == 100:
#         break
        
# for obj_id in all_randomize_obj_ids[:75]:
#     print(f"camera_random_100_save_data_name_{idx}='{randomize_objs_dict[obj_id]}'")
#     idx += 1
...
for obj_id in all_randomize_obj_ids[:37]:
    if obj_id not in non_randomize_objs_dict: 
        ...
        print(f"camera_random_50_save_data_name_{idx}='{non_randomize_objs_dict[obj_id]}'")
        idx += 1
...
for idx2 in range(246):
    ...
    obj_id = old_save_data_name.split("-")[-1]
    if obj_id not in all_randomize_obj_ids:
        print(f"camera_random_50_save_data_name_{idx}='{old_save_data_name}'")
        idx += 1
    if idx == 50:
        break
        
for obj_id in all_randomize_obj_ids[:37]:
    print(f"camera_random_50_save_data_name_{idx}='{randomize_objs_dict[obj_id]}'")
    idx += 1
