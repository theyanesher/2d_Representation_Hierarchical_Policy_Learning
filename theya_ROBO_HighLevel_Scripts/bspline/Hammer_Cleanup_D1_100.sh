...
#SBATCH --job-name hammer-cleanup-d1-high-level-bspline-subgoals-100demo
#SBATCH -o /ocean/projects/cis240052p/eswaramo/code/2d_Representation_Hierarchical_Policy_Learning/theya_ROBO_HighLevel_Scripts/bspline/logs/hammer-cleanup-d1-high-level-bspline-subgoals-100demo_%j.out
#SBATCH -e /ocean/projects/cis240052p/eswaramo/code/2d_Representation_Hierarchical_Policy_Learning/theya_ROBO_HighLevel_Scripts/bspline/logs/hammer-cleanup-d1-high-level-bspline-subgoals-100demo_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# 100-DEMO variant: train articubot (GMM cross-displacement high-level policy)
# on HAMMER_CLEANUP_D1 using bspline subgoals (goal_gripper_pcd_bspline) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_HAMMER_CLEANUP_D1_BSPLINE_SUBGOALS_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
