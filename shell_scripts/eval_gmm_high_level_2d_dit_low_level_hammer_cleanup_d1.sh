...
#
# Hammer_Cleanup_D1 variant. Sister of eval_gmm_high_level_2d_dit_low_level_coffee_d2.sh.
# Same env / same action-conversion path. The LL is the GMM-conditioned variant
# trained with:
#   task=MimicGen_Tasks/hammer_cleanup_gmm_goal
...
# HL: MULTIMODAL Articubot ckpt trained on Hammer_Cleanup_d2.
HL_CKPT="${PROJ_ROOT}/SMITH_High_Level_FineTune/HIGH_LEVEL_POLICIES/Hammer_Cleanup_d2/GHOST_High_Level_MULTIMODAL/periodic-epoch=epoch=74.ckpt"

# LL: the GMM-trained run (use_goal_cross_attention + use_weighted_cross_attention
# + gmm_top_k=1024). Hydra exp dir + checkpoint name.
LL_EXP_DIR="${PROJ_ROOT}/SMITH_High_Level_FineTune/LOW_LEVEL_POLICIES/Hammer_Cleanup_d2/Weighted_Cross_Attn_Cond"
LL_CKPT="epoch_100.ckpt"

# Optional .npy with a (1152,) text embedding for the HL FiLM block.
# Leave empty to use zeros (matches the hammer ckpt's training).
TEXT_EMBED_CACHE=""
...
OUTPUT_DIR="GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D1"
