...
#
# Hammer_Cleanup_D2 variant. Mirrors eval_ghost_high_level_2d_dit_low_level_coffee_d2_100epoch.sh
# but with Hammer_Cleanup_d2 HL/LL checkpoints. The env_meta is sourced from the
# Hammer_Cleanup_D1 HDF5 (no D2 HDF5 is present under ORIGINAL_DATASET/;
# the D1/D2 split here refers to the training-dataset variant, not the env spec).
#
...
DATASET_PATH="${PROJ_ROOT}/SMITH_High_Level_FineTune/ORIGINAL_DATASET/hammer_cleanup_d1.hdf5"
HL_CKPT="${PROJ_ROOT}/SMITH_High_Level_FineTune/HIGH_LEVEL_POLICIES/Hammer_Cleanup_d2/GHOST_High_Level_NO_MULTIMODAL/CORRECT_FOUR_POINT_TRAIN/periodic-epoch=epoch=54.ckpt"
LL_EXP_DIR="${PROJ_ROOT}/SMITH_High_Level_FineTune/LOW_LEVEL_POLICIES/Hammer_Cleanup_d2/DiT_Goal_Gripper_Cond"
LL_CKPT="epoch_90.ckpt"

# Optional .npy with a (1152,) text embedding for the HL FiLM block.
# Leave empty to use zeros (matches the NO_MULTIMODAL ckpt's training).
TEXT_EMBED_CACHE=""
...
# Hammer_Cleanup envs in the vendored mimicgen depend on `robosuite_task_zoo`
# (see external/mimicgen/mimicgen/envs/robosuite/hammer_cleanup.py:26-27).
# That package isn't installed in the pixi env, but it is checked out under
# MimicGen/robosuite-task-zoo/. Adding its parent to PYTHONPATH makes the
# `import robosuite_task_zoo` succeed and lets the env register itself.
# (Not needed for the Coffee_D2 wrapper because Coffee doesn't depend on it.)
ROBOSUITE_TASK_ZOO_ROOT="${PROJ_ROOT}/MimicGen/robosuite-task-zoo"
...
CAMERA_H=256
CAMERA_W=256
HL_IN_CHANNELS=4    # 4 = xyz + 1 mask channel (use_rgb=False, matches the hammer ckpt)
...
OUTPUT_DIR="${SMITH_MIMICGEN}/GHOST_HIGH_LEVEL_PROPER_CANONICAL_EP_54_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D2_19_EPOCH_NO_MULTIMODAL_HIGHLEVEL"
...
for f in "${DATASET_PATH}" "${HL_CKPT}" \
         ...
         "${ROBOSUITE_TASK_ZOO_ROOT}/robosuite_task_zoo/__init__.py" \
         "${ENV_PY}"; do
  if [[ ! -e "${f}" ]]; then
...
# robosuite_task_zoo MUST be on PYTHONPATH so the vendored mimicgen __init__.py's
# `from mimicgen.envs.robosuite.hammer_cleanup import *` succeeds and registers
# HammerCleanup_D0/D1 with robosuite.
export PYTHONPATH="${LFD3D_REPO}/src:${DIT_2D_REPO}:${SMITH_MIMICGEN}:${ROBOSUITE_TASK_ZOO_ROOT}:${PYTHONPATH:-}"
