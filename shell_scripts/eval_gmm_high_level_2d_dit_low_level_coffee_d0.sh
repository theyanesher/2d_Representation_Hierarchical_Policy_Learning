...
#
# Sister of eval_ghost_high_level_2d_dit_low_level.sh. Same env / same HL / same
# action-conversion path. The LL is the GMM-conditioned variant trained with:
#   task=MimicGen_Tasks/coffee_gmm_goal
...
# HL: same Articubot ckpt the existing ghost eval uses (per user instruction).
HL_CKPT="${PROJ_ROOT}/SMITH_High_Level_FineTune/HIGH_LEVEL_POLICIES/Coffee_d0/GHOST_High_Level/rmse_and_std_combi=0.016.ckpt"

# LL: the GMM-trained run (use_goal_cross_attention + use_weighted_cross_attention
# + gmm_top_k=1024). Hydra exp dir + checkpoint name.
LL_EXP_DIR="${PROJ_ROOT}/2D_Hierarchical_Policy_Learning_Github/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/outputs/2026.05.08/18.04.48_groot_GMM_Weighted_Cross_Attention_Coffee_coffee_gmm_goal"
LL_CKPT="epoch_30.ckpt"
...
OUTPUT_DIR="GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_COFFEE_50_SAMPLES_D0"
