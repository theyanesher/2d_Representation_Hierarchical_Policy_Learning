...
# c1 (aux_gmm_loss_weight) = 0.03 ABLATION.
#
...
#
# This checkpoint is a point on the c1 (auxiliary GMM loss weight) sweep:
#     c1 = 0.03   <- THIS RUN
#     c1 = 0.1    ../../../Approach2/...c1_0.1_ALL_SEEDS.sh
#     c1 = 0.3    ../../../Approach2/...c1_0.3_ALL_SEEDS.sh
#     c1 = null   ../../../Approach2_no_GMM_Ablation/  (no aux GMM loss at all)
# It uses the DEFAULT keypoint source (task=coffee_preperation_goal_gmm_aux,
# no aux_goal_key override), so the only thing that varies across the sweep is
# how strongly the auxiliary GMM head shaped the visual representation during
# training. That is a TRAINING-TIME difference only: Approach 2 is never given
# a goal at rollout, so the eval path here is byte-identical to every other
# Approach 2 eval and the numbers are directly comparable.
#
...
#   ${SCRIPT_DIR}/APPROACH2_FROMGMM_2D_DIT_LOW_LEVEL_COFFEE_PREPERATION_50_SAMPLES_D1_DINOV2_c1_0.03_<NTH>_SEED/
...
# LL: Approach 2 run on Coffee_Preperation_D1, default keypoints, c1 = 0.03.
LL_EXP_DIR="${INFERENCE_ROOT}/APPROACH2_POLICIES/Coffee_Preperation_D1/Ablations/c003_GMM_Weight_Ablation"
...
OUTPUT_BASE="APPROACH2_FROMGMM_2D_DIT_LOW_LEVEL_COFFEE_PREPERATION_50_SAMPLES_D1_DINOV2_c1_0.03"
...
# parallel.
export TMPDIR="${TMPDIR:-/tmp}/approach2_fromgmm_c003_eval_$$"
mkdir -p "${TMPDIR}"
