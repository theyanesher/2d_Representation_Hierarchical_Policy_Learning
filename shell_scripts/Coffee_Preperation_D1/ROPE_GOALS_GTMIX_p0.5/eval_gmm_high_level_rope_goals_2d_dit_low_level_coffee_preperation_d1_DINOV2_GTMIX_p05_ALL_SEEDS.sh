...
# ROPE4D-GROUNDED visual encoder WITH GOALS IN THE TRUNK (GTMIX_p0.5).
#
# Sibling of ../ROPE_GTMIX_p0.5/. The two LL runs differ by exactly one
# training override:
#     plain : visual_encoder=dinov2_rope4d_grounded
#     this  : visual_encoder=dinov2_rope4d_grounded_goals
# so the top-K GMM candidates also join the trunk sequence as grounded
# tokens (alpha_trunk 1.0 / beta_trunk 1.0 / gamma_trunk 0.0). The DiT's
# own WCA goal pathway is UNCHANGED -- the trunk route is additive.
#
...
#
# The SAME evaluator serves this variant: the trunk reads its goals from nobs,
# and run_episode already injects gmm_all_goals / gmm_all_weights there. Both
# are identity-normalised in the checkpoint (scale=1 / offset=0), so they
# arrive as raw base-frame coordinates -- which is what the RoPE positions need.
#
# LL run knobs (from its saved .hydra/config.yaml, NOT set here):
#     visual_encoder            dinov2_rope4d_grounded_goals
#     goal_key gmm_all_goals / goal_weights_key gmm_all_weights, goal_top_k 6
#     alpha_trunk 1.0, beta_trunk 1.0, gamma_trunk 0.0
...
#   ${SCRIPT_DIR}/GMM_HIGH_LEVEL_ROPE_GOALS_2D_DIT_LOW_LEVEL_COFFEE_PREPERATION_50_SAMPLES_D1_DINOV2_GTMIX_p05_<NTH>_SEED/
...
# Same checkpoint as the sibling ../ROPE_GTMIX_p0.5/ run.
...
# LL: 100-demo GTMIX_p0.5 WCA + RoPE4D-grounded encoder WITH GOALS in the trunk.
LL_EXP_DIR="/home/theyanesh/2d_Representation_Hierarchical_Policy_Learning/APPROACH1_POLICIES/Low_Level_Policy/Coffee_Preperation_D1/ROPE_GOAL_GTMIX_0.5"
...
OUTPUT_BASE="GMM_HIGH_LEVEL_ROPE_GOALS_2D_DIT_LOW_LEVEL_COFFEE_PREPERATION_50_SAMPLES_D1_DINOV2_GTMIX_p05"
...
export TMPDIR="${TMPDIR:-/tmp}/coffee_rope_goals_eval_$$"
mkdir -p "${TMPDIR}"
