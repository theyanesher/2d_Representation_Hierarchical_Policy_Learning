#!/usr/bin/env bash
# Hierarchical eval on Kitchen_D1: multimodal Articubot HL + 2D DiT LL (GTMIX_p0.5).
#
# _ROBO variant: identical experiment to
#   eval_gmm_high_level_2d_dit_low_level_kitchen_d1_DINOV2_GTMIX_p05_ALL_SEEDS.sh
# but with every path repointed at PSC (the original targets /project_data on a
# machine that is no longer reachable). The original is left untouched.
#
# Single script, runs seeds 100000 / 150000 / 250000 SEQUENTIALLY.
# Per-seed outputs land in:
#   ${SCRIPT_DIR}/GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_KITCHEN_50_SAMPLES_D1_DINOV2_GTMIX_p05_ROBO_<NTH>_SEED/
...
# LL: 100-demo GTMIX_p0.5 WCA run on Kitchen_D1.
LL_EXP_DIR="${LL_REPO}/outputs/2026.07.04/02.38.15_groot_GMM_WCA_100demo_dinov2_Kitchen_D1_GTMIX_p0.5_kitchen_D1_gmm_goal_gt_mix"
...
OUTPUT_BASE="GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_KITCHEN_50_SAMPLES_D1_DINOV2_GTMIX_p05_ROBO"
...
export TMPDIR="${TMPDIR:-/tmp/kitchen_gmm_eval_$$}"
mkdir -p "${TMPDIR}"
