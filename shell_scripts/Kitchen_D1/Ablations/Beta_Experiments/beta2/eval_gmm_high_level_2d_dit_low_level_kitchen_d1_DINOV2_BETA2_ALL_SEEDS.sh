#!/usr/bin/env bash
# Hierarchical eval on Kitchen_D1: multimodal Articubot HL + 2D DiT LL.
# BETA ABLATION, wca_beta = 2.
#
# Same structure and eval knobs as the GTMIX_p0.5 _ROBO script; the ONLY thing
# that differs is which low-level checkpoint is loaded. All WCA settings come
# from that run's saved .hydra/config.yaml, NOT from this script:
#
#     use_goal_cross_attention   true
#     use_weighted_cross_attention true
#     use_alpha                  true      wca_alpha 1.0
#     wca_beta                   2         <-- the ablated knob
#     gmm_top_k                  2500      (the GTMIX run used 6)
#     visual_encoder_type        dinov2
#     LL training task           kitchen_D1_gmm_goal   (NOT ..._gt_mix)
#
# The eval script prints these back at load time as
#   [LL] use_goal_cross_attention=... use_weighted_cross_attention=... gmm_top_k=...
# so check that line reads gmm_top_k=2500 before trusting a run.
#
...
#   ${SCRIPT_DIR}/GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_KITCHEN_50_SAMPLES_D1_DINOV2_BETA2_<NTH>_SEED/
...
# LL: 100-demo WCA run on Kitchen_D1, top-2500 / alpha 1.0 / BETA 2, bs64,
# resumed from epoch 30 of the parent run.
# NOTE: beta2 resumed from E30 while beta3/beta4 resumed from E80 -- the three
# runs did not all continue from the same parent epoch.
LL_EXP_DIR="${LL_REPO}/outputs/2026.08.16/17.35.33_groot_GMM_WCA_100demo_dinov2_Kitchen_D1_top2500_alpha1.0_beta2_bs64_resumeE30_kitchen_D1_gmm_goal"
...
OUTPUT_BASE="GMM_HIGH_LEVEL_2D_DIT_LOW_LEVEL_KITCHEN_50_SAMPLES_D1_DINOV2_BETA2"
...
export TMPDIR="${TMPDIR:-/tmp}/kitchen_beta2_eval_$$"
mkdir -p "${TMPDIR}"
