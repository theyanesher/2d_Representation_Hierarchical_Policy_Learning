...
#
# Kitchen D1 variant. Same script as eval_ghost_high_level_2d_dit_low_level_coffee_d0.sh
# but with the Kitchen HL/LL checkpoints and dataset, dumping results to a D1-tagged
# output directory.
#
...
LL_EXP_DIR="${PROJ_ROOT}/SMITH_High_Level_FineTune/LOW_LEVEL_POLICIES/Kitchen_D1/100_demos/DinoV2_goal_gripper"
LL_CKPT="epoch_90.ckpt"

# Optional .npy with a (1152,) text embedding for the HL FiLM block.
# Leave empty to use zeros (matches the kitchen ckpt's training).
TEXT_EMBED_CACHE=""
...
CAMERA_H=256
CAMERA_W=256
HL_IN_CHANNELS=4    # 4 = xyz + 1 mask channel (use_rgb=False, matches the kitchen ckpt)
...
OUTPUT_DIR="${SCRIPT_DIR}/GHOST_HIGH_LEVEL_2D_DIT_LOW_LEVEL_KITCHEN_50_SAMPLES_D1_DINOV2_goal_gripper_1ST_SEED"

# --------------------------------------------------------------------------- #
# Auto-resume + auto-merge.  (ported from the GMM_WCA sister script)
#
# The Python eval opens results.jsonl with mode "w" (truncates) and numbers
# episodes 1..N internally, so a bare relaunch would OVERWRITE prior results.
# Instead, when episodes already exist we route the resumed batch to a sibling
# temp dir ${OUTPUT_DIR}_RESUME_${COMPLETED}, then fold that back into
# ${OUTPUT_DIR} after Python finishes (renaming videos by seed so episode_NNN
# is continuous, rewriting results.jsonl rows, appending). On startup we also
# fold in any pre-existing ${OUTPUT_DIR}_RESUME_* dirs (from earlier interrupted
# runs) so the next COMPLETED count is correct.
#
