#!/usr/bin/env bash
# Hierarchical eval on Hammer_Cleanup_D1 with RDP-trained HL + 2D DiT LL (goal_gripper).
# Single script, runs seeds 100000 / 150000 / 250000 SEQUENTIALLY.
# Per-seed outputs land in:
#   ${SCRIPT_DIR}/GHOST_HIGH_LEVEL_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_RDP_50_SAMPLES_D1_DINOV2_goal_gripper_<NTH>_SEED/
...
# HL: RDP-trained multimodal Articubot ckpt for Hammer_Cleanup_d2.
HL_CKPT="${PROJ_ROOT}/SMITH_High_Level_FineTune/HIGH_LEVEL_POLICIES/Hammer_Cleanup_d2_RDP/periodic-epoch=epoch=74.ckpt"

# LL: 100-demo goal_gripper run on Hammer_Cleanup_D1 (RDP LL tree).
LL_EXP_DIR="${PROJ_ROOT}/SMITH_High_Level_FineTune/LOW_LEVEL_POLICIES/Hammer_Cleanup_d2_RDP/100_demo/goal_gripper/"
...
# HammerCleanup_D1 is registered via mimicgen.envs.robosuite.hammer_cleanup, which
# imports robosuite_task_zoo — its parent must be on PYTHONPATH (see launch below).
...
OUTPUT_BASE="GHOST_HIGH_LEVEL_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_RDP_50_SAMPLES_D1_DINOV2_goal_gripper"
...
# ROBOSUITE_TASK_ZOO_ROOT on the path so `from mimicgen.envs.robosuite.hammer_cleanup
# import *` succeeds and registers HammerCleanup_D1 (unlike Coffee, which is base robosuite).
