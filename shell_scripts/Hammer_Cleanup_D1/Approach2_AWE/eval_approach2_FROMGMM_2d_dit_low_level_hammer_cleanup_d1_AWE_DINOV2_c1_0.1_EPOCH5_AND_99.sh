#!/usr/bin/env bash
# APPROACH 2 eval on Hammer_Cleanup_D1 -- AWE goal-source variant, FROM-GMM.
#
# Same conventions as
...
# supervised the aux GMM head during training) but points at the AWE-goal-source
# run synced via sync_best_checkpoints.sh, and evaluates BOTH the best-val_loss
# checkpoint (epoch 4, nearest saved epoch 5) and the final checkpoint
# (epoch_99), one seed each. Unlike the sibling UVD run, both checkpoints here
# synced at the healthy full size and verified as valid torch/zip archives --
# no corruption workaround needed.
#
# Per-checkpoint outputs land in:
#   ${SCRIPT_DIR}/APPROACH2_FROMGMM_AWE_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D1_DINOV2_c1_0.1_EPOCH<N>/
...
# LL: Approach 2 AWE-goal-source run on Hammer_Cleanup_D1, c1 = 0.1.
LL_EXP_DIR="${INFERENCE_ROOT}/theya_approach2_policies/06.48.32_hammercleanup_D1_APPROACH2_awe_c1_0.1_100demo_dinov2_DIT_hammercleanup_D1_goal_gmm_aux"
...
OUTPUT_BASE="APPROACH2_FROMGMM_AWE_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D1_DINOV2_c1_0.1"
...
run_one_ckpt() {
  ...
  export TMPDIR="${TMPDIR:-/tmp}/approach2_fromgmm_awe_eval_${tag}_$$"
  mkdir -p "${TMPDIR}"
...
# run_one_ckpt "epoch_5.ckpt" "EPOCH5"
...
echo "  ${SCRIPT_DIR}/${OUTPUT_BASE}_EPOCH5"
