#!/usr/bin/env bash
# APPROACH 2 eval on Hammer_Cleanup_D1 -- FROM-GMM VARIANT,
# NO-GMM ABLATION (aux_gmm_loss_weight = null).
#
...
#
# This checkpoint is the NO-GMM control for the Approach 2 sweep. It was
# trained with
#     policy.aux_gmm_loss_weight=null
#     task=MimicGen_Tasks/hammercleanup_D1_goal_gmm_aux   (default keypoints, no aux_goal_key)
# i.e. the auxiliary GMM head was switched OFF entirely, so NOTHING shaped the
# visual representation toward the goal during training. It is the floor that
# every c1 > 0 run is measured against -- compare with ../../Approach2/  (c1 = 0.1 baseline).
#
# The eval script detects this from the LL's saved Hydra config and prints
#   [LL][WARN] aux_gmm_loss_weight=None: this checkpoint had no auxiliary GMM
# rather than failing, since this is an intentional ablation. Seeing that line
# is CONFIRMATION the right checkpoint loaded, not an error.
#
# The rollout path is byte-identical to every other Approach 2 eval (Approach 2
# is never given a goal at rollout), so the numbers are directly comparable.
#
...
#   ${SCRIPT_DIR}/APPROACH2_NO_GMM_ABLATION_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D1_DINOV2_<NTH>_SEED/
...
# LL: Approach 2 NO-GMM ablation run on Hammer_Cleanup_D1 (c1 = null).
LL_EXP_DIR="${INFERENCE_ROOT}/APPROACH2_POLICIES/Hammer_Cleanup_D1/Ablations/NO_GMM_Loss_Ablations"
...
OUTPUT_BASE="APPROACH2_NO_GMM_ABLATION_2D_DIT_LOW_LEVEL_HAMMER_CLEANUP_50_SAMPLES_D1_DINOV2"
...
# parallel.
export TMPDIR="${TMPDIR:-/tmp}/approach2_no_gmm_hammer_no_gmm_eval_$$"
mkdir -p "${TMPDIR}"
