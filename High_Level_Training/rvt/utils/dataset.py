...
def create_replay(
    ...
    if generate_data_low_level:
        observation_elements.extend([ReplayElement("sample_gripper_pose_for_low_level", (gripper_pose_size,), np.float32), ReplayElement("sample_gripper_open_for_low_level", (1,), np.float32)])
    ...
    replay_buffer = (
        UniformReplayBuffer(  # all tuples in the buffer have equal sample weighting
            ...
            gripper_pose_shape=(7,),
            gripper_open_shape=(1,),
            ...
            generate_data_low_level = generate_data_low_level
        )
    )
...
def _get_action(
    ...
    return (
        ...
        ignore_collisions,
        np.concatenate([obs_tp1.gripper_pose, np.array([grip])]), ###################### ACTION WE CARE ABOUT
...
def _add_keypoints_to_replay(
    ...
    for k in range(
        ...
        # ForkedPdb().set_trace()
        ##### EXTRACTS dict_keys(['left_shoulder_rgb', 'left_shoulder_depth', 'left_shoulder_point_cloud', 'right_shoulder_rgb', 'right_shoulder_depth', 
        # 'right_shoulder_point_cloud', 'wrist_rgb', 'wrist_depth', 'wrist_point_cloud', 'front_rgb', 'front_depth', 'front_point_cloud', 'ignore_collisions', 
        # 'low_dim_state', 'front_camera_extrinsics', 'front_camera_intrinsics', 'left_shoulder_camera_extrinsics', 'left_shoulder_camera_intrinsics', 
        # 'right_shoulder_camera_extrinsics', 'right_shoulder_camera_intrinsics', 'wrist_camera_extrinsics', 'wrist_camera_intrinsics', 'lang_goal_embs'])
        ...
        final_obs = {
            ...
            "sample_gripper_pose_for_low_level": demo[sample_frame].gripper_pose, 
            "sample_gripper_open_for_low_level": np.array([demo[sample_frame].gripper_open]),
            "delta_action_for_low_level": np.concatenate((obs_tp1.gripper_pose - demo[sample_frame].gripper_pose, np.array([obs_tp1.gripper_open - demo[sample_frame].gripper_open]))),
    ...
    obs_dict_tp1.pop("delta_action_for_low_level", None)
    replay.add_final(task, task_replay_storage_folder, **obs_dict_tp1)


def fill_replay(
    replay: ReplayBuffer, # Actually this is UniformReplayBuffer
    task: str,
    task_replay_storage_folder: str,
    start_idx: int,
    num_demos: int,
    ...
    if disk_exist:
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            for i in range(len(demo) - 1):
                ...
                obs = demo[i]
                desc = descs[0]    #### NOTE THAT WE ARE ALWAYS USING THE 0TH TASK DESCRIPTION HERE => FOR EXAMPLE WE USE ONLY 'put the ring on the black spoke' FROM ['put the ring on the black spoke', 'slide the ring onto the black colored spoke', 'place the ring onto the black spoke']
