...
def train(agent, dataset, training_iterations, rank=0, val_dataset = None, val_iterations = None):
    agent.train()
    ...
    for iteration in tqdm.tqdm(
        ...
        if val_dataset is not None and iteration % validation_interval == 0:
            agent.eval()  # set evaluation mode
            val_losses = []

            # VAL_ITERS = 100  # cap validation batches to avoid exhausting dataset
            val_iter = iter(val_dataset)

            with torch.no_grad():
                for val_iteration in tqdm.tqdm(
                    range(val_iterations),
                    disable=(rank != 0),
                    desc=f"Validation @ step {iteration}",
                    leave=False,
                ):
                    try:
                    ...
                    batch_val["tasks"] = val_batch["tasks"]
                    batch_val["lang_goal"] = val_batch["lang_goal"]

                    update_args_val = {
                        "step": iteration,
                        "replay_sample": batch_val,
                        "backprop": False,   # no gradient
                        "reset_log": (val_iteration == 0), #False,
                        "eval_log": True,    # mark as validation
                    }
                    agent.update(**update_args_val)
            ...
            # print(f"Iteration {iteration}: validation_loss={avg_val_loss:.4f}")
...
def experiment(rank, cmd_args, devices, port):
    ...
    TRAIN_REPLAY_STORAGE_DIR = "/project_data/held/pratik/run_sample_basic_experiments/Bimanual_Manipulation/RVT2_GMM_Codebase/Bimanual_Manipulation/replay/replay_train/diffusion_policy_dataset" #"../../Bimanual_Manipulation/replay/replay_train/diffusion_policy_dataset/"#"replay/replay_train"
    TEST_REPLAY_STORAGE_DIR = "../../Bimanual_Manipulation/replay/replay_val/diffusion_policy_dataset/"# "replay/replay_val"
    ...
    # from rvt.mvt.utils import ForkedPdb;
    # ForkedPdb().set_trace()
    TRAINING_ITERATIONS = int(len(train_dataset))
    # ForkedPdb().set_trace()
    ...
    while True:
        ...
        out = train(agent, train_dataset, TRAINING_ITERATIONS,  rank, val_dataset, VAL_ITERATIONS)
