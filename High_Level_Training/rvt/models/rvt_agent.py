...
from utils.rvt_utils import ForkedPdb
...
# from rvt.mvt.robogen_utils import get_4_points_from_gripper_pos_orient
...
class RVTAgent:
    ...
    # def save_batch_images(self, img, episodes, base_dir="outputs_10th/"):
    #     """
    ...
    #     assert num_feats >= 4, "Expected at least 4 feature channels per camera."
    ...
    #             # ForkedPdb().set_trace()
    #             rgb = cam_feat[0:3, :, :]   # adjust slice if needed
    #             depth = cam_feat[3:4, :, :]
    ...
    #             # Prepare directories
    #             dirs = {
    #                 "rgb_norm": os.path.join(folders["rgb_norm"], f"episode{episode_id}", f"camera4"),
    #                 "rgb_raw": os.path.join(folders["rgb_raw"], f"episode{episode_id}", f"camera4"),
    #                 "depth_norm": os.path.join(folders["depth_norm"], f"episode{episode_id}", f"camera4"),
    #                 "depth_raw": os.path.join(folders["depth_raw"], f"episode{episode_id}", f"camera4"),
    ...
    def update(
        ...
        with torch.no_grad():
            ...
            replay_sample["sample_gripper_pose_for_low_level"][:, 0, :3] = wpt_local
            # sample_gripper_pose_for_low_level_4_points = get_4_points_from_gripper_pos_orient(replay_sample["sample_gripper_pose_for_low_level"][:3], replay_sample["sample_gripper_pose_for_low_level"][3:], cur_joint_angle = replay_sample["sample_gripper_open_for_low_level"][:,:,0])
            # ForkedPdb().set_trace()
            bs = len(pc)
        ...
        with autocast(enabled=self.amp):
            ...
            if not eval_log:
                # ForkedPdb().set_trace()
                # self.save_batch_images(img = torch.cat([replay_sample["wrist_rgb"], replay_sample["wrist_depth"]],dim=2), episodes = replay_sample["episode_idx"], base_dir="outputs_10th/")
                out = self._network(
                    pc=pc,
                    img_feat=img_feat,
                    proprio=proprio,
                    ...
                    episode_idx = replay_sample["episode_idx"],
                    indices = replay_sample["sample_frame"], 
                    gripper_pose = replay_sample["sample_gripper_pose_for_low_level"], 
                    gripper_open_close = replay_sample["sample_gripper_open_for_low_level"][:,:,0].unsqueeze(2),
                    gripper_action = replay_sample["action"].unsqueeze(1), 
                    gripper_delta_action = replay_sample["delta_action_for_low_level"]
                )
            else:
                out = self._network(
                    pc=pc,
                    ...
                    indices = replay_sample["sample_frame"],
                    gripper_pose = replay_sample["sample_gripper_pose_for_low_level"], 
                    gripper_open_close = replay_sample["sample_gripper_open_for_low_level"][0],
                    gripper_action = replay_sample["action"].unsqueeze(1), 
                    gripper_delta_action = replay_sample["delta_action_for_low_level"]
                )
        ...
        if backprop:
            with autocast(enabled=self.amp):
                ...
                roll_rot_acc = (rot_q[:, 0 * self._num_rotation_classes : 1 * self._num_rotation_classes,].argmax(-1) == action_rot_x_one_hot.argmax(-1)).float().mean() 
                pitch_rot_acc = (rot_q[:, 1 * self._num_rotation_classes : 2 * self._num_rotation_classes,].argmax(-1) == action_rot_y_one_hot.argmax(-1)).float().mean() 
                yaw_rot_acc = (rot_q[:, 2 * self._num_rotation_classes : 3 * self._num_rotation_classes,].argmax(-1) == action_rot_z_one_hot.argmax(-1)).float().mean() 
                collion_acc = (collision_q.argmax(-1) == action_collision_one_hot.argmax(-1)).float().mean()
                grip_acc = (grip_q.argmax(-1) == action_grip_one_hot.argmax(-1)).float().mean()
            ...
            loss_log = {
                ...
                "roll_rot_acc": roll_rot_acc.item(),
                "pitch_rot_acc": pitch_rot_acc.item(),
                "yaw_rot_acc": yaw_rot_acc.item(),
                "collion_acc": collion_acc.item(),
        ...
        if eval_log:
            with torch.no_grad():
                with autocast(enabled=self.amp):
                    ...
                    pred_wpt, pred_rot_quat, pred_grip, pred_coll, pred_rot = self.get_pred( #  pred_wpt, pred_rot_quat, _, _
                    out,
                    rot_q,
                    grip_q,
                    collision_q,
                    ...
                    # ForkedPdb().set_trace()
                    mse_trans_val = torch.linalg.vector_norm(pred_wpt - wpt, dim=1).mean() #torch.norm(pred_wpt - wpt, dim=1).mean() #torch.mean((pred_wpt - action_trans_con) ** 2)
                    trans_loss_val = self._cross_entropy_loss(q_trans, action_trans).mean()
                    rot_loss_x_val = rot_loss_y_val = rot_loss_z_val = 0.0
                    grip_loss_val = 0.0
                    collision_loss_val = 0.0
                    # ForkedPdb().set_trace()
                    if self.add_rgc_loss:
                        rot_loss_x_val = self._cross_entropy_loss(
                            rot_q[
                                :,
                                0 * self._num_rotation_classes : 1 * self._num_rotation_classes,
                            ],
                        ...
                        rot_loss_y_val = self._cross_entropy_loss(
                            rot_q[
                                :,
                                1 * self._num_rotation_classes : 2 * self._num_rotation_classes,
                            ],
                        ...
                        rot_loss_z_val = self._cross_entropy_loss(
                            rot_q[
                                :,
                                2 * self._num_rotation_classes : 3 * self._num_rotation_classes,
                            ],
                        ...
                        grip_loss_val = self._cross_entropy_loss(
                            grip_q,
                            action_grip_one_hot.argmax(-1),
                        ).mean()

                        collision_loss_val = self._cross_entropy_loss(
                            collision_q, action_collision_one_hot.argmax(-1)
                        ).mean()
                ...
                roll_rot_acc_val = (rot_q[:, 0 * self._num_rotation_classes : 1 * self._num_rotation_classes,].argmax(-1) == action_rot_x_one_hot.argmax(-1)).float().mean() 
                pitch_rot_acc_val = (rot_q[:, 1 * self._num_rotation_classes : 2 * self._num_rotation_classes,].argmax(-1) == action_rot_y_one_hot.argmax(-1)).float().mean() 
                yaw_rot_acc_val = (rot_q[:, 2 * self._num_rotation_classes : 3 * self._num_rotation_classes,].argmax(-1) == action_rot_z_one_hot.argmax(-1)).float().mean() 
                collion_acc_val = (collision_q.argmax(-1) == action_collision_one_hot.argmax(-1)).float().mean()
                grip_acc_val = (grip_q.argmax(-1) == action_grip_one_hot.argmax(-1)).float().mean()
                loss_log_val = {
                    "total_loss_val": total_loss_val.item(),
                    "trans_loss_val": trans_loss_val.item(),
                    "rot_loss_x_val": rot_loss_x_val.item(),
                    "rot_loss_y_val": rot_loss_y_val.item(),
                    "rot_loss_z_val": rot_loss_z_val.item(),
                    "grip_loss_val": grip_loss_val.item(),
                    "collision_loss_val": collision_loss_val.item(),
                    "roll_rot_acc_val": roll_rot_acc_val.item(),
                    "pitch_rot_acc_val": pitch_rot_acc_val.item(),
                    "yaw_rot_acc_val": yaw_rot_acc_val.item(),
                    "collion_acc_val": collion_acc_val.item(),
                    "grip_acc_val": grip_acc_val.item(),
                    "val_tran_mean_mse": mse_trans_val.item()
                }
                # ForkedPdb().set_trace()
                # from pprint import pprint
                pprint.pprint(loss_log_val)
                manage_loss_log(self, loss_log_val, reset_log=reset_log)
                return_out.update(loss_log_val)
            # with torch.no_grad():
            #     wpt = torch.cat([x.unsqueeze(0) for x in wpt])
    ...
    def get_pred(
        ...
        return pred_wpt, pred_rot_quat, pred_grip, pred_coll, pred_rot
