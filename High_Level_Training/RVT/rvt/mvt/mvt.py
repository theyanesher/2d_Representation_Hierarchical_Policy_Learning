...
class MVT(nn.Module):
    def __init__(
        ...
        del args["img_aug_2"]
        del args["no_virtual_image"]

        self.no_virtual_image = no_virtual_image
        if no_virtual_image:
            # When bypassing rendering, the rendered extras (point-cloud
            # coordinates, depth, pixel-location encoding) are never appended
            # to the image tensor.  Force these flags off so that MVTSingle
            # builds input_preprocess / patchify for img_feat_dim channels
            # only (e.g. 3 for RGB) instead of img_feat_dim+3+3+1=10.
            args["add_corr"] = False
            args["add_depth"] = False
            args["add_pixel_loc"] = False
            # feat_ver=1 projects features at the predicted waypoint pixel
            # location, which requires wpt_local during training.  Without
            # rendering there is no waypoint in virtual-image space, so fall
            # back to feat_ver=0 (softmax-weighted heatmap feature pooling).
            args["feat_ver"] = 0
    ...
    def forward(
        ...
        **kwargs,
        ...
        :param real_img: tensor of shape (bs, num_img, img_feat_dim, h, w).
            Used only when no_virtual_image=True to bypass rendering.
        """
        if not self.no_virtual_image:
            self.verify_inp(
                pc=pc,
                img_feat=img_feat,
                proprio=proprio,
        ...
        with torch.no_grad():
            if self.no_virtual_image:
                # Skip point-cloud rendering entirely; use real camera images.
                assert real_img is not None, (
                    "real_img must be provided when no_virtual_image=True"
                )
                assert real_img.shape[1] == self.num_img, (
                    f"real_img has {real_img.shape[1]} camera views but the "
                    f"network was built for {self.num_img}. "
                    f"Make sure the number of stacked camera images matches "
                    f"the renderer's num_img (set via rend_three_views / renderer config)."
                )
                img = real_img
            else:
        ...
        if self.stage_two:
            with torch.no_grad():
                if self.no_virtual_image:
                    # No point-cloud centering or re-rendering for stage two;
                    # reuse the same real images for mvt2.
                    img = real_img
                    wpt_local_stage_one_noisy = wpt_local_stage_one
                    wpt_local2 = wpt_local
                    rev_trans = lambda x: x  # identity: no coordinate transform
                else:
