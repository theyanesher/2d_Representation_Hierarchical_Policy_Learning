...
class RVTAgent:
    ...
    def update(
        ...
        if self.use_articubot_dataset:
            # --- Articubot HDF5 batch ---
            goal_gripper = replay_sample["goal_gripper_pcd"].to(self._device)  # (bs, 10)
            bs_art = goal_gripper.shape[0]

            # Extract 6D rotation (indices 3:9) and convert to quaternion xyzw.
            # The 6D representation stores the first two columns of the rotation
            # matrix (Zhou et al. 2019).  pytorch3d reconstructs the full SO(3)
            # matrix via Gram-Schmidt and then converts to wxyz quaternion.
            rot6d = goal_gripper[:, 3:9]  # (bs, 6)
            rot_mat = torch3d_tf.rotation_6d_to_matrix(rot6d)  # (bs, 3, 3)
            quat_wxyz = torch3d_tf.matrix_to_quaternion(rot_mat)  # (bs, 4) wxyz
            # Convert wxyz -> xyzw to match the rest of the RVT pipeline
            quat_xyzw = torch.cat(
                [quat_wxyz[:, 1:], quat_wxyz[:, :1]], dim=1
            )  # (bs, 4) xyzw
            action_rot = quat_xyzw.cpu().numpy()  # (bs, 4) numpy xyzw
            # Normalise and ensure positive w (same convention as normal path)
            for i, _q in enumerate(action_rot):
                _q = aug_utils.normalize_quaternion(_q)
                if _q[-1] < 0:
                    _q = -_q
                action_rot[i] = _q
            ...
            state = replay_sample["state"].to(self._device)  # (bs, D)
            if state.shape[1] >= proprio_dim:
                proprio = state[:, :proprio_dim]
            else:
                pad = torch.zeros(
                    bs_art, proprio_dim - state.shape[1], device=self._device
                )
                proprio = torch.cat([state, pad], dim=1)
            action_trans_con = None  # not used — heatmaps used for trans loss
        else:
            # --- Normal RLBench batch ---
        ...
        with torch.no_grad():
            if self.no_virtual_image:
                # Skip point-cloud extraction, bounding, and cube-centering.
                # Stack all *_image keys from replay_sample (sorted alphabetically
                # so cam0_image, cam1_image, cam2_image come in consistent order)
                # into real_img of shape (bs, num_img, C, H, W).
                img_keys = sorted(
                    k for k in replay_sample if k.endswith("_image")
                )
                real_img = torch.stack(
                    [replay_sample[k] for k in img_keys], dim=1
                )  # (bs, num_img, C, H, W)
                pc = None
                img_feat = None
                wpt_local = None
            ...
            bs = real_img.shape[0] if self.no_virtual_image else len(pc)
        ...
        with autocast(enabled=self.amp):
            ...
            if self.no_virtual_image and self.use_articubot_dataset:
                ...
                action_trans = action_trans / (
                    action_trans.sum(dim=1, keepdim=True) + 1e-8
                )
            else:
