...
from rvt.utils.articubot_dataset import get_articubot_dataset
...
def experiment(rank, cmd_args, devices, port):
    ...
    if cmd_args.use_articubot_dataset:
        assert cmd_args.articubot_dataset_path != "", (
            "--articubot_dataset_path must be provided when using --use_articubot_dataset"
        )
        train_dataset = get_articubot_dataset(
            data_dir=cmd_args.articubot_dataset_path,
...
if __name__ == "__main__":
    ...
    # Articubot dataset arguments
    parser.add_argument(
        "--use_articubot_dataset",
        action="store_true",
        default=False,
        help="Use the Articubot HDF5 dataset instead of the default RLBench dataset.",
    )
    parser.add_argument(
        "--articubot_dataset_path",
        type=str,
        default="",
        help=(
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Skip the virtual-image rendering pipeline (BoxRenderer, point-cloud "
            "centering, move_pc_in_bound). When set, the network expects a "
            "'real_img' key in the replay_sample dict with shape "
            "(bs, num_img, C, H, W) — e.g. real camera images stacked from "
            "cam0_image / cam1_image / cam2_image."
        ),
    )
