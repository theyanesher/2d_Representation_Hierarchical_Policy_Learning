...
def save_data(pc_list, state_list, feature_map_list, gripper_pcd_list, pcd_mask_list, action_list, goal_gripper_pcd, save_dir):
    ...
    feature_map_arrays = np.zeros((len(state_arrays), 1,1,1,1))
    gripper_pcd_arrays = np.zeros((len(state_arrays), 1,1))
    pcd_mask_list = np.zeros((len(state_arrays), 1))
    ...
    if goal_gripper_pcd is not None:
        ...
        gripper_displacement = ggp - state_arrays[:, 0:3]
        state_arrays = np.concatenate([state_arrays, gripper_displacement], axis=1)
        whole_ggp = goal_gripper_pcd.reshape(-1, 12)
        state_arrays = np.concatenate([state_arrays, whole_ggp], axis=1) # (n, 10 + 3 + 12)
        # also add the point cloud displacement
        pc_displacement = ggp.reshape(-1, 1, 3) - point_cloud_arrays
        point_cloud_arrays = np.concatenate([point_cloud_arrays, pc_displacement], axis=2) # (n, 4500, 6)
...
new_zarr_path = "/scratch/ziyu/dp3_demo/0619-act3d-obj-45448-remove-reaching-collision-resize-2-full-per-step-gripper-mlp-goal-wzy-per-step"
...
# def save_data(pc_list, state_list, feature_map_list, gripper_pcd_list, pcd_mask_list, action_list, save_dir=None, goal_gripper_pcd=None):
...
# new_zarr_path = "/scratch/ziyu/dp3_demo/0619-act3d-obj-45448-remove-reaching-collision-resize-2-full-per-step-gripper-mlp-goal-wzy"
# zarr_path = "data/dp3_demo/0607-act3d-obj-45448-remove-reaching-collision-resize-2-full"
# demo_path = "data/temp/open_the_door_of_the_storagefurniture_by_its_handle_StorageFurniture_45448_2024-03-27-22-40-39/task_open_the_door_of_the_storagefurniture_by_its_handle/experiment/0511-vary-obj-2-loc-ori-init-angle-robot-init-joint-near-handle-300-demo-0.4-0.15-translation-first"
