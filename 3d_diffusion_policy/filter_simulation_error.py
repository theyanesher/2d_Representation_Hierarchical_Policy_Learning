            ...
            for rpy in rpy_list:
                if not parallel:
                    ...
                    if np.any(door_joint_angle_diffs > 0.8 / 100 * 3):
                        ...
                        bad_experiments.append(experiment)
                    # plt.title(experiment)
                    # plt.plot(door_joint_angle_diffs)
                    # plt.show()
...
def extract_demos_from_a_directory(dirtory_path, object_category, exp_name=None, in_gripper_frame=False, parallel=True, 
    ...
    for task_path in task_paths[args.task_beg_idx:args.task_end_idx]:
        ...
        for batch_idx in range(num_batch):
            print("Processing batch idx: {}/{} ".format(batch_idx, num_batch))
            beg_idx = batch_idx * batch_size
            end_idx = min((batch_idx + 1) * batch_size, num_experiment)
            
            bad_experiments = extract_pc_states_for_all_trajectories(
                ...
                save_path=save_path)
            all_bad_experiments.extend(bad_experiments)
            
        cprint("all bad experiments: ", "red")
        print(all_bad_experiments)
...
if __name__ == "__main__":
    ...
    args.add_argument("--parallel", type=int, default=0)
