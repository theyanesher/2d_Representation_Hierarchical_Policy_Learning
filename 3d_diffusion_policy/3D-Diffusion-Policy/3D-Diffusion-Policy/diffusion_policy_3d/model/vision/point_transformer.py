"""
Only implement PointTransformerV1 for now
referenced from
...
class Backbone(nn.Module):
    def __init__(self, npoints, n_c, nblocks=4, nneighbor=16, d_points=3, transformer_dim=512, base_dim=32, downsample_ratio=4):
        """
        npoints: number of points
        n_c: number of classes
        nblocks: number of transformer blocks
        nneighbor: number of neighbors
        d_points: dimension of points
        transformer_dim: dimension of transformer
        """
        super().__init__()
        self.fc1 = nn.Sequential(
            nn.Linear(d_points, base_dim),
            nn.ReLU(),
            nn.Linear(base_dim, base_dim)
        )
        self.transformer1 = TransformerBlock(base_dim, transformer_dim, nneighbor)
        self.transition_downs = nn.ModuleList()
        self.transformers = nn.ModuleList()
        for i in range(nblocks):
            channel = base_dim * 2 ** (i + 1)
            self.transition_downs.append(TransitionDown(npoints // downsample_ratio ** (i + 1), nneighbor, [channel // 2 + 3, channel, channel]))
...
class PointTransformerSeg(nn.Module):
    def __init__(self, npoints, n_c, nblocks=4, nneighbor=16, d_points=3, transformer_dim=512, base_dim=32, downsample_ratio=4, hidden_dim=512):
        """
        npoints: number of points
        n_c: number of classes
        nblocks: number of transformer blocks
        ...
        """
        super().__init__()
        self.backbone = Backbone(npoints=npoints, n_c=n_c, nblocks=nblocks, nneighbor=nneighbor, d_points=d_points, transformer_dim=transformer_dim, base_dim=base_dim)
        self.fc2 = nn.Sequential(
            nn.Linear(base_dim * 2 ** nblocks, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, base_dim * 2 ** nblocks)
        )
        self.transformer2 = TransformerBlock(base_dim * 2 ** nblocks, transformer_dim, nneighbor)
        self.nblocks = nblocks
        self.transition_ups = nn.ModuleList()
        self.transformers = nn.ModuleList()
        for i in reversed(range(nblocks)):
            channel = base_dim * 2 ** i
...
# =======================================================================================================
# ======================= SIMILAR CODE In pointnet2_utils, modified for PointTransformer =================
# =======================================================================================================
...
# =======================================================================================================
# ============================= END OF SIMILAR CODE In pointnet2_utils ================================
# =======================================================================================================

if __name__ == '__main__':
    model = PointTransformerSeg(npoints=4500, n_c=60, nblocks=3, nneighbor=16, d_points=3, transformer_dim=32, base_dim=12, downsample_ratio=8, hidden_dim=128)
    model = model.cuda()
