...
from diffusion_policy_3d.model.vision.position_encodings import RotaryPositionEncoding
...
def multi_head_attention_forward(query,  # type: Tensor
    ...
    if use_separate_proj_weight is not True:
        if qkv_same:
            ...
            if _b is not None:
                _b = _b[_start:_end]
            #print("PRINTING QUERY", query.shape)
            q = F.linear(query, _w, _b)
            #print("PRINTING Q", q.shape)
            if key is None:
    ...
    if rotary_pe is not None:  # rotary pe ROPE disentangeld
        #import pdb; pdb.set_trace()
        #print("INSIDE ROTARY PE",q.shape)
        qp, kvp = rotary_pe
        q_cos, q_sin = qp[..., 0], qp[..., 1]
        k_cos, k_sin = kvp[..., 0], kvp[..., 1]
        #print("INSIDE ROTARY PE",q_cos.shape, q_sin.shape, qp.shape, kvp.shape)
        #print("INSIDE ROTARY PE",q.transpose(0,1).shape)
