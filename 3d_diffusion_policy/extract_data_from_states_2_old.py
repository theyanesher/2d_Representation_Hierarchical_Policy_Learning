    ...
    for task_path in task_paths[args.task_beg_idx:args.task_end_idx]:
        ...
        for batch_idx in range(num_batch):
            ...
            for traj_idx in tqdm.tqdm(range(len(all_traj_pc)), total=len(all_traj_pc)):
                ...
                # [Chialiang] Ignore
                test_experiment_path = os.path.dirname(traj_store_label_path)
                test_data_save_path = os.path.join(save_path, test_experiment_path.split("/")[-1])
                if os.path.exists(test_data_save_path):
                    cprint(f'Ingnore {test_data_save_path} because it exists', 'green')
                    continue
                ...
                if not os.path.exists(path):
                    cprint(f"failure reason: {failure_reason}", 'green')
