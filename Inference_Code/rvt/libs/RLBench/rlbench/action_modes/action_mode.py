...
class MoveArmThenGripper(ActionMode):
    ...
    def action(self, scene: Scene, action: np.ndarray, obs_dict = None):
        # import pdb; pdb.set_trace();
        arm_act_size = np.prod(self.arm_action_mode.action_shape(scene))
        arm_action = np.array(action[:arm_act_size])
        ee_action = np.array(action[arm_act_size:arm_act_size+1])
        ignore_collisions = True #bool(action[arm_act_size+1:arm_act_size+2])
        # import pdb; pdb.set_trace();
        self.arm_action_mode.action(scene, arm_action, ignore_collisions, obs_dict, self.gripper_action_mode)
