...
class RVTAgent:
    ...
    @torch.no_grad()
    def act(
        self, step: int, observation: dict, deterministic=True, pred_distri=False, low_level_obs_dict = None, episodes = None, timestep = None
    ) -> ActResult:
        if self.add_lang:
        ...
        rev_trans = []
        # import pdb; pdb.set_trace();
        wpt = [torch.tensor(low_level_obs_dict["gripper_pose"][-1][:3]).to(pc[0].device)]
        for _pc, _wpt in zip(pc, wpt):
            # import pdb; pdb.set_trace();
            a, b = mvt_utils.place_pc_in_cube(
                _pc,
        ...
        wpt_local = torch.cat(wpt_local, axis=0).detach().cpu().numpy() 
        # import pdb; pdb.set_trace();
        gripper_idx_pose = low_level_obs_dict["gripper_pose"][-1]

        # Replace first 3 values with wpt_local
        gripper_idx_pose[:3] = wpt_local.reshape(-1)[:3]
        data_dict = {}
        data_dict["gripper_idx_pose"] = gripper_idx_pose
        import pdb; pdb.set_trace();
        low_level_obs_dict["lang_goal_tokens"] = observation.get("lang_goal_tokens", None)
        timesteps = 4
        # import pdb; pdb.set_trace();
        if low_level_obs_dict.get("gripper_pose_low_level") is None:
            low_level_obs_dict["gripper_pose_low_level"] = [gripper_idx_pose for _ in range(timesteps)]
        else:
            low_level_obs_dict["gripper_pose_low_level"].append(gripper_idx_pose)
            low_level_obs_dict["gripper_pose_low_level"].pop(0)
        # import pdb; pdb.set_trace();
        if low_level_obs_dict.get("reverse_trans_low_level") is None:
            low_level_obs_dict["reverse_trans_low_level"] = [rev_trans[-1] for _ in range(timesteps)]
        else:
            low_level_obs_dict["reverse_trans_low_level"].append(rev_trans[-1])
            low_level_obs_dict["reverse_trans_low_level"].pop(0)
        # import pdb; pdb.set_trace();
        if low_level_obs_dict.get("gripper_open_low_level") is None:
            low_level_obs_dict["gripper_open_low_level"] = [low_level_obs_dict["gripper_open"][-1] for _ in range(timesteps)]
        else:
            low_level_obs_dict["gripper_open_low_level"].append(low_level_obs_dict["gripper_open"][-1])
            low_level_obs_dict["gripper_open_low_level"].pop(0)
        ...
        for _pc in pc:
            ...
            rev_trans.append(b)
        pc = pc_new
        data_dict["pcd"] = pc[-1].detach().cpu().numpy()
        # import pdb; pdb.set_trace();
        if low_level_obs_dict.get("pcd_low_level") is None:
            low_level_obs_dict["pcd_low_level"] = [pc[-1].detach().cpu().numpy() for _ in range(timesteps)]
        else:
            low_level_obs_dict["pcd_low_level"].append(pc[-1].detach().cpu().numpy())
            low_level_obs_dict["pcd_low_level"].pop(0)
        import pickle
        pickle.dump(data_dict, open("check_low_level_inference_dict.pkl", "wb"))
        ...
        dyn_cam_info = None
        # import pdb; pdb.set_trace();
        self.save_batch_images(img = torch.cat([torch.from_numpy(low_level_obs_dict["wrist_rgb"][-1]), torch.from_numpy(low_level_obs_dict["wrist_depth"][-1])],dim=0), episodes = episodes, base_dir="outputs_11th_TRAIN_DATASET_ALL_EPISODE_TRAIN_PRETRAINED_RVT_ONLY_FIRST_FRAME")
        out = self._network(
            pc=pc,
            img_feat=img_feat,
            proprio=proprio,
            lang_emb=lang_goal_embs,
            img_aug=0, low_level_obs_dict = low_level_obs_dict,
            episodes = episodes , # no img augmentation while acting
            step = timestep
        )
    ...
    def save_batch_images(self, img, episodes, base_dir="outputs_10th/"):
        ...
        for b in range(B):
            # import pdb; pdb.set_trace()
            episode_id = int(episodes.item()) if torch.is_tensor(episodes) else int(episodes)
