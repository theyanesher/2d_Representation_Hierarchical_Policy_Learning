...
@hydra.main(
    ...
    config_name="train_diffusion_transformer_lowdim_highlevel_heatmaps_workspace.yaml"
)
def main(cfg):
    # ---------------------------------------------------------
    ...
    with torch.no_grad():
        for i, batch in enumerate(inference_loader):
            ...
            # if i >= 4:
            #     break
    print("AVERAGE", mse_trans_sum/len(dataset), mse_quat_sum/len(dataset))
