...
class CustomRLBenchEnv(RLBenchEnv):
    ...
    def _get_current_oracle_goal(self):
        kp = self._kp_data[self._kp_ptr]
        return kp["pose"], kp["open"]

    def extract_obs(self, obs: Observation, t=None, prev_action=None):
        obs.joint_velocities = None
        grip_mat = obs.gripper_matrix
        grip_pose = obs.gripper_pose
        grip_state = obs.gripper_open
        ...
        obs_dict['gripper_open'] = np.array([grip_state], dtype=np.float32)

        
        if hasattr(self, "_kp_data") and self._kp_data is not None:
            goal_pose, goal_open = self._get_current_oracle_goal()
            obs_dict["goal_gripper_pose"] = goal_pose.astype(np.float32)
            obs_dict["goal_gripper_open"] = goal_open.astype(np.float32)
    ...
    def reset_to_demo(self, i):
        ...
        print("[SANITY][ENV] reset_to_demo called, episode:", i)
        ...
        kp_indices = demo_loading_utils.keypoint_discovery(d)
        self._kp_ptr = 0

        self._kp_data = []
        for k in kp_indices:
            pose = np.array(d._observations[k].gripper_pose, dtype=np.float32)
            open_ = np.array([d._observations[k].gripper_open], dtype=np.float32)

            # enforce quaternion sign convention
            if pose[6] < 0:
                pose[3:7] *= -1

            self._kp_data.append(
                {
                    "t": k,
                    "pose": pose,
                    "open": open_,
        ...
        print("[SANITY][ENV] kp_data initialized, len =", len(self._kp_data))
