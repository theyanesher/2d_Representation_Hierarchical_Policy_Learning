...
def train_one_epoch(
    ...
    writer, epoch, global_step, plot_fn, cfg, wandb_run
):
    # rank = 0
    # ws = 1
        ...
        if (i + 1) % cfg.train.print_freq == 0:
            ...
            if rank == 0:
                ...
                print(
                    ...
                    f'  loss {loss.item():.5f}  '
                    f"  last layer loss: {losses['layer9/weighted displacement loss'][1].item():.5f}  "
                )
                
                wandb_log_losses = {key: val[1] for (key, val) in losses.items()}
                wandb_log_losses['epoch'] = epoch
                wandb_run.log(wandb_log_losses, step=global_step)
...
def train(cfg):
    ...
    train_sampler, train_loader = get_data_loader(
        cfg, 'train', use_ddp, training=True, articubot=True,
    )
    valid_sampler, valid_loader = get_data_loader(
        # cfg, 'test', use_ddp, training=False
        cfg, 'test', use_ddp, training=True, articubot=True,
    )
    ...
    if rank == 0:
        ...
        wandb_run = wandb.init(
            project="m2t2_articubot",
            name=cfg.train.log_dir.split("/")[-1],
            dir=cfg.train.log_dir,
        )
    else:
        wandb_run = None
    ...
    for epoch in range(init_epoch, cfg.train.num_epochs):
        ...
        global_step = train_one_epoch(
            ...
            epoch + 1, global_step, plot_fn, cfg, wandb_run
        )
...
@hydra.main(config_path='.', config_name='config_articubot', version_base='1.3')
