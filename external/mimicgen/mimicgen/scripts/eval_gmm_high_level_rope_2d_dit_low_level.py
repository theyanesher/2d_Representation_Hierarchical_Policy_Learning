...
and this variant adds the geometry the RoPE4D trunk unprojects:
    cam0_depth cam1_depth
    cam0_intrinsic cam1_intrinsic
    cam0_extrinsic cam1_extrinsic
    present_gripper_pts

Differences from the parent, and ONLY these:
  1. get_shape_meta registers the two *_depth keys (the env populates a depth
     key only if it is registered as the "depth" modality).
  2. create_mimicgen_env sets camera_depths=True.
  3. create_mimicgen_env overrides DepthModality's processor so metric depth is
     NOT clipped to [0, 1] -- see the comment there. This matters more here than
     anywhere else: these runs train with identity_normalize_depth=true, so the
...
latter reads its goals from nobs, which run_episode already injects.

NOTE the encoder entry point differs from Approach 2: this policy calls
visual_encoder.encode(nobs) -- geometry read from NORMALIZED obs -- whereas the
Approach 2 policy calls encode_with_positions(nobs, raw_obs). That is why these
tasks set identity_normalize_depth: true; it is the mechanism that lets metres
survive normalization. Do not port Approach 2's raw-obs plumbing here.
"""
...
def get_shape_meta(camera_h: int, camera_w: int):
    return {
        "obs": {
            ...
            # ROPE variant only: the env populates a *_depth key ONLY if it is
            # registered here as the "depth" modality. The parent script omits these.
...
def create_mimicgen_env(dataset_path: str, shape_meta: dict, camera_h: int, camera_w: int):
    ...
    env_kwargs["camera_depths"]  = True     # ROPE trunk unprojects depth
    ...
    # env returns 0.7435-1.0000.
    #
    # These runs use identity_normalize_depth=true, so the checkpoint's cam0_depth
    # normalizer is scale=1/offset=0 -- the clipped values would go straight into
    # the trunk's unprojection with nothing downstream to catch it.
...
# ROPE variant: same TILING convention as the parent (one env observation
# repeated across n_obs_steps) plus the geometry the RoPE4D trunk needs.
...
def build_ll_obs_dict(obs, n_obs_steps: int, device):
    ...
    Depth is handed over in METRES and reaches the trunk unchanged, because these
    runs train with identity_normalize_depth=true (ckpt cam0_depth normalizer is
    scale=1 / offset=0). Extrinsics are camera-to-ROBOT-BASE -- the env already
    pre-multiplies them by base_world_T_base_robot, matching the training h5.
    """
    import torch
