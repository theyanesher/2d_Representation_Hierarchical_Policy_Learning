...
def dataset_states_to_obs(args):
    ...
    pool_size = getattr(args, 'pool_size', None) or num_workers
    ...
    # resume: skip demos that already have a completed output directory
    if getattr(args, 'output_dir', None) is not None:
        before = len(demos)
        demos = [ep for ep in demos
                 if not os.path.exists(os.path.join(args.output_dir, ep, "0.npz"))]
        print("Resuming: skipping {}/{} already completed demos".format(
            before - len(demos), before))
    ...
    with multiprocessing.Pool(pool_size) as pool:
        for i in tqdm(range(0, len(demos), num_workers), desc="demos", unit="batch"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="number of demos to batch per outer loop iteration",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="number of parallel worker processes (defaults to num_workers). "
             "Set this lower than num_workers to limit simultaneous MuJoCo envs.",
    )
    ...
    parser.add_argument(
        "--use_bayesian_decomp",
        action='store_true',
        help="use BOCPD-based subgoal decomposition (gripper + curvature + jerk + angular velocity)",
    )
    parser.add_argument(
        "--bocpd_config",
        type=str,
        default="third_party/robogen/bocpd_config.yaml",
        help="path to BOCPD hyperparameter config yaml (used only with --use_bayesian_decomp)",
    )
    parser.add_argument(
        "--gripper_only_subgoal",
        action='store_true',
        help="use original gripper open/close only subgoal decomposition (default: gripper + curvature)",
    )
    parser.add_argument(
        "--curvature_threshold",
        type=float,
        default=0.5,
        help="fraction of max curvature to use as peak threshold (default: 0.5)",
    )
    parser.add_argument(
        "--min_segment_len",
        type=int,
        default=10,
        help="minimum timesteps between two curvature-based subgoals (default: 10)",
    )
    parser.add_argument(
        "--warmup_steps",
        type=int,
        default=20,
        help="no subgoals allowed in the first N timesteps (default: 20)",
    )
    ...
    if args.use_bayesian_decomp:
        ...
        print(f"[BOCPD] loaded config from {args.bocpd_config}: {args.bocpd_config_dict}")
    else:
