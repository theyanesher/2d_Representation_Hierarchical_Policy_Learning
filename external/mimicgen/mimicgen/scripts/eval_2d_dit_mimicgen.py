...
def get_shape_meta(camera_h: int, camera_w: int):
    """
    Modalities registered here drive what SMITH's env_robosuite.get_observation
    emits. In particular, only camera keys registered as `rgb` get added to the
    obs dict (and run through postprocess_visual_obs to become (3, H, W) float32
    in [0, 1]). The LL model wants those same images under aliases
    cam0_image / cam1_image — that rename happens inside build_ll_obs_dict.
    """
    return {
        "obs": {
            # Robosuite camera keys — must be registered as rgb so the env emits them.
...
def create_mimicgen_env(dataset_path: str, shape_meta: dict, camera_h: int, camera_w: int):
    """
    Build the robosuite env directly via EnvUtils.create_env so we keep our
    requested camera resolution (SMITH's create_env_from_metadata hard-overrides
    camera_heights/widths to 512, which mismatches the LL's training resolution).

    Camera set mirrors convert_dataset.py — SMITH's env_robosuite accumulates
    point_cloud across every camera in `camera_names`, so we use the same set
    that produced the training data to keep the HL input distribution consistent.
    """
    ...
    env_kwargs.pop("env_name", None)  # passed positionally below; some serialized env_metas duplicate it
    env_kwargs["camera_names"]   = camera_names
    env_kwargs["camera_heights"] = camera_h
    env_kwargs["camera_widths"]  = camera_w

    # Modality registration must run BEFORE env construction — env_robosuite.get_observation
    # only adds an rgb key to obs if it's in OBS_KEYS_TO_MODALITIES as "rgb".
...
def _agentview_to_uint8_rgb(obs):
    """
    SMITH's env_robosuite (with postprocess_visual_obs=True) emits agentview_image
    as (3, H, W) float32 in [0, 1]. PyAV's h264 stream (rgb24) wants (H, W, 3) uint8.
    """
...
def run_episode(
    ...
    # Collect agentview frames each step. Buffered in memory and only emitted on success
    # by the caller. ~256x256x3 bytes/frame * 400 max steps ~ 78 MB worst case per episode.
    ...
    while step < max_steps:
        ...
        with torch.no_grad():
            ...
            subgoal = infer_multitask_high_level_model(
                ...
                high_level_args=hl_args, extra=None,
            )                                                   # (1, 4, 3)
...
def main():
    ...
    parser.add_argument("--save_videos", action=argparse.BooleanOptionalAction, default=True,
        help="If set (default), write an mp4 of every episode to <output_dir>/media/. "
             "Filenames carry the outcome (..._success.mp4 / ..._failure.mp4) so you "
             "can filter by glob. Pass --no-save-videos to disable.")
    parser.add_argument("--video_fps", type=int, default=10,
        help="Frame rate for saved mp4s. SMITH's articubot_pcd_runner uses 10.")
    ...
    hl_model, hl_args = load_multitask_high_level_model(args.high_level_path)
    hl_model.eval().to(device)
    hl_one_hot = bool(hl_args.general.get("add_one_hot_encoding", 0))
    print(f"[HL] add_one_hot_encoding={hl_one_hot} (from training config.json)")
    ...
    # Video setup. SMITH's articubot_pcd_runner uses VideoRecorder.create_h264 (PyAV);
    # we reuse it so encoding is consistent across the two eval paths. Every episode is
    # written to <output_dir>/media/episode_{NNN}_seed_{SSSS}_{success|failure}.mp4 — the
    # outcome is encoded into the filename so the failure cases can be inspected too.
    ...
    if args.save_videos:
        ...
        print(f"[video] all rollouts -> {videos_dir.resolve()} (fps={args.video_fps}, h264 crf=22; "
              "filter by *_success.mp4 / *_failure.mp4)")
