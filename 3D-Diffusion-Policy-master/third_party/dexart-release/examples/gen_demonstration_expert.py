...
from dexart.env.rl_env.base import convert_velocity_to_position_action
...
def parse_args():
    ...
    parser.add_argument('--control_mode', type=str, default="velocity", choices=["velocity", "position"])
...
def main():
    ...
    save_dir = os.path.join('/home/xinyu/3D-Diffusion-Policy', 'dexart_'+args.task_name+'_expert_test.zarr')
    ...
    progress_arrays = [] 
    demo_id_arrays = []
    goal_gripper_pcd_arrays = []    

    # demo_save_dir_success = os.path.join('demo_dp3_15_2', task_name, 'success_demo')
    # demo_save_dir_failure = os.path.join('demo_dp3_15_2', task_name, 'failure_demo')
    ...
    with tqdm(total=num_episodes) as pbar:
        ...
        while num_success < num_episodes:
            ...
            episode_seed = init_seed + demo_id * seed_step
            ...
            progress_index = -1
            total_count_sub = 0
            save_last_obs_flag = False
            j = 0
            while j < env.horizon or save_last_obs_flag:
                ...
                if isinstance(obs, dict):
                    ...
                    obs = obs[np.newaxis, :]
                if not save_last_obs_flag:
                    action = policy.predict(observation=obs, deterministic=True)[0]
                    if control_mode == "position":
                        action = convert_velocity_to_position_action(env, action)
                # print(action.shape)
                ...
                obs_progress = obs['progress'][0]
                if obs_progress > 10e-5 and progress_index == -1:
                    progress_index = j

                # print(j, obs_progress, progress_index)

                # print("point_cloud shape:", obs_point_cloud.shape)
                # print("imagin_robot shape:", obs_imagin_robot.shape)

                '''
                point_cloud shape: (1024, 6)
                imagin_robot shape: (96, 7)
                '''
                ...
                # progress_arrays_sub.append(obs_progress)
                
                if save_last_obs_flag:
                    break
            ...
            if eval_success:
                ...
                progress_arrays.append(progress_index)
                demo_id_arrays.append(demo_id)
                ...
                # progress_arrays.extend(progress_arrays_sub)

                length = len(imagin_robot_arrays_sub)
                goal_gripper_pcd_arrays_sub = [None] * length
                for i in range(length):
                    if i < progress_index:
                        goal_gripper_pcd_arrays_sub[i] = imagin_robot_arrays_sub[progress_index]
                    else:
                        goal_gripper_pcd_arrays_sub[i] = imagin_robot_arrays_sub[-1]
                goal_gripper_pcd_arrays.extend(goal_gripper_pcd_arrays_sub)
    ...
    goal_gripper_pcd_arrays = np.stack(goal_gripper_pcd_arrays, axis=0)
    point_cloud_arrays = np.stack(point_cloud_arrays, axis=0)
    depth_arrays = np.stack(depth_arrays, axis=0)
    action_arrays = np.stack(action_arrays, axis=0)
    episode_ends_arrays = np.array(episode_ends_arrays)
    demo_id_arrays = np.array(demo_id_arrays)
    progress_arrays = np.array(progress_arrays)
    # progress_arrays = np.stack(progress_arrays, axis=0)

    # print(episode_ends_arrays)
    # print(demo_id_arrays)
    # print(progress_arrays)
    ...
    goal_gripper_pcd_chunk_size = (env.horizon, goal_gripper_pcd_arrays.shape[1], goal_gripper_pcd_arrays.shape[2])
    ...
    # progress_chunk_size = (env.horizon, progress_arrays.shape[1])
    ...
    zarr_meta.create_dataset('demo_id', data=demo_id_arrays, dtype='int64', overwrite=True, compressor=compressor)
    zarr_meta.create_dataset('progress', data=progress_arrays, dtype='int64', overwrite=True, compressor=compressor)
    ...
    cprint(f'goal_gripper_pcd shape: {goal_gripper_pcd_arrays.shape}, range: [{np.min(goal_gripper_pcd_arrays)}, {np.max(goal_gripper_pcd_arrays)}]', 'green')
    cprint(f'point_cloud shape: {point_cloud_arrays.shape}, range: [{np.min(point_cloud_arrays)}, {np.max(point_cloud_arrays)}]', 'green')
    cprint(f'depth shape: {depth_arrays.shape}, range: [{np.min(depth_arrays)}, {np.max(depth_arrays)}]', 'green')
    cprint(f'state shape: {state_arrays.shape}, range: [{np.min(state_arrays)}, {np.max(state_arrays)}]', 'green')
    cprint(f'action shape: {action_arrays.shape}, range: [{np.min(action_arrays)}, {np.max(action_arrays)}]', 'green')
    # cprint(f'progress shape: {progress_arrays.shape}, range: [{np.min(progress_arrays)}, {np.max(progress_arrays)}]', 'green')
