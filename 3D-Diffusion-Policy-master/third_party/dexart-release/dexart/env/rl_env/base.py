...
def convert_velocity_to_position_action(env, vel_action):
    """
    Convert velocity-based action to position-based action

    Args:
        env: BaseRLEnv instance with arm robot
        vel_action: velocity action from PPO policy (normalized [-1, 1])

    Returns:
        position_action: target joint positions (normalized [-1, 1])
    """
    current_qpos = env.robot.get_qpos()
    vel_action = np.clip(vel_action, -1, 1)

    # Simulate what arm_sim_step would do to get the target position
    target_root_velocity = recover_action(vel_action[:6], env.velocity_limit[:6])
    ...
    target_arm_qpos = arm_qvel * env.control_time_step + current_qpos[:env.arm_dof]

    # For hand joints: use the same approach as arm_sim_step
    hand_qpos = recover_action(vel_action[6:], env.robot.get_qlimits()[env.arm_dof:])

    # Combine arm and hand targets (matching arm_sim_step exactly)
    target_qpos = np.concatenate([target_arm_qpos, hand_qpos])

    # Clamp to joint limits
    qlimits = env.robot.get_qlimits()
    target_qpos = np.clip(target_qpos, qlimits[:, 0], qlimits[:, 1])

    # Convert back to normalized action space [-1, 1] for position control
    position_action = 2 * (target_qpos - qlimits[:, 0]) / (qlimits[:, 1] - qlimits[:, 0]) - 1

    return np.clip(position_action, -1, 1)


class BaseRLEnv(BaseSimulationEnv, gym.Env):
    def __init__(self, use_gui=True, frame_skip=5, use_visual_obs=False, renderer: str = "sapien", control_mode="velocity", **renderer_kwargs):
    ...
    def setup(self, robot_name):
        ...
        if self.control_mode == "position" and not self.is_robot_free:
            self.rl_step = self.arm_position_sim_step
        elif self.is_robot_free:
    ...
    def arm_position_sim_step(self, action: np.ndarray):
        """
        Position control for arm robot - all joints controlled via position targets.
        Args:
            action: target joint positions (normalized to [-1, 1])
        """
        action = np.clip(action, -1, 1)
        target_qpos = recover_action(action, self.robot.get_qlimits())

        current_qpos = self.robot.get_qpos()

        # Compute velocity targets from position error
        # For arm joints: use computed velocities
        # For hand joints: keep zero
        position_error = target_qpos - current_qpos
        desired_qvel = position_error / self.control_time_step
        arm_qvel = desired_qvel[:self.arm_dof]
        arm_qvel = np.clip(arm_qvel, -np.pi / 1, np.pi / 1)
        target_qvel = np.zeros_like(target_qpos)
        target_qvel[:self.arm_dof] = arm_qvel

        # Use the same drive target mechanism as velocity control
        self.robot.set_drive_target(target_qpos)
        self.robot.set_drive_velocity_target(target_qvel)
        for i in range(self.frame_skip):
            self.robot.set_qf(self.robot.compute_passive_force(external=False, coriolis_and_centrifugal=False))
    ...
    def get_visual_observation(self):
        ...
        camera_obs.update(dict(state=robot_obs, oracle_state=oracle_obs, progress = progress))
    ...
    def get_camera_obs(self):
        ...
        for name, camera_cfg in self.camera_infos.items():
            ...
            # print("modalities:", modalities)    # NOTE: modalities: ['point_cloud', 'rgb', 'depth']
            # ic(modalities)
            texture_names = []
