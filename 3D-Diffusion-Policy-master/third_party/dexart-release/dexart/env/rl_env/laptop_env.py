...
class LaptopRLEnv(LaptopEnv, BaseRLEnv):
    def __init__(self, use_gui=False, frame_skip=5, robot_name="adroit_hand_free", friction=5, index=0, rand_pos=0.0,
                 rand_orn=0.0, control_mode="velocity", **renderer_kwargs):
    ...
    def update_cached_state(self):
        ...
        self.is_contact = True if: 1) At least two fingers are in contact and
                                2) The palm is also in contact.
        This is a stricter definition than loosen_contact above it (which needs ≥1 finger OR palm contact). 
        It's likely used as a criterion for “secure grasp” vs. “loose touch.”
        '''
        ...
        self.early_done = (self.progress > self.target_progress)
        self.is_eval_done = (self.progress > self.target_progress)
    ...
    def get_is_contact(self):
        ...
        [
            self.is_contact,
            np.sum(self.robot_object_contact[:-1]),
            self.robot_object_contact[-1]
        ],
        dtype=np.float32
    )
