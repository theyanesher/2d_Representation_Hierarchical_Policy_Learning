...
#SBATCH -t 12:00:00 # Estimated time, 16hour max. DD-HH:MM.
#SBATCH --job-name hammer-cleanup-d1-high-level-100demo
...
# Train articubot (GMM cross-displacement high-level policy) on HAMMER_CLEANUP_D1.
# Uses the FIRST NUM_DEMOS demos (demo_0 .. demo_(N-1)) from the full
# 909-trajectory dataset on /ocean. Only the requested demo dirs are rsynced
# to node-local scratch ($LOCAL on PSC Bridges-2) before training — the rest
# stay on /ocean.
#
...
# empty demo dir would silently shrink the training set, so fail fast if any
# requested demo is missing or empty.
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_HammerCleanup_D1_GOAL_SWAP_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
