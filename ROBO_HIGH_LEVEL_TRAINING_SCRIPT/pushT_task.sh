...
#SBATCH --job-name push-t-task-high-level
...
# Train articubot (GMM cross-displacement high-level policy) on PushT_Task.
# Uses ALL 206 demos (~1 GB total). PushT differences vs the MimicGen tasks:
#   - No camera intrinsics/extrinsics and all-zero depth  -> dataset has
#     has_camera: False; no depth/rgb is served, pixel metrics and wandb
#     camera-projection visualizations are skipped (pcd metrics unaffected).
#   - Arena-normalized coordinates (~0.25 scene span)     -> fixed_variance
#     scaled down accordingly (sharpest sigma ~0.022 vs 0.1 default).
#   - Goal segments are short (~13-15 frames)             -> transition_radius=1
#     (transition frame +/-1) so the weighted sampler / label swap stay
#     focused instead of covering ~75% of all frames.
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_PushT_Task_GOAL_SWAP/$(date +%Y-%m-%d/%H-%M-%S)" \
