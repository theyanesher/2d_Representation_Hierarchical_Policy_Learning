...
#SBATCH --job-name mug-cleanup-d1-high-level-fixed-interval-100demo
...
# on Mug_Cleanup_D1 using fixed-interval goals (goal_gripper_pcd_fixed_interval) from the
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Mug_Cleanup_D1_${NUM_DEMOS}demo"
DEST_EXTRA_DIR="${SCRATCH_ROOT}/Mug_Cleanup_D1_extra_goals_${NUM_DEMOS}demo"
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Mug_Cleanup_D1_GOAL_SWAP_FIXED_INTERVAL_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
