...
#SBATCH --job-name kitchen-d1-high-level-fixed-interval-100demo
...
# on KITCHEN_D1 using fixed-interval goals (goal_gripper_pcd_fixed_interval) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_KITCHEN_D1_GOAL_SWAP_FIXED_INTERVAL_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
