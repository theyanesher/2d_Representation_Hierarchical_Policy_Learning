...
#SBATCH --job-name kitchen-d1-high-level-rdp-gripper-100demo
...
# on KITCHEN_D1 using RDP-gripper goals (goal_gripper_pcd_rdp_gripper) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_KITCHEN_D1_GOAL_SWAP_RDP_GRIPPER_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
