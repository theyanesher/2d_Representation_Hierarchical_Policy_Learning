...
#SBATCH --job-name kitchen-d1-high-level-random-100demo
...
# on KITCHEN_D1 using random-keypoint goals (goal_gripper_pcd_random) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_KITCHEN_D1_GOAL_SWAP_RANDOM_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
