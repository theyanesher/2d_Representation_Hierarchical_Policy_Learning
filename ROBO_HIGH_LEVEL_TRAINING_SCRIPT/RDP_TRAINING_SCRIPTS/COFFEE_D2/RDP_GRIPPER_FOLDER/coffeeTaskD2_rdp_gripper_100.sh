...
#SBATCH --job-name coffee-d2-rdp-gripper-high-level-100demo
...
# on Coffee_D2 using RDP-gripper goals (goal_gripper_pcd_rdp_gripper) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Coffee_D2_GOAL_SWAP_RDP_GRIPPER_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
