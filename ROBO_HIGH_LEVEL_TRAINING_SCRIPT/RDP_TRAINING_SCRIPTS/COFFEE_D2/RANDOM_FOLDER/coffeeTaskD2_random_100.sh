...
#SBATCH --job-name coffee-d2-random-high-level-100demo
...
# on Coffee_D2 using random-keypoint goals (goal_gripper_pcd_random) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Coffee_D2_GOAL_SWAP_RANDOM_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
