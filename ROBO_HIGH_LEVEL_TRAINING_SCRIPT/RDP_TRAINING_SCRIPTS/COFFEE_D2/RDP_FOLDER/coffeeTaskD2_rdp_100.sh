...
#SBATCH --job-name coffee-d2-rdp-high-level-100demo
...
# on Coffee_D2 using RDP goals (goal_gripper_pcd_rdp) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Coffee_D2_GOAL_SWAP_RDP_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
