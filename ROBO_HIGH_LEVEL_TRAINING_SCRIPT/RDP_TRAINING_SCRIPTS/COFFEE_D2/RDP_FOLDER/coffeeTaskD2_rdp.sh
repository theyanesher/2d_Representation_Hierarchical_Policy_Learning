...
#SBATCH --job-name coffee-d2-rdp-high-level
#SBATCH -o /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/2d_Representation_Hierarchical_Policy_Learning/ROBO_HIGH_LEVEL_TRAINING_SCRIPT/RDP_TRAINING_SCRIPTS/COFFEE_D2/RDP_FOLDER/logs/job_%j.out
#SBATCH -e /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/2d_Representation_Hierarchical_Policy_Learning/ROBO_HIGH_LEVEL_TRAINING_SCRIPT/RDP_TRAINING_SCRIPTS/COFFEE_D2/RDP_FOLDER/logs/job_%j.err
...
# Stages both the dataset and the extra-goal files onto the compute node's
# local scratch before training. Sampler/swap caches are written to the
# /ocean EXTRA_KEYPOINTS source dir so the full-dataset goal scan runs once.
...
EXTRA_SRC_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/Coffee_D2"
...
# checkpoint of the 2026-07-04 run (killed by the /ocean disk-full incident
# during epoch 9; epoch 4 was the last periodic save). Set RESUME_CKPT="" to
# train from scratch:
#   RESUME_CKPT="" sbatch this_script.sh
RESUME_CKPT="${RESUME_CKPT:-${REPO_DIR}/logs/train_Coffee_D2_GOAL_SWAP_RDP/2026-07-04/02-47-47/checkpoints/periodic-epoch=epoch=4.ckpt}"
...
DEST_EXTRA_DIR="${SCRATCH_ROOT}/Coffee_D2_extra_goals"
...
# Stage the extra-goal files (small; ~16KB per frame).
find "${EXTRA_SRC_DIR}" -mindepth 1 -maxdepth 1 -printf '%f\n' \
    | xargs -P "${THREADS}" -I {} \
...
if [ -n "${RESUME_CKPT}" ]; then
    ...
    # '=' (periodic-epoch=epoch=4.ckpt), which otherwise breaks the override
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Coffee_D2_GOAL_SWAP_RDP/$(date +%Y-%m-%d/%H-%M-%S)" \
    +dataset.goal_source="${GOAL_SOURCE}" \
    +dataset.extra_goals_dir="${DEST_EXTRA_DIR}" \
    +dataset.transition_cache_dir="${EXTRA_SRC_DIR}" \
