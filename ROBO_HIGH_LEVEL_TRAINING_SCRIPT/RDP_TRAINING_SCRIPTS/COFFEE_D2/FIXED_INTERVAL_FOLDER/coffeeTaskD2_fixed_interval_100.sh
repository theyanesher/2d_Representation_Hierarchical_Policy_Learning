...
#SBATCH --job-name coffee-d2-fixed-interval-high-level-100demo
...
# on Coffee_D2 using fixed-interval goals (goal_gripper_pcd_fixed_interval) from the
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Coffee_D2_${NUM_DEMOS}demo"
DEST_EXTRA_DIR="${SCRATCH_ROOT}/Coffee_D2_extra_goals_${NUM_DEMOS}demo"
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_Coffee_D2_GOAL_SWAP_FIXED_INTERVAL_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
