...
#SBATCH --job-name coffee-prep-d1-high-level-rdp-gripper-100demo
#SBATCH -o /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/2d_Representation_Hierarchical_Policy_Learning/ROBO_HIGH_LEVEL_TRAINING_SCRIPT/RDP_TRAINING_SCRIPTS/COFFEE_PREPERATION_D1/RDP_GRIPPER_FOLDER/logs/job_%j.out
#SBATCH -e /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/2d_Representation_Hierarchical_Policy_Learning/ROBO_HIGH_LEVEL_TRAINING_SCRIPT/RDP_TRAINING_SCRIPTS/COFFEE_PREPERATION_D1/RDP_GRIPPER_FOLDER/logs/job_%j.err
#SBATCH --mail-type=END
#SBATCH --mail-user=pbhowal@andrew.cmu.edu

# 100-DEMO variant: train articubot (GMM cross-displacement high-level policy)
# on COFFEE_PREPERATION_D1 using RDP-gripper goals (goal_gripper_pcd_rdp_gripper) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_COFFEE_PREPERATION_D1_GOAL_SWAP_RDP_GRIPPER_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
