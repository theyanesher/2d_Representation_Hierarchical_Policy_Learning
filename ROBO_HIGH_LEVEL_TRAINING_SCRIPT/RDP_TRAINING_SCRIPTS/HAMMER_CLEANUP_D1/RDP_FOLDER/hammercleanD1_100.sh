...
# on HammerCleanupD1 using RDP goals (goal_gripper_pcd_rdp) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_HammerCleanup_D1_GOAL_SWAP_RDP_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
