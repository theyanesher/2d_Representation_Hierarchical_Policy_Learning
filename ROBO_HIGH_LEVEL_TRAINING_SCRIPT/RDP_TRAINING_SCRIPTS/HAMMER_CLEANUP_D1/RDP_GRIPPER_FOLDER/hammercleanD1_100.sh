...
#SBATCH --job-name hammer-cleanup-d1-high-level-rdp-gripper-100demo
...
# on HammerCleanupD1 using RDP-gripper goals (goal_gripper_pcd_rdp_gripper) from the
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_HammerCleanup_D1_GOAL_SWAP_RDP_GRIPPER_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
