...
#SBATCH --job-name coffee-preperation-d1-high-level-100demo
...
# 1000-trajectory dataset on /ocean. Only the requested demo dirs are rsynced
...
# Sanity check: every staged demo dir must actually contain .npz frames. The
# COFFEE_PREPERATION_D1 dataset is fully populated, but an empty demo dir would
# silently shrink the training set, so fail fast if any requested demo is empty.
...
pixi run python scripts/train.py \
    ...
    "hydra.run.dir=logs/train_COFFEE_PREPERATION_D1_GOAL_SWAP_${NUM_DEMOS}demo/$(date +%Y-%m-%d/%H-%M-%S)" \
