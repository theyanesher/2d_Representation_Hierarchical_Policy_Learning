...
def correct_obs_format(obs): 
    ...
    setattr(obs, f'left_shoulder_mask', None)
    setattr(obs, f'right_shoulder_mask', None)
    setattr(obs, f'front_mask', None)
    setattr(obs, f'wrist_mask', None)
...
def _add_keypoints_to_replay(
    ...
    for k in range(
        ...
        obs_tp1 = correct_obs_format(demo[keypoint])
        obs_tm1 = correct_obs_format(demo[max(0, keypoint - 1)])
        (
            trans_indicies,
            rot_grip_indicies,
            ignore_collisions,
...
def fill_replay(
    ...
    if disk_exist:
        ...
        for d_idx in range(start_idx, start_idx + num_demos):
            ...
            detection_threshold_path = os.path.join(data_path, f'episode{d_idx}', 'detection_threshold.npy')
            custom_threshold = np.load(detection_threshold_path)
            print(f'Detection threshold used: {custom_threshold}')
            ...
            # extract keypoints
            episode_keypoints = keypoint_discovery(demo, stopping_delta=custom_threshold)
            print(f'Keypoints: {episode_keypoints}')
