        ...
        for k, v in vals.items():
            if "image" in k:
                ...
                if isinstance(v, (int, float)):
                    ...
                    print(f"[Warning] Skipping non-numeric score for {k}: {v}")
...
def get_eval_parser():
    ...
    # High-level selection. Default high-level is GHOST (lfd3d ArticubotNetwork).
    # Pass --use_rvt_baseline to use the original RVT-2 high-level instead.
    parser.add_argument("--use_rvt_baseline", action="store_true")

    parser.add_argument("--rvt_model_path", type=str, default=None)
    parser.add_argument("--rvt_exp_cfg_path", type=str, default=None)
    # parser.add_argument("--mvt_cfg_path", type=str, default=None)

    # GHOST high-level args (used unless --use_rvt_baseline).
    parser.add_argument("--ghost_model_path", type=str, default=None)
    parser.add_argument(
        "--lfd3d_repo",
        type=str,
        default=(
    ...
    parser.add_argument("--ghost_in_channels", type=int, default=4)
    parser.add_argument("--ghost_num_points", type=int, default=4500)
    parser.add_argument("--ghost_argmax_weight", type=int, default=1)

    parser.add_argument("--dp3_model_path", type=str, default=None)
    parser.add_argument("--dp3_exp_cfg_path", type=str, default=None)

    # Receding-horizon rollout: DP3 predicts a full horizon; execute this many
    # actions before re-querying the high-level (predict 16 / execute 8).
