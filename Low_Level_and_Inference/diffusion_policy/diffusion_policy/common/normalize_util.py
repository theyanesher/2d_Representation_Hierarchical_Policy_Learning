from diffusion_policy.model.common.normalizer import SingleFieldLinearNormalizer, WelfordOnlineActionNormalizer, RobotStateNormalizer
...
def get_robot_state_normalizer_from_stat(stat, output_max=1, output_min=-1, range_eps=1e-7):
    input_max = stat['max']
    input_min = stat['min']
    input_range = input_max - input_min
    ignore_dim = input_range < range_eps
    ...
    return RobotStateNormalizer.create_manual(
        scale=scale,
        offset=offset,
        input_stats_dict=stat
    )

def get_welford_online_action_normalizer(horizon=16, action_dim=10, max_samples=None) -> WelfordOnlineActionNormalizer:
    return WelfordOnlineActionNormalizer.create_manual(
        horizon=horizon,
        action_dim=action_dim,
        max_samples=max_samples
    )

def get_online_range_robot_state_normalizer(state_dim: int = 10, max_samples: int = None,
                                            output_max: float = 1.0, output_min: float = -1.0,
                                            range_eps: float = 1e-7) -> OnlineRangeRobotStateNormalizer:
    return OnlineRangeRobotStateNormalizer.create_manual(
        state_dim=state_dim,
        max_samples=max_samples,
...
def get_welford_robot_state_normalizer(state_dim: int = 10, max_samples: int = None, sigma_normalization: float = 3.0) -> WelfordOnlineRobotStateNormalizer:
    return WelfordOnlineRobotStateNormalizer.create_manual(
        state_dim=state_dim,
        max_samples=max_samples,
        sigma_normalization=sigma_normalization,
    )

def get_welford_image_normalizer(num_channels: int, max_samples: int = None, sigma_normalization: float = 3.0) -> WelfordImageNormalizer:
    return WelfordImageNormalizer.create_manual(
        num_channels=num_channels,
        max_samples=max_samples,
        sigma_normalization=sigma_normalization
    )

def get_identity_normalizer() -> SingleFieldLinearNormalizer:
    """
    Returns a SingleFieldLinearNormalizer that performs no transformation.
    Useful for geometric data like Plucker coordinates.
    """
    # Define neutral parameters
    scale = np.array([1.0], dtype=np.float32)
    offset = np.array([0.0], dtype=np.float32)
    
    # Standard metadata required by the LinearNormalizer structure
    input_stats_dict = {
...
def get_depth_scalar_normalizer(data_array):
    ...
    scale = np.array([(output_max - output_min) / input_range], dtype=np.float32)
    offset = np.array(output_min - scale * global_min, dtype=np.float32)

    stat = {
        'min': np.array([global_min], dtype=np.float32),
        'max': np.array([global_max], dtype=np.float32),
