...
class TrainDiffusionUnetHybridWorkspace(BaseWorkspace):
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if not is_last_batch and os.environ["LOCAL_RANK"] == "0":
                ...
                if (self.epoch % cfg.training.val_every) == 0:
                    with torch.no_grad():
                        ...
                        if len(val_losses) > 0:
                            ...
                            step_log['val_loss'] = val_loss_tensor.item()
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    ...
                    if os.environ['LOCAL_RANK'] == '0':
                        ...
                        if cfg.checkpoint.save_last_snapshot:
                            self.save_snapshot()
                    torch.distributed.barrier() # make sure checkpoint is saved before moving on
    ...
    def save_checkpoint(self, path=None, tag='latest', 
            ...
            use_thread=False):
        """
        DDP-compatible save_checkpoint. 
        Only Rank 0 performs the actual I/O to avoid race conditions.
        """
        # 1. Rank Guard: Prevent multiple GPUs from writing to the same file
        ...
        for key, value in self.__dict__.items():
            if hasattr(value, 'state_dict') and hasattr(value, 'load_state_dict'):
                ...
                if key not in exclude_keys:
                    ...
                    # 3. Offload to CPU before serialization to keep GPU memory stable
                    if use_thread:
                        # Use your existing helper if defined, otherwise dict comprehension
                        payload['state_dicts'][key] = {k: v.cpu() for k, v in state_dict.items()}
                    else:
        ...
        # 4. Serialization
        if use_thread:
            # Note: Ensure self._saving_thread is handled to prevent overlapping saves
