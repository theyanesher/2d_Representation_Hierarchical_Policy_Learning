...
class CropRandomizer(nn.Module):
    ...
    def forward_with_offsets(self, inputs):
        """Like forward(), but also returns crop top-left offsets as a (B, 2) long tensor [offset_h, offset_w]."""
        assert self.num_crops == 1, "forward_with_offsets only supports num_crops=1"
        if self.training:
            ...
            # crop_inds: (B, 1, 2) -> (B, 2)
            offsets = crop_inds.squeeze(1)
            return tu.join_dimensions(out, 0, 1), offsets
        else:
            H, W = inputs.shape[-2:]
            offset_h = (H - self.crop_height) // 2
            offset_w = (W - self.crop_width) // 2
            out = ttf.center_crop(img=inputs, output_size=(self.crop_height, self.crop_width))
            offsets = torch.zeros(inputs.shape[0], 2, dtype=torch.long, device=inputs.device)
            offsets[:, 0] = offset_h
            offsets[:, 1] = offset_w
            return out, offsets
