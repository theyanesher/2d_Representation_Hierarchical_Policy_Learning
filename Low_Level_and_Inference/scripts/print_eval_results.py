...
def plot_open_door_results(directory, num_objs=10, start_idx=0):
    ...
    for j in range(start_idx, num_objs):
        ...
        if not os.path.exists(json_file): 
            print(f"File {json_file} does not exist, skipping.")
            continue
        with open(json_file) as f:
            ...
            expert_angles = []
            initial_angles = []
            for trial, key in enumerate(data):
                if trial >= args.max_trials:
                    break
                if data[key]["expert_door_joint_angle"] <= data[key]['initial_joint_angle']:
                    continue
                # sim error where door closes into the furniture (not possible in real world)
                opened_joint_angles.append(max(data[key]["final_door_joint_angle"],0.0))
