...
#SBATCH -t 30:00:00 # 30-hour budget
#SBATCH --job-name coffee-prep-d1-approach2-awe-dp
#SBATCH -o /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-dp_job_%j.out
#SBATCH -e /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-dp_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on COFFEE_PREPERATION_D1  —  goal_source=awe (DP subgoals), c1=0.1
# ===========================================================================
...
# (goal_gripper_pcd_awe) but sourced from the standalone
# EXTRA_KEYPOINTS_awe tree instead of the combined
# EXTRA_KEYPOINTS_bspline_..._mix_... tree used by the sibling
# coffee_preperationD1_approach2_awe.sh. This tree holds AWE subgoals
# derived from the DP (diffusion-policy) rollout rather than the original
# heuristic used for the combined tree's 'awe' column — the two are NOT
# numerically identical even though they share the same npz key name.
# c1=0.1 is the established "start here" auxiliary-loss weight for
# Approach 2 (see the sibling *_c1_0.1.sh scripts in
# ROBO_LOW_LEVEL_TRAINING_SCRIPT/100_trajectory_trainings/Approach2/ for how
# it was chosen).
#
# Pipeline:
...
#      --inject_extra_goals) from the EXTRA_KEYPOINTS_awe npz tree into the
#      staged, node-local copy. Only appends a small (T,4,3) array, so it's
#      cheap to redo every job even though the staged copy is ephemeral.
#   3. Train task=MimicGen_Tasks/coffee_preperation_goal_gmm_aux with
#      +task.dataset.goal_source=awe and policy.aux_gmm_loss_weight=0.1.
#
# CAUTION — as of 2026-08-20 the EXTRA_KEYPOINTS_awe tree is still being
# generated and does NOT yet have a COFFEE_PREPERATION_D1 subdir at all
# (only HAMMER_CLEANUP_D1 exists so far, and even that is a partial,
# non-contiguous set of demo_N/ dirs). The injection step stages
# demo_0.h5 .. demo_(NUM_DEMOS-1).h5 sequentially and will raise
# FileNotFoundError the first time it hits a missing
# EXTRA_KEYPOINTS_awe/COFFEE_PREPERATION_D1/demo_N dir. Do not submit this
# script until that subdir exists with contiguous demo_0..demo_(NUM_DEMOS-1)
# coverage — check with:
#   ls /jet/home/eswaramo/data/D2/EXTRA_KEYPOINTS/EXTRA_KEYPOINTS_awe/COFFEE_PREPERATION_D1 | wc -l
#
...
# combined-tree *_awe.sh script — it is NOT passed to hydra (goal_source
# stays "awe" because that's the npz/h5 key name in both trees).
SOURCE_TAG="awe_dp"
C1=0.1
...
EXTRA_GOALS_DIR="/jet/home/eswaramo/data/D2/EXTRA_KEYPOINTS/EXTRA_KEYPOINTS_awe/COFFEE_PREPERATION_D1"
...
LOCAL_NO_GMM_H5_DIR="/jet/home/eswaramo/data/D2/NO_GMM_preds/COFFEE_PREPERATION_D1"
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Coffee_Preperation_D1_Approach2_${SOURCE_TAG}_${NUM_DEMOS}demo"
...
echo "[inject] ensuring obs/goal_gripper_pts_awe exists in staged demos (from EXTRA_KEYPOINTS_awe)"
(
    cd "${REPO_DIR}"
    USE_TF=0 \
    GIT_LFS_SKIP_SMUDGE=1 \
...
RUN_NAME="coffee_preperation_D1_APPROACH2_${SOURCE_TAG}_c1_${C1}_${NUM_DEMOS}demo_dinov2_DIT"
