...
#SBATCH --job-name coffee-prep-d1-approach2-awe-dp-th0.05
#SBATCH -o /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-dp-th0.05_job_%j.out
#SBATCH -e /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-dp-th0.05_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on COFFEE_PREPERATION_D1  —  goal_source=awe (DP subgoals, err_th=0.05), c1=0.1
# ===========================================================================
...
# EXTRA_KEYPOINTS_awe-dp-th0.05 tree instead of the combined
...
# derived from the DP (diffusion-policy) rollout at distance-error
# threshold 0.05 (tighter threshold -> more, closer-spaced keypoints) —
# NOT numerically identical to the sibling *_awe.sh or default-threshold
# *_awe_dp.sh trees even though they share the same npz key name. c1=0.1
# is the established "start here" auxiliary-loss weight for Approach 2
# (see the sibling *_c1_0.1.sh scripts in
...
#      --inject_extra_goals) from the EXTRA_KEYPOINTS_awe-dp-th0.05 npz tree
#      into the staged, node-local copy. Only appends a small (T,4,3) array,
#      so it's cheap to redo every job even though the staged copy is
#      ephemeral.
...
#
# EXTRA_KEYPOINTS_awe-dp-th0.05/COFFEE_PREPERATION_D1 has been verified to
# cover demo_0..demo_99 (100/100, contiguous) as of 2026-08-21.
#
...
# combined-tree/DP-tree *_awe.sh / *_awe_dp.sh scripts — it is NOT passed to
# hydra (goal_source stays "awe" because that's the npz/h5 key name in every
# tree).
SOURCE_TAG="awe_dp_th0.05"
C1=0.1
...
EXTRA_GOALS_DIR="/jet/home/eswaramo/data/D2/EXTRA_KEYPOINTS/EXTRA_KEYPOINTS_awe-dp-th0.05/COFFEE_PREPERATION_D1"
...
echo "[inject] ensuring obs/goal_gripper_pts_awe exists in staged demos (from EXTRA_KEYPOINTS_awe-dp-th0.05)"
(
    cd "${REPO_DIR}"
    USE_TF=0 \
    GIT_LFS_SKIP_SMUDGE=1 \
