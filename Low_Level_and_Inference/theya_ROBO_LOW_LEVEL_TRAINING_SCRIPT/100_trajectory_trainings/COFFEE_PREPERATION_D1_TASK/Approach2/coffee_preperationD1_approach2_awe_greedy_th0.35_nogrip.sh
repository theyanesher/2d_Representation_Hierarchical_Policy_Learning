...
#SBATCH --job-name coffee-prep-d1-approach2-awe-greedy-th0.35_nogrip
#SBATCH -o /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-greedy-th0.35_nogrip_job_%j.out
#SBATCH -e /jet/home/eswaramo/code/Low_Level_and_Inference/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/theya_ROBO_LOW_LEVEL_TRAINING_SCRIPT/logs/coffee-prep-d1-approach2-awe-greedy-th0.35_nogrip_job_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=teswaram@andrew.cmu.edu

# ===========================================================================
# APPROACH 2 (GMM-as-auxiliary-loss) on COFFEE_PREPERATION_D1  —  goal_source=awe (greedy, err_th=0.2), c1=0.1
# ===========================================================================
...
# EXTRA_KEYPOINTS_awe-greedy-th0.6 tree instead of the combined
# EXTRA_KEYPOINTS_bspline_..._mix_... tree used by the sibling
# coffee_preperationD1_approach2_awe.sh. This tree holds AWE subgoals produced by
# the greedy keypoint-selection method at error threshold 0.2 (tighter
# threshold -> more, closer-spaced keypoints) — NOT numerically identical to
# the sibling *_awe.sh or *_awe_dp.sh trees even though they share the same
# npz key name. c1=0.1 is the established "start here" auxiliary-loss weight
# for Approach 2 (see the sibling *_c1_0.1.sh scripts in
...
#      --inject_extra_goals) from the EXTRA_KEYPOINTS_awe-greedy-th0.6 npz
#      tree into the staged, node-local copy. Only appends a small (T,4,3)
#      array, so it's cheap to redo every job even though the staged copy is
#      ephemeral.
...
#
# EXTRA_KEYPOINTS_awe-greedy-th0.6/COFFEE_PREPERATION_D1 has been verified to
...
EXTRA_GOALS_DIR="/jet/home/eswaramo/data/D2/EXTRA_KEYPOINTS/EXTRA_KEYPOINTS_awe-greedy-th0.35_nogrip/COFFEE_PREPERATION_D1"
...
echo "[inject] ensuring obs/goal_gripper_pts_awe exists in staged demos (from EXTRA_KEYPOINTS_awe-greedy-th0.6)"
(
    cd "${REPO_DIR}"
    USE_TF=0 \
    GIT_LFS_SKIP_SMUDGE=1 \
