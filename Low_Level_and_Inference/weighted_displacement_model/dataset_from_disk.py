...
class PointNetDatasetFromDisk(torch.utils.data.Dataset):
    def __init__(self, all_obj_paths, beg_ratio=0, end_ratio=0.9, use_all_data=False):
        ...
        for obj_path in all_obj_paths:
            ...
            for s in ['action_dist', 'demo_rgbs', 'all_demo_path.txt', 'meta_info.json', 'example_pointcloud', 'camera_params.npz']:
            ...
            # self.all_trajectory_path += [list(Path(obj_path, s).glob('*.pkl')) for s in all_subfolder]
        ...
        for idx, traj_path in enumerate(tqdm(self.all_trajectory_path)):
            ...
            all_substeps = [p.name for p in Path(traj_path).glob("*.pkl")]
...
def get_dataset_from_pickle(all_obj_paths=None, beg_ratio=0, end_ratio=0.9, use_all_data=False, dataset_prefix=None, num_train_objects=200):
    ...
    if all_obj_paths is None:
        ...
        elif num_train_objects == 'only_41510':
            from pathlib import Path
            # data_dir = Path(f'{dataset_prefix}/test_gen_demo/41510/')
            # all_obj_paths = [dir_name for dir_name in data_dir.iterdir() if dir_name.is_dir()]
            # print(all_obj_paths[0])
            all_obj_paths = [f'{dataset_prefix}/test_gen_demo/41510']
        else:
