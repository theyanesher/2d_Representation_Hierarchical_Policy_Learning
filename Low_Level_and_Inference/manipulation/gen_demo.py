...
from manipulation.utils import build_up_env, save_numpy_as_mp4, parse_center
    ...
    if os.path.exists(path):
        ...
        for exp in all_experiments:
            ...
            if os.path.exists(state_path):
                if len(os.listdir(state_path)) > 1 and os.path.exists(os.path.join(path, exp, "all.mp4")):
                    num += 1
    return num
...
def create_config_variant(config_path, try_times, fix_object=False):
    ...
    for config_dict in new_config:
        if 'size' in config_dict:
            if not fix_object:
                ...
                config_dict['size'] = 0.7 * config_dict['size']
                config_dict['is_crop_size'] = True
...
def _gen_init_state(q, config_path, env_name, render, far_distance=0.7, near_distance=0.3, fix_object=False, fix_robot=False):
    ...
    while not good_init_pos:
        ...
        if fix_object:
            new_orient = p.getQuaternionFromEuler([init_euler[0], init_euler[1], base_euler[2] + np.random.uniform(-np.pi / 36, np.pi / 36)])
        else:
        ...
        for test_time in range(100):
            if fix_robot:
                initial_joint_angles = [0, -np.pi/4, 0, -3*np.pi/4, 0, np.pi/2, np.pi/4]
                initial_joint_angles = np.array(initial_joint_angles) + np.random.uniform(-0.1, 0.1, size=7) # small perturbations
            else:
...
# handle memory leak             
def gen_init_state(config_path, env_name, render, far_distance=0.7, near_distance=0.3, fix_object=False, fix_robot=False):
    q = mp.Queue()  
    p = mp.Process(target=_gen_init_state, args=(q, config_path, env_name, render, far_distance, near_distance, fix_object, fix_robot))
    p.start()
    p.join()
    success = q.get()
    return success
...
def _execute(q, config_path, env_name, solution_path, experiment_path, time_string=None):
    ...
    if len(states) > 10:
        with open(os.path.join(experiment_path, "last_state_files.txt"), 'w') as f:
            f.write("\n".join(str(states[-1])))
        save_numpy_as_mp4(np.array(rgbs), "{}/{}.mp4".format(experiment_path, "all"))
        q.put(True)
        return True
    else:
        q.put(False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fix_robot", action='store_true')
    ...
    for i in range(generate_times):
        ...
        while True:
            for config_path, solution_path, obj_id in zip(all_config_paths, all_solution_paths, all_obj_ids):
                ...
                new_config_path = create_config_variant(config_path, try_times, fix_object=args.fix_object)

                # generate initial state
                success = gen_init_state(new_config_path, env_name, args.render, near_distance=near_distance,
                                         far_distance=far_distance, fix_object=args.fix_object, fix_robot=fix_robot)
