...
class SimpleEnv(gym.Env):
    ...
    def render(self, return_depth=False, mode=None):
        ...
        if mode is None:
            ...
            view_matrices, projection_matrices = mode
            images = []
            for view_mat, proj_mat in zip(view_matrices, projection_matrices):
                w, h, img, depth, segmask = p.getCameraImage(256, 256,  # self.camera_width, self.camera_height, 
                    view_mat, proj_mat, 
                    renderer=p.ER_BULLET_HARDWARE_OPENGL, 
                    physicsClientId=self.id)
                img = np.reshape(img, (h, w, 4))[:, :, :3]
