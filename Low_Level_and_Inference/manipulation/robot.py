...
class Robot(Agent):
    ...
    def init(self, body, id, np_random, ik_limit=True):
        super(Robot, self).init(body, id, np_random, ik_limit=ik_limit)
    ...
    def pose_to_mat(pose):
        pos, orn = pose
        mat = np.eye(4)
        mat[:3, :3] = np.array(p.getMatrixFromQuaternion(orn)).reshape(3, 3)
        mat[:3, 3] = pos
        return mat

    def get_eef_to_world(self):
        eef_pos, eef_orn = self.get_pos_orient(self.right_end_effector)
        return self.pose_to_mat((eef_pos, eef_orn))

    def get_robot_to_world(self):
        pos, orn = self.get_base_pos_orient()
        return self.pose_to_mat((pos, orn))
