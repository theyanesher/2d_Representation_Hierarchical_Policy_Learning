...
def prepare_env(experiment_folder, experiment_path, all_experiments):
    ...
    for experiment in all_experiments:
        ...
        first_step_folder = os.path.join(experiment_folder, experiment)
        if not os.path.isdir(first_step_folder):
            print("Skipping {} because the first step folder does not exist".format(experiment))
            continue
        if os.path.exists(os.path.join(first_step_folder, "label.json")):
            ...
            if not label['good_traj']: 
                print("Skipping {} because the expert trajectory is not good".format(experiment))
                continue
        ...
        config_file = os.path.join(experiment_folder, experiment_path, experiment, "task_config.yaml")
...
def run_eval_non_parallel(cfg, low_level_policy, high_level_policy, 
    ...
    for dataset_idx,experiment_folder in enumerate(cfg.task.env_runner.experiment_folder):
        ...
        config_files, init_state_files, expert_opened_angles = prepare_env(experiment_folder, '', all_experiments)
        ...
        for exp_idx, (config_file, init_state_file) in enumerate(zip(config_files, init_state_files)):
            ...
            save_numpy_as_mp4(np.array(all_rgbs), gif_save_path.replace(".gif", ".mp4"))
