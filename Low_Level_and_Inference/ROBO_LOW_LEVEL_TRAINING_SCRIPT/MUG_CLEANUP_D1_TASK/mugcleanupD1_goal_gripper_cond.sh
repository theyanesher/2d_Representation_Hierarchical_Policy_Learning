...
#SBATCH --job-name mug-cleanup-d1-goal-gripper
...
# the single ground-truth goal_gripper_pts (4 keypoints) rather than the GMM
# distribution. The trainer's LazyArticuBotDataset reads demo_*.h5 shards, but
# the source on /ocean is the GROOT-style demo_N/ npz tree, so we first
# consolidate it into h5 (idempotent — skips already-converted demos), then
# stage the h5 files onto node-local /local scratch before training.
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=mug_cleanup_D1_goal_gripper_DIT \
    name=mug_cleanup_D1_goal_gripper_DIT \
    dataloader.batch_size=164 \
    dataloader.num_workers=32 \
    training.checkpoint_every=10 \
    training.resume=true \
    +training.resume_ckpt_path=/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/Low_Level_Policy/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/outputs/2026.05.18/23.23.38_mug_cleanup_D1_goal_gripper_DIT_mug_cleanup_goal_gripper/checkpoints/epoch_60.ckpt
