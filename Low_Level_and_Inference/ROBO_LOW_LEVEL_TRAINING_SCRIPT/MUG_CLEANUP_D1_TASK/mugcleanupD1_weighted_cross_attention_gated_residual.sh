...
#SBATCH --job-name mug-cleanup-d1-weighted-cross-attention-gated-residual
...
# cross-attention AND ReZero-style asymmetric gated goal residual. Each cross-attn
# block multiplies its WCA output by a learnable scalar α_goal (init=0, one per
# block) before the residual add. Visual CA / self-attn / FF residuals stay
# ungated, so at step 0 the model is "visual-CA + SA + FF only" — vision is
# forced to carry the action signal from the very first gradient step, and the
# optimizer only grows α_goal if WCA actually reduces loss beyond what vision
# alone gives.
#
...
#   policy.use_gated_goal_residual=true     — wrap WCA output in learnable α_goal,
#                                             init=0, so vision carries the model
#                                             at step 0.
USE_TF=0 \
...
pixi run python diffusion_policy/train.py \
    ...
    policy.use_gated_goal_residual=true \
    policy.gmm_top_k=1024 \
    logging.project=MimicGen_GMM_Low_Level_Policy \
    logging.name=groot_GMM_Weighted_Cross_Attention_Gated_Residual_MugCleanup_D1 \
    name=groot_GMM_Weighted_Cross_Attention_Gated_Residual_MugCleanup_D1 \
