...
#SBATCH --job-name coffee-d2-goal-gripper-rdp-100demo-dinov2
...
# 100-demo goal_gripper baseline on Coffee_D2, conditioned on RDP goals
# (obs/goal_gripper_pts_rdp) instead of the default goal_gripper_pts.
#
# Same pipeline as the default coffee_d2_goal_gripper.sh, plus one idempotent
# injection step: after the npz->h5 consolidation, backfill the four extra
# goal keys (rdp / rdp_gripper / random / fixed_interval) into the NO_GMM h5
# files from the EXTRA_KEYPOINTS npz tree. The first RDP-family job pays this
# once (~seconds); later jobs of any variant find the keys and skip. Training
# then selects the key via +task.dataset.goal_source.
#
...
# --- paths ---------------------------------------------------------------
SRC_NPZ_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/Coffee_D2"
EXTRA_GOALS_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/Coffee_D2"
NO_GMM_H5_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/LOW_LEVEL_GROOT_TRAINING_DATASET/NO_GMM_DATASET/Coffee_D2"
...
# Resumes full training state (model + EMA + optimizer + epoch counter) via
# training.resume=true + training.resume_ckpt_path. Defaults to epoch_60 of
# the 2026-07-04 run: every checkpoint after epoch 60 (incl. latest.ckpt) was
# truncated by the /ocean disk-full incident; epoch_60 verified loadable.
# Set RESUME_CKPT="" to train from scratch:
#   RESUME_CKPT="" sbatch this_script.sh
RESUME_CKPT="${RESUME_CKPT:-/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/Low_Level_Policy/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/outputs/2026.07.04/03.58.42_coffee_D2_goal_gripper_rdp_100demo_dinov2_DIT_coffee_D2_goal_gripper/checkpoints/epoch_60.ckpt}"
...
pixi run python diffusion_policy/train.py \
    ...
    task=MimicGen_Tasks/coffee_D2_goal_gripper \
    task.dataset.data_dir="${DEST_DATA_DIR}" \
    +task.dataset.goal_source=${GOAL_SOURCE} \
    visual_encoder=dinov2 \
    logging.project=mimicgen_tasks \
    logging.name=coffee_D2_goal_gripper_${GOAL_SOURCE}_${NUM_DEMOS}demo_dinov2_DIT \
    name=coffee_D2_goal_gripper_${GOAL_SOURCE}_${NUM_DEMOS}demo_dinov2_DIT \
