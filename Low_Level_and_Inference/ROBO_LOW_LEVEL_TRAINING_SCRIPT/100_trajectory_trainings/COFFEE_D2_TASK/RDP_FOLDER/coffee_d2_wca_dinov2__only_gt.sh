...
#SBATCH --job-name coffee-d2-wca-100demo-dinov2-rdp-onlygt
...
# RDP ONLY-GT run on Coffee_D2 (gt_mix_p=1.0 -> every TRAIN sample uses the
# ground-truth RDP goal set, never the high-level prediction):
...
#
# RDP twist vs ../coffee_d2_wca_dinov2__only_gt.sh: the GT goals come from the
# EXTRA_KEYPOINTS npz tree (goal_gripper_pcd_rdp) so transition windows follow
# the RDP keypoint schedule, and VALIDATION (which always uses the predicted
# GMM) is served from the RDP prediction npz tree. The h5 files are read-only;
# their default gmm/goal keys are never loaded.
#
...
GT_SRC_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/Coffee_D2"
...
DEST_GT_DIR="${SCRATCH_ROOT}/Coffee_D2_rdp_goals_${NUM_DEMOS}demo"
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_D2_RDP_ONLY_GT \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_D2_RDP_ONLY_GT \
