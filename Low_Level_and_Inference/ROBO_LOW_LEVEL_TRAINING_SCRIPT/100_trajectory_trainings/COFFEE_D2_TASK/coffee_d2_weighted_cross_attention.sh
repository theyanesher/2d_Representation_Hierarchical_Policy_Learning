...
#SBATCH --job-name coffee-d2-wca-100demo-dinov2
...
# 100-demo version of the Coffee_D2 GMM weighted-cross-attention baseline.
#
# Train flow-matching DiT low-level policy on Coffee_D2 with GMM weighted
# cross-attention. Uses the FIRST NUM_DEMOS demos (demo_0.h5 .. demo_(N-1).h5)
# from the full 1000-trajectory GMM dataset on /ocean. Only the requested demos
# are rsynced to node-local scratch — the rest stay on /ocean.
#
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_D2 \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_D2 \
