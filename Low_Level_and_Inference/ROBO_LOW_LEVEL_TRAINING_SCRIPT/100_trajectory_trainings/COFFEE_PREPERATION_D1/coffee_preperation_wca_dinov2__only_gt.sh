...
#SBATCH --job-name coffee-prep-d1-wca-100demo-dinov2-onlygt
...
# 100-demo ONLY-GT run on COFFEE_PREPERATION_D1 (no high-level policy / no predicted GMM).
#
...
# It reuses the `coffee_preperation_gmm_goal_gt_mix` task (the GT goals are written
# into the same 4500-slot GMM container, all other slots weight 0), with
# gmm_top_k=6 and the original COFFEE_PREPERATION_D1 dataset.
#
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_Preperation_D1_ONLY_GT \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_Preperation_D1_ONLY_GT \
