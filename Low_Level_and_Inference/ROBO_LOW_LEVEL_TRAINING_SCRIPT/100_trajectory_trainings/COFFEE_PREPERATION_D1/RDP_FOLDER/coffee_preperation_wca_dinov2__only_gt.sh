...
#SBATCH --job-name coffee-prep-d1-wca-100demo-dinov2-rdp-onlygt
...
# RDP ONLY-GT run on COFFEE_PREPERATION_D1 (gt_mix_p=1.0 -> every TRAIN sample
# uses the ground-truth RDP goal set, never the high-level prediction):
...
#
# RDP twist vs ../coffee_preperation_wca_dinov2__only_gt.sh: the GT goals come
# from the EXTRA_KEYPOINTS npz tree (goal_gripper_pcd_rdp) so transition windows
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_Preperation_D1_RDP_ONLY_GT \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_Coffee_Preperation_D1_RDP_ONLY_GT \
