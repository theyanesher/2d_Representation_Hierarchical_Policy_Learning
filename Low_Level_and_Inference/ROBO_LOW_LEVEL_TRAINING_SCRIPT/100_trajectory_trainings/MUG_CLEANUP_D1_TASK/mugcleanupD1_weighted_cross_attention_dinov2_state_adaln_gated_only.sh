...
#SBATCH --job-name mug-d1-wca-100demo-dinov2-stateadaln-gated
...
# Ablation: state-AdaLN with ReZero-style per-block gate (no per-component state).
#
# Same architecture as mugcleanupD1_weighted_cross_attention_dinov2_state_adaln_only.sh
# (flow-matching DiT, serial WCA → visual-CA, DINOv2 backbone, gmm_top_k=32,
# state encoded by the original SimpleMLP into 2 tokens), but switches StateAdaLN
# from "zero-init linear" to the ReZero parameterization:
#
#   policy.use_gate_on_state_adaln=true
#     Each block's StateAdaLN allocates its OWN learnable scalar α_state init=0,
#     and linear_state uses default random init. The state branch's contribution
#     is α_state · linear_state(SiLU(state_ctx)). At init α=0 so the branch
#     contributes nothing; the optimizer grows α_state during training only if
#     state modulation reduces loss.
#
# 12 independent per-block gates (one α_state per block in the 12-layer DiT)
# allow implicit per-block sparsity — blocks that don't want state modulation
# can train α → 0 while blocks that benefit can grow theirs.
#
# Logged each epoch to WandB:
#   - alpha_state/block_{0..11}  — per-block gate values
#   - alpha_state/mean           — mean across blocks
#   - alpha_state/abs_mean       — mean of |α_state| across blocks
#
# Routed to a SEPARATE WandB project so the gate dynamics can be analyzed
# without mixing into the main policy comparison dashboard.
#
...
pixi run python diffusion_policy/train.py \
    ...
    policy.use_per_component_state=false \
    policy.use_state_adaln=true \
    policy.use_gate_on_state_adaln=true \
    policy.gmm_top_k=32 \
    logging.project=MimicGen_StateAdaLN_Gate_Analysis \
    logging.name=groot_GMM_WCA_StateAdaLN_Gated_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
    name=groot_GMM_WCA_StateAdaLN_Gated_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
