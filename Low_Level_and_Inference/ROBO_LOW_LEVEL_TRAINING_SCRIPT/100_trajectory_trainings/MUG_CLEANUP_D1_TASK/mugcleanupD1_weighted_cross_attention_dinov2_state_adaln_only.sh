...
#SBATCH --job-name mug-d1-wca-100demo-dinov2-stateadaln-only
...
# Ablation: state-AdaLN ONLY (no per-component state encoding).
#
# Same architecture as mugcleanupD1_weighted_cross_attention_dinov2.sh (flow-matching
# DiT, serial WCA → visual-CA, DINOv2 backbone, gmm_top_k=32), plus:
#
#   policy.use_state_adaln=true
#     Each block's `norm1` becomes a StateAdaLN that adds per-channel state-derived
#     (γ_state, β_state) modulation on top of temb. The state branch is zero-init
#     so behavior matches standard AdaLN at step 0; the optimizer grows state
#     modulation only if it reduces loss. Gives state a global modulation pathway
#     across every block, independent of attention.
#
# Per-component state encoding is OFF: state is still encoded by the original
# SimpleMLP(10→D→D), producing 2 state tokens per batch (not 6). This isolates
# the effect of the StateAdaLN modulation pathway from the per-component-token
# attention-bandwidth change. Compare against the 3-tokens-only run to disentangle
# which contributes more to handling state-dependent failure cases.
#
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_StateAdaLN_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
    name=groot_GMM_WCA_StateAdaLN_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
