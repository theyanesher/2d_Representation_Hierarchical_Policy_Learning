...
#SBATCH --job-name mug-d1-wca-gated-res-100demo-resnet
...
# 100-demo Option A: weighted cross-attention + ReZero-style asymmetric gated
# goal residual + ResNet-18 visual encoder. Uses the first NUM_DEMOS demos
# (demo_0.h5 through demo_(NUM_DEMOS-1).h5) from the full 1000-trajectory
# dataset on /ocean. NUM_DEMOS defaults to 100 — override with
# `NUM_DEMOS=200 sbatch …`.
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_GatedRes_${NUM_DEMOS}demo_resnet_MugCleanup_D1 \
    name=groot_GMM_WCA_GatedRes_${NUM_DEMOS}demo_resnet_MugCleanup_D1 \
