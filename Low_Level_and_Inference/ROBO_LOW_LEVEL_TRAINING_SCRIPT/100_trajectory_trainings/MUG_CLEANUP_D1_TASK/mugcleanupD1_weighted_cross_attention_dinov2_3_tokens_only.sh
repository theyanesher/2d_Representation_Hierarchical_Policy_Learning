...
#SBATCH --job-name mug-d1-wca-100demo-dinov2-3tok-only
...
# Ablation: per-component state encoding ONLY (no state-AdaLN).
#
...
#     previously — better self-attention bandwidth (6/22 = 27% of query slots vs
#     2/18 = 11%) and explicit per-component structure via learned identity embeddings.
#
# State-AdaLN is OFF: norm1 remains standard AdaLayerNorm(temb). This isolates
# the effect of the per-component state encoding from the AdaLN modulation pathway.
# Compare against the state-AdaLN-only run to disentangle which contributes more
# to handling state-dependent failure cases.
#
...
pixi run python diffusion_policy/train.py \
    ...
    policy.use_per_component_state=true \
    policy.use_state_adaln=false \
    policy.gmm_top_k=32 \
    logging.project=MimicGen_GMM_Low_Level_Policy \
    logging.name=groot_GMM_WCA_3tok_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
    name=groot_GMM_WCA_3tok_${NUM_DEMOS}demo_dinov2_MugCleanup_D1 \
