...
#SBATCH --job-name mug-d1-wca-100demo-dinov2-rdp-gtmix
...
# RDP GT/predicted goal-MIX run on Mug_Cleanup_D1: per TRAIN sample,
...
#
# RDP twist vs ../mugcleanupD1_wca_dinov2_gt_predicted_mix.sh: the GT goals
# come from the EXTRA_KEYPOINTS npz tree (goal_gripper_pcd_rdp) so transition
# windows follow the RDP keypoint schedule, and VALIDATION (which always uses
# the predicted GMM) is served from the RDP prediction npz tree. The h5 files
# are read-only; their default gmm/goal keys are never loaded.
#
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_MugCleanup_D1_RDP_GTMIX_p${GT_MIX_P} \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_MugCleanup_D1_RDP_GTMIX_p${GT_MIX_P} \
