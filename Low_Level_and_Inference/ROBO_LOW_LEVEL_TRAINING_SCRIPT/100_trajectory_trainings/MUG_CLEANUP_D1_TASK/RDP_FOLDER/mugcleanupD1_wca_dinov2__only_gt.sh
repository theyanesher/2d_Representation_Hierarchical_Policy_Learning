...
#SBATCH --job-name mug-d1-wca-100demo-dinov2-rdp-onlygt
#SBATCH -o /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/Low_Level_Policy/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/ROBO_LOW_LEVEL_TRAINING_SCRIPT/100_trajectory_trainings/MUG_CLEANUP_D1_TASK/RDP_FOLDER/logs/job_%j.out
#SBATCH -e /ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Code/Low_Level_Policy/2d_Representation_Hierarchical_Policy_Learning/Low_Level_and_Inference/ROBO_LOW_LEVEL_TRAINING_SCRIPT/100_trajectory_trainings/MUG_CLEANUP_D1_TASK/RDP_FOLDER/logs/job_%j.err
#SBATCH --mail-type=END
#SBATCH --mail-user=pbhowal@andrew.cmu.edu

# RDP ONLY-GT run on Mug_Cleanup_D1 (gt_mix_p=1.0 -> every TRAIN sample uses
# the ground-truth RDP goal set, never the high-level prediction):
...
#
# RDP twist vs ../mugcleanupD1_wca_dinov2__only_gt.sh: the GT goals come from
# the EXTRA_KEYPOINTS npz tree (goal_gripper_pcd_rdp) so transition windows
# follow the RDP keypoint schedule, and VALIDATION (which always uses the
# predicted GMM) is served from the RDP prediction npz tree. The h5 files are
# read-only; their default gmm/goal keys are never loaded.
#
# GT_MIX_P is pinned to 1.0 here. NUM_DEMOS=100. Override at submission time:
...
echo "[gt_mix] gt_mix_p=${GT_MIX_P}  (ONLY ground-truth RDP goals; high-level prediction never used in train)"
...
PRED_SRC_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/Mug_Cleanup_D1_GMM_PRED"
GT_SRC_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/Mug_Cleanup_D1"
...
DEST_PRED_DIR="${SCRATCH_ROOT}/Mug_Cleanup_D1_GMM_PRED_${NUM_DEMOS}demo"
DEST_GT_DIR="${SCRATCH_ROOT}/Mug_Cleanup_D1_rdp_goals_${NUM_DEMOS}demo"
...
pixi run python diffusion_policy/train.py \
    ...
    logging.name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_MugCleanup_D1_RDP_ONLY_GT \
    name=groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_MugCleanup_D1_RDP_ONLY_GT \
