...
#SBATCH --job-name coffee-prep-d1-NO-GMM
...
# ===========================================================================
# APPROACH 2 on COFFEE_PREPERATION_D1  —  NO GMM  (ablation of the aux loss)
# ===========================================================================
# Same model as the c1 siblings in this folder, with the goal-GMM head and its
# loss removed entirely:
#
#     total_loss = flow_loss          (no c1 * gmm_loss term)
#
# Set via policy.aux_gmm_loss_weight=null. With null the head is never even
# CONSTRUCTED (not merely unused) and goal_gripper_pts is never read — see the
# short-circuit in FlowMatchingDiTGoalGMMPolicy.compute_loss. Use null rather
# than 0.0: numerically identical, but 0.0 would still build the head, run its
# forward and the ten-term NLL over 516 components every step, and put ~530K
# dead parameters under weight decay and in the EMA shadow.
#
# EVERYTHING ELSE IS IDENTICAL to the c1 siblings — the goal is still absent from
# the DiT input, and the model keeps:
#   * the DINOv2 -> projector -> RoPE4D trunk (2 blocks, 1032 tokens)
#   * per-patch 3D anchors unprojected from depth
#   * the 8 gripper-keypoint grounded tokens from present_gripper_pts
#   * DiT cross-attention into those grounded tokens
# so this ablates the auxiliary LOSS, not the architecture.
#
# Because the GMM head is constructed LAST in the policy __init__ (after the
# DiT, the state/action encoders and the grounded encoder), it consumes RNG only
# after every shared parameter is initialised. With seed=42 and the same data
# order, this run and the c1 runs start from identical weights on every shared
# parameter and see identical batches — the auxiliary gradient is the only
...
# present_gripper_pts) are already present in those files. Note the dataset
# directory name "NO_GMM_DATASET" is unrelated to this script's name — it refers
# to the h5 files carrying a single ground-truth goal instead of a GMM.
#
...
echo "[config] NO GMM (aux_gmm_loss_weight=null), NUM_DEMOS=${NUM_DEMOS} (demo_0.h5 .. demo_$((NUM_DEMOS-1)).h5)"
...
RUN_NAME="coffee_prep_D1_approach2_NO_GMM_${NUM_DEMOS}demo"
...
#
# Keep every knob below identical to the c1 sibling scripts; a comparison across
# differing batch size / epochs / seed says nothing about the auxiliary loss.
USE_TF=0 \
