...
#SBATCH -t 48:00:00 # 12-hour budget
#SBATCH --job-name coffee-prep-d1-armD-gmm-aux-100demo
...
# ===========================================================================
# APPROACH 2 — ARM D (treatment): no goal input + goal-GMM auxiliary loss.
# ===========================================================================
...
# being an input.
#
# Pair with coffee_prep_d1_armB_control.sh (identical architecture,
# aux_gmm_loss_weight=null). B vs D isolates the auxiliary loss as the single
# variable — do not compare D against the goal-conditioned baseline alone, since
# that changes both the goal input and the architecture.
#
...
#
# Trains from scratch — the architecture differs from the baseline, so the
# goal_gripper checkpoints will not load. Expect to need more than one 12-hour
# job; resume with:
...
# --- resume from checkpoint (empty => from scratch) ----------------------
...
pixi run python diffusion_policy/train.py \
    ...
    policy.aux_gmm_loss_weight=0.01 \
    logging.project=mimicgen_tasks \
    logging.name=coffee_prep_D1_approach2_armD_gmmaux_${NUM_DEMOS}demo \
    name=coffee_prep_D1_approach2_armD_gmmaux_${NUM_DEMOS}demo \
