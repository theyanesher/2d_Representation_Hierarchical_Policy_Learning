...
#SBATCH --job-name hammer-cleanup-d1-c1-0.1
...
# ===========================================================================
# APPROACH 2 on HAMMER_CLEANUP_D1  —  c1 = 0.1
# ===========================================================================
...
#     fm_loss  ~ 1.29     |grad_fm  -> shared trunk| ~ 1.25
#     gmm_loss ~ 8.00     |grad_gmm -> shared trunk| ~ 3.95  (per unit c1)
#   c1 = 0.161 equalises the two loss magnitudes
#   c1 = 0.318 equalises the gradient each delivers to the shared trunk
#   c1 = 0.1   -> auxiliary gradient is ~31% of the flow gradient: a real
#                 signal, but subordinate to the actual task. Start here.
# (Coffee_Prep_D1 measured 26% at the same c1, so 0.1 is comparable across tasks.)
...
#
# NOTE: HAMMER_CLEANUP_D1 has exactly 100 converted h5 files, so the default
# NUM_DEMOS=100 consumes all of them — NUM_DEMOS>100 will fail the staging check.
#
...
DEST_DATA_DIR="${SCRATCH_ROOT}/Hammer_Cleanup_D1_Approach2_${NUM_DEMOS}demo"
...
RUN_NAME="hammercleanup_D1_APPROACH2_c1_${C1}_${NUM_DEMOS}demo"
