...
#SBATCH -t 30:00:00 # fresh 100-epoch budget: ~17.3 min/epoch on H100
                    # (alpha0.1 run: 100 epochs in 28h51m) + margin.
#SBATCH --job-name kitchen-d1-wca-alpha0.001-ablation
...
# ALPHA ABLATION on KITCHEN_D1 (100 demos, DINOv2, serial WCA) — ALPHA=0.001 variant.
#
# Same treatment as kitchenD1_wca_dinov2_alpha0.1_ablation.sh, one knob
# changed: ALPHA defaults to 0.001 — even deeper into the PRIOR-ONLY regime than 0.01. Per the interpretation anchors
# (content-logit spread ~10): 0.3 ~ effective top-6, 0.1 ~ top-1..3,
# <=0.01 ~ PRIOR-ONLY — at this alpha the WCA content logits are essentially
# muted and attention follows the high-level GMM weights (log w) almost
# exactly, i.e. this run probes "trust the high-level prior" as a policy.
#
#   logits = ALPHA * QK^T/sqrt(d) + log(w_j)
#
# Launch matrix (env overrides, same pattern as the other scripts):
#   sbatch this_script.sh                       # top-2500, alpha=0.001
#   ALPHA=0.01  sbatch this_script.sh           # top-2500, alpha=0.01
...
# --- ablation knobs -------------------------------------------------------
ALPHA="${ALPHA:-0.001}"         # wca_alpha value (used only when USE_ALPHA=true)
