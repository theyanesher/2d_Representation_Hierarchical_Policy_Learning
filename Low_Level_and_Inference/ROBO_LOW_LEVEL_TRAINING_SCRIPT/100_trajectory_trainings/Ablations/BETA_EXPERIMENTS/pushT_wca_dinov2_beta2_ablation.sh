...
#SBATCH -t 20:00:00 # fresh 200-epoch budget: ~5.8 min/epoch on H100
                    # (alpha0.01/0.001 runs: 200 epochs in ~18h45m) + margin.
#SBATCH --job-name push-t-wca-beta2-ablation
...
# BETA ABLATION on PushT_Task GOAL_SWAP (DINOv2, serial WCA, GT-mix) — BETA=2 variant.
#
...
#   sbatch this_script.sh                       # top-2500, alpha=1.0, beta=2
#   BETA=3 sbatch this_script.sh                # top-2500, beta=3
...
# --- ablation knobs -------------------------------------------------------
BETA="${BETA:-2}"              # wca_beta: prior temperature on log(w) (1.0 = original)
...
RUN_NAME="groot_GMM_WCA_${NUM_DEMOS}demo_dinov2_PushT_Task_GOALSWAP_GTMIX_p${GT_MIX_P}_top${TOP_K}_${ALPHA_TAG}_beta${BETA}_bs${BATCH_SIZE}"
