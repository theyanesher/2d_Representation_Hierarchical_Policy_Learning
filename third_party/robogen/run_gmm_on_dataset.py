...
# Import only the model-related functions directly to avoid pulling in
# the real-world robot stack (pyk4a, ROS, etc.) via eval_policy imports.
...
def process_demo_dir(demo_dir, goal_policy, cat_embedding, args):
    ...
    # Save as <dataset_dir>/<demo_name>.h5, mirroring All_Heatmap_Dataset layout exactly
...
if __name__ == '__main__':
    ...
    parser.add_argument('--high_level_ckpt_path', type=str, required=True,  help='Path to high-level model checkpoint (.pth)')
    parser.add_argument('--cat_idx',              type=int, required=True,  help='Task category index for SigLIP embedding')
    ...
    # Load SigLIP task embedding using the provided cat_idx directly
