...
import numpy as np
# from equi_diffpo.model.vision.pointnet_extractor import replace_bn_with_gn
try:
...
class PointNet2_small2(nn.Module):
    def __init__(self, num_classes,  in_channels=6):
        ...
        self.sa1 = PointNetSetAbstractionMsg(npoint=1024, radius_list=[0.05, 0.1], nsample_list=[16, 32], in_channel=in_channels-3, mlp_list=[[16, 16, 16], [32, 32, 32]])
        ...
        self.sa3 = PointNetSetAbstractionMsg(64, [0.2, 0.4], [16, 32], 128, [[128, 192, 128], [128, 192, 128]])
    ...
    def forward(self, xyz):
        if xyz.shape[2] == 3 or xyz.shape[2] == 6: 
            xyz = xyz.permute(0, 2, 1)
        l0_points = xyz
...
class PointNet2_super(nn.Module):
    def __init__(self, num_classes, input_channel=3, keep_gripper_in_fps=False, use_group_norm=False):
        ...
        if use_group_norm:
            for i in range(1, 7):
                ...
                # print(f'replacing batch norm in sa{i}')
                replace_bn_with_gn(sa)
            for i in range(1, 7):
                fp = getattr(self, f"fp{i}")
                # print(f'replacing batch norm in fp{i}')
                replace_bn_with_gn(fp)
            print(self.sa3)
