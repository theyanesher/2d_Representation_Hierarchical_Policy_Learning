...
class PointTransformer_super(nn.Module):
    def __init__(self, num_classes, input_channel=3):
        ...
        self.conv2 = nn.Conv1d(64, num_classes, kernel_size=1)

    def forward(self, xyz):
        B, C, N = xyz.shape
        # print("xyz.shape", xyz.shape)
        feat = xyz.permute(0, 2, 1).contiguous().reshape(-1, C)        # (B, N, C) → (B*N, C)
        coord = xyz[:, :3, :].permute(0, 2, 1).contiguous().reshape(-1, 3)  # (B, N, 3) → (B*N, 3)
        ...
        feat = point_out.feat # (B*N, 64)
        x = feat.view(B, N, -1) # (B, N, 64)
        x = x.permute(0, 2, 1).contiguous() # (B, 64, N)
