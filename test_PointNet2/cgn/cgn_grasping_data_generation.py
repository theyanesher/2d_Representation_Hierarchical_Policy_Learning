...
class ContactGraspNetEnv():
    def __init__(self, scene_path,
        ...
        # p.setPhysicsEngineParameter(numSubSteps=10, contactSlop=0.005, numSolverIterations=50, physicsClientId=self.id)
        # p.setPhysicsEngineParameter(numSubSteps=10, numSolverIterations=50, physicsClientId=self.id)
        res = p.getPhysicsEngineParameters(physicsClientId=self.id)
        # print(res)
        # import pdb; pdb.set_trace()
        ...
        panda_id = self.load_robot()
        self.urdf_ids['robot'] = panda_id
        ...
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[0], lateralFriction=friction, physicsClientId=self.id)
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[0], lateralFriction=friction, physicsClientId=self.id)
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[0], rollingFriction=friction, physicsClientId=self.id)
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[1], rollingFriction=friction, physicsClientId=self.id)
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[1], spinningFriction=friction, physicsClientId=self.id)
        p.changeDynamics(self.urdf_ids['robot'], self.robot.right_gripper_indices[1], spinningFriction=friction, physicsClientId=self.id)
    ...
    def load_robot(self):
        ...
        self.asset_dir = os.path.join(project_dir, "manipulation/assets/")
        from gym.utils import seeding
        np_random, _ = seeding.np_random()
        self.robot.init(self.asset_dir, self.id, np_random, fixed_base=True, use_suction=False)
        ...
        ### NOTE: randomize the robot base position
        robot_base_pos_z = self.floor_height + 0.06
        lower_x = self.scene_max_aabb[0] - 0.85
        upper_x = self.scene_min_aabb[0] - 0.2
        assert lower_x < upper_x
        lower_y = -0.2
        upper_y = 0.2
        
        robot_base_pos_x = -0.8 # np.random.uniform(lower_x, upper_x)
        robot_base_pos_y = 0 # np.random.uniform(lower_y, upper_y)
        robot_base_pos = [robot_base_pos_x, robot_base_pos_y, robot_base_pos_z]
    ...
    def load_scene(self):
        ...
        p.resetBasePositionAndOrientation(self.urdf_ids["plane"], [0, 0, self.floor_height], [0, 0, 0, 1], physicsClientId=self.id)
    ...
    def add_object(self, obj_path, obj_scale, obj_transform):
        ...
        volume = mesh.volume
        density = 1000 ## water density is 1000
        mass = volume * density 
        # mass = 1
    ...
    def set_camera(self, camera_width=640, camera_height=480, distance_ratio=1.5):
        ...
        ### TODO: when doing actual rendering, should randomize the camera position and orientation
        robot_base_pos = self.robot.get_base_pos_orient()[0]
        left_cam_center = np.array(robot_base_pos) + np.array([0, -0.2, 1.0])
        rigth_cam_center = np.array(robot_base_pos) + np.array([0, 0.2, 1.0])
        x_noise = np.random.uniform(-0.2, 0.2)
        y_noise = np.random.uniform(-0.2, 0.2)
        z_noise = np.random.uniform(-0.3, 0.3)
        
        scene_center = (self.scene_max_aabb + self.scene_min_aabb) / 2
        camera_target = scene_center
        
        camera_position = left_cam_center
        self.setup_camera(camera_position, camera_target, camera_width=camera_width, camera_height=camera_height)
        
    def get_obs(self):
        # TODO: get pc in world frame and subtract the robot pos to center it at robo origin
    ...
    def set_eef_to_pose(self, target_pos, target_quat):
        ...
        ### open gripper fully
        p.resetJointState(self.urdf_ids['robot'], self.robot.right_gripper_indices[0], 0.04, 0, physicsClientId=self.id)
        p.resetJointState(self.urdf_ids['robot'], self.robot.right_gripper_indices[0], 0.04, 0, physicsClientId=self.id)
    ...
    def step(self, grasp, debug=False):
        ...
        target_pos, target_rotation = grasp[:3, 3], grasp[:3, :3]
        target_rotation = target_rotation @ np.array([[0, 1, 0], [-1, 0, 0], [0, 0, 1]])  # flip the xy axis
        approaching_direction = target_rotation[:, 2]  # The z-axis of the grasp orientation
        target_pos = target_pos + approaching_direction * 0.1034
        target_quat = R.from_matrix(target_rotation).as_quat()
        target_orientation = target_quat
        
        mp_target_pos = target_pos - 0.05 * target_rotation[:, 2]  # move the end effector a bit back in the approaching direction
        real_target_pos = target_pos
        
        ### TODO: add an opening gripper stage
        
        ## TODO: change this to be motion planning        
        ...
        state = save_env(simulator, simplified=True)
        ...
        res, path, path_translation_length, path_rotation_length = motion_planning(
            simulator, mp_target_pos, target_orientation, obstacles=obstacles, allow_collision_links=allow_collision_links, save_path=None, 
            smooth_path=True, interpolation_num=interpolation_steps)

        if res:
            ### NOTE set the joint angle to reach the joint pose along the motion planned path
            stage_length['reach_pregrasp'] = len(path)
            # import pdb; pdb.set_trace()
            
            for idx, q in enumerate(path):
                self.robot.set_joint_angles(simulator.robot.right_arm_joint_indices, q)
                # right_eef_pos = simulator.robot.get_pos_orient(simulator.robot.right_end_effector)[0]
                # p.addUserDebugPoints([right_eef_pos], [[0, 1, 0]], 9, 0, physicsClientId=simulator.id)
                rgb = self.render()
                rgbs.append(rgb)
                state = save_env(simulator, simplified=True)
                intermediate_states.append(state)
            
            # move to the real grasp pose
            target_pos = real_target_pos
            target_link = self.robot.right_end_effector
            joint_angles = p.calculateInverseKinematics(self.urdf_ids['robot'], target_link, target_pos, target_quat, physicsClientId=self.id)
            p.setJointMotorControlArray(self.urdf_ids['robot'], jointIndices=self.robot.right_arm_joint_indices, 
                                        controlMode=p.POSITION_CONTROL, targetPositions=joint_angles[:7], physicsClientId=self.id)
            moving_stage = 20
            stage_length['reach_grasp'] = moving_stage
            # import pdb; pdb.set_trace()
            for _ in range(moving_stage):
                p.stepSimulation(physicsClientId=self.id)
                state = save_env(simulator, simplified=True)
                intermediate_states.append(state)
                rgb = self.render()
            ...
            # close the gripper
            p.setJointMotorControlArray(self.urdf_ids['robot'], jointIndices=self.robot.right_gripper_indices, 
                                        controlMode=p.POSITION_CONTROL, targetPositions=[0,0], physicsClientId=self.id)
            closing_stage = 15
            stage_length['close_gripper'] = closing_stage
            for _ in range(closing_stage):
                # import pdb; pdb.set_trace()
                p.stepSimulation(physicsClientId=self.id)
                state = save_env(simulator, simplified=True)
                intermediate_states.append(state)
            ...
            ### lift up
            lifting_stage = 50
            stage_length['lift_up'] = lifting_stage
            delta_movement = 0.02
            for i in range(lifting_stage):
                cur_eef_pos, cur_eef_orient = self.robot.get_pos_orient(self.robot.right_end_effector)
                target_eef_pos = cur_eef_pos + np.array([0, 0, delta_movement])
                target_link = self.robot.right_end_effector
                joint_angles = p.calculateInverseKinematics(self.urdf_ids['robot'], target_link, target_eef_pos, cur_eef_orient, physicsClientId=self.id)
                
                p.setJointMotorControlArray(self.urdf_ids['robot'], jointIndices=self.robot.right_arm_joint_indices, 
                                            controlMode=p.POSITION_CONTROL, targetPositions=joint_angles[:7], physicsClientId=self.id)
                # import pdb; pdb.set_trace()
                p.stepSimulation(physicsClientId=self.id)
                intermediate_states.append(save_env(simulator, simplified=True))
                rgb = self.render()
                rgbs.append(rgb)
            ...
            ### NOTE: check if the obj is still inside the gripper
            res = p.getContactPoints(bodyA=self.urdf_ids['robot'], bodyB=contact_body, physicsClientId=self.id)
            if len(res) > 0:
                return 1, intermediate_states, rgbs, stage_length
            else:
...
if __name__ == "__main__":
    ...
    for scene_path in scene_path_list:
        env = ContactGraspNetEnv(scene_path=scene_path, gui=True, num_points_in_pc=20000)
        ...
        grasp_locations = env.scene_filtered_grasps[:, :3, 3]  # grasp locations in the camera frame
        good_idx = grasp_locations[:, 0] < -0.1
        possible_grasps = env.scene_filtered_grasps[good_idx]
        for grasp in possible_grasps[::5][:10]:
            sucecss, states, rgbs, stage_length = env.step(grasp, debug=True)
            if sucecss:
                save_numpy_as_gif(np.array(rgbs), "{}/{}.gif".format("data", "debug"))
            import pdb; pdb.set_trace()
