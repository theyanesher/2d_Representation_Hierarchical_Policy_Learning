...
def draw_l2_distribution(l2_distribution):
    # l2: B
    plt.figure()
    plt.hist(l2_distribution, bins=20, density=True, stacked=True)
    plt.savefig('displacement/l2_distribution.png')
    # plt.show()
...
def eval(args):
    ...
    for i, data in enumerate(tqdm(dataset)):
        ...
        outputs = model(inputs_) # B, N, 12

        B, N, _ = outputs.shape
        outputs = outputs.view(B, N, 4, 3)
        outputs = outputs + inputs.unsqueeze(2) # B, N, 4, 3
        with torch.no_grad():
            outputs = outputs.mean(dim=1) # B, 4, 3
...
if __name__ == "__main__":
    args = parse_args()
    if args.all_zarr_path is None:
        temp_list = ["0705-obj-40147", "0705-obj-44817", "0705-obj-44962", "0705-obj-45132", "0705-obj-45219", "0705-obj-45243", "0705-obj-45332", "0705-obj-45378", "0705-obj-45384", "0705-obj-45463"]
        args.all_zarr_path = [f"/project_data/held/ziyuw2/Robogen-sim2real/data/dp3_demo/{s}" for s in temp_list]
    eval(args)
