...
def remove_duplicate_reset_traj_folder(traj_path: str):
    """
    ...
    # Copy files beg_t..last_idx (only those that exist in idxs)
...
def remove_duplicate_reset_whole_folder(root_path: str, num_workers: int = None):
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as ex:
        futures = [ex.submit(remove_duplicate_reset_traj_folder, p) for p in traj_paths]
...
if __name__ == "__main__":
    ...
    remove_duplicate_reset_whole_folder(root_path, num_workers=4)
