...
download_with_retry() {
  ...
  # Optional: avoid concurrent duplicate downloads of same zip
  lock="/tmp/gcs_cp_${base}.lock"
  exec 9>"$lock"
  flock -n 9 || { echo "🔒 Skip locked: $base"; return 0; }
  ...
  while (( attempt <= max )); do
    ...
    if gcloud --quiet storage cp "$file" "$target_dir"; then
      echo "✅ Success: $file"
      return 0
    else
...
#   )
    ...
    mapfile -t zips < <(
        gcloud --quiet storage ls "$source" | grep '\.zip$' | head -n "$ZIPS_LIMIT"
    )
    else
    mapfile -t zips < <(
        gcloud --quiet storage ls "$source" | grep '\.zip$'
    )
    fi
  ...
  # parallel -j "$JOBS" --line-buffer \
  #   bash -lc 'download_with_retry "$1" "$2"' _ {} "$target" ::: "${zips[@]}"
  #  parallel -j "$JOBS" --line-buffer \
  #   bash -lc 'download_with_retry "$1" "$2"' _ {} "$target" ::: "${zips[@]}"
