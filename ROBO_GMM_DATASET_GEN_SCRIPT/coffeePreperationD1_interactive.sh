#!/bin/bash
# Interactive (no-sbatch) GMM dataset generation for COFFEE_PREPERATION_D1 — ALL demos.
#
# Coffee twin of mugCleanupD1_interactive.sh. Same pipeline (stage npz -> run
# high-level GMM -> ship h5 back to /ocean), with --save_modes_separately so each
# h5 also gets the halo-collapsed obs/gmm_modes + obs/gmm_mode_weights. NO
# visualization.
#
# NOTE: COFFEE_PREPERATION_D1 has no h5 generated yet — this run creates them all
# from the per-step npz. FINAL_OCEAN_DIR is created fresh (mkdir -p); nothing is
# read from it, so a first-time generation behaves exactly like a re-run.
#
# Meant to be run *inside* an already-running interactive allocation (no #SBATCH
# header, no sbatch needed). Uses $SLURM_JOB_ID (set inside the allocation) for
# the node-local scratch dir, exactly like the batch script.
#
# --- HOW TO RUN ----------------------------------------------------------
...
#        bash coffeePreperationD1_interactive.sh
...
FINAL_OCEAN_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/LOW_LEVEL_WITH_GMM_DATASET_GROOT_STYLE_DATASET/D2/COFFEE_PREPERATION_D1"
...
DEST_H5_DIR="${SCRATCH_ROOT}/COFFEE_PREPERATION_D1_gmm_h5"    # converter outputs
