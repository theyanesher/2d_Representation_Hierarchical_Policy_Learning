...
#SBATCH -t 12:00:00
#SBATCH --job-name hammer-cleanup-d1-rdp-pred-gen
...
# Generate per-frame GMM prediction npz files for HammerCleanupD1 by running the
...
#      EXTRA_KEYPOINTS/HAMMER_CLEANUP_D1_GMM_PRED/ (with a free-space guard first —
...
FINAL_OCEAN_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/EXTRA_KEYPOINTS/HAMMER_CLEANUP_D1_GMM_PRED"

# --- checkpoint (EDIT HERE to change) --------------------------------------
CKPT_PATH="${REPO_DIR}/logs/train_HammerCleanup_D1_GOAL_SWAP_RDP_100demo/2026-07-05/17-03-47/checkpoints/periodic-epoch=epoch=74.ckpt"
if [ ! -f "${CKPT_PATH}" ]; then
...
DEST_PRED_DIR="${SCRATCH_ROOT}/HAMMER_CLEANUP_D1_GMM_PRED"  # generator outputs
