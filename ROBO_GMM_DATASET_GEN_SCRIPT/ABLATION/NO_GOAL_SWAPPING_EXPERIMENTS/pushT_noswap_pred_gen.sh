...
#SBATCH -t 12:00:00
#SBATCH --job-name push-t-noswap-pred-gen
...
# Generate per-frame GMM prediction npz files for PushT_Task from the
# NO-GOAL-SWAP ablation high-level checkpoint (trained WITHOUT
# transition_label_swap and WITHOUT the weighted sampler).
# Same pipeline as ../../RDP_DATAGEN/kitchenD1.sh but with --key_suffix
# noswap (this checkpoint was trained on the STANDARD goals, not RDP —
# keys become gmm_pred_goal_noswap / gmm_all_goals_noswap /
# gmm_all_weights_noswap so they can never collide with the _rdp tree).
#
# NUM_DEMOS defaults to 206 = ALL PushT demos: the low-level WCA PushT
# experiments train on all 206 demos (unlike kitchen's first-100).
#
# Pipeline:
...
#      D2/ABLATIONS/NO_GOAL_SWAPPING_EXPERIMENTS_GMM_PREDICTIONS/PushT/
#      (with a free-space guard first).
#
...
FINAL_OCEAN_DIR="/ocean/projects/cis240052p/pbhowal/2d_Representation_Hierarchical_Policy_Learning/MimicGen_Uncertainty_Dataset/D2/ABLATIONS/NO_GOAL_SWAPPING_EXPERIMENTS_GMM_PREDICTIONS/PushT"

# --- checkpoint (EDIT HERE to change) --------------------------------------
CKPT_PATH="${REPO_DIR}/logs/ABLATION/NO_GOAL_SWAPPING_EXPERIMENTS/PUSH_T_TASK/train_PushT_Task_NO_GOAL_SWAP/2026-07-30/20-32-07/checkpoints/periodic-epoch=epoch=74.ckpt"
if [ ! -f "${CKPT_PATH}" ]; then
...
DEST_PRED_DIR="${SCRATCH_ROOT}/PushT_GMM_PRED_NOSWAP"         # generator outputs
